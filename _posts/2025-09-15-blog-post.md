...
<p><em>Context.</em> In Sec. 3 we iteratively <strong>straightened</strong> FM with ReFlow; in Sec. 4 we <strong>distilled</strong> a rectified teacher into a one-step student (InstaFlow). Both still inherit a core mismatch: training learns an <em>instantaneous</em> velocity field <code>v(x,t)</code>, while inference with few/one steps requires executing a <em>large discrete jump</em>. The <strong>Shortcut model</strong> addresses this by directly conditioning on the desired step size and learning the <em>finite-interval direction</em> to the next point. It thereby bridges multi-step and one-step generation within a <em>single</em> model.</p>
