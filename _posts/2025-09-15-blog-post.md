  ...
  <ul>
	...
	<li><a href="#section2">2. Straighten trajectories for Efficient Flow Matching</a>
		<ul>
			...
			<li><a href="#section2.2">2.2 ReFlow — Iterative Rectification of Flow Matching</a>
				<ul>
...
where $x_0 \sim p_{\text{init}}$ and $x_1 \sim p_{\text{data}}$, such definition set to drive the flow to follow the direction $x_1 − x_0$ of the linear path pointing from $x_0$ to $x_1$ as much as possible.
...
& = \underbrace{[\partial_{x_t} u, \partial_{t} u, \partial_{t_{\text{end}}} u]}_{\text{Jacobian Matrix}} \cdot [v, 1, 0]^T
\end{align}
$$
...
Let $u_\theta(x_t, t, t_{\text{end}})$ denotes the neural network predicting the mean velocity. Substituting Eq. (2) into Eq. (1) yields a regression target:
[
u_{\text{target}}
= v_t - (t-r),
\Big(v_t \cdot \partial_z u_\theta
+ \partial_t u_\theta\Big),
\tag{3}
]
where (v_t = a_t' x_0 + b_t' \epsilon) is the *conditional velocity*, an unbiased estimator of the marginal (v(z_t, t)).
The training loss is a simple mean-squared error:
[
\mathcal L(\theta)
= \big|
u_\theta(z_t, r, t)
...
When (r{=}t), the second term vanishes and the loss degenerates to Flow Matching, ensuring compatibility.

**Sampling rule (1-NFE):**
[
x_0
= \epsilon
- u_\theta(\epsilon, r{=}0, t{=}1).
\tag{5}
]
