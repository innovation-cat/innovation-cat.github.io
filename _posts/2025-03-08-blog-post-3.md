...
<details>
  <summary><strong>Tables of Contents</strong></summary>

- [Numerical ODE Solver](#numerical-ode-solver)
  - [One-step vs Multi-step](#one-step-vs-multi-step)
  - [Implicit vs Explicit](#implicit-vs-explicit)
- [Local Truncation Error and Global Truncation Error](#local-truncation-error-and-global-truncation-error)
  - [Example with Euler Method](#example-with-euler-method)
  - [Example with Heun Method](#example-with-heun-method)
- [Numerical ODE Solvers for Sampling](#numerical-ode-solvers-for-sampling)
</details>



Mathematically, the forward process can be modeled as a stochastic differential equation
A key insight is that reverse SDE can be equivalently represented as a deterministic ordinary differential equation (ODE) known as the probability flow ODE (PF ODE):
$\frac{dx}{dt} = f(x, t) - \frac{1}{2} g(t)^2 \nabla_x \log p_t(x), $
where $\nabla_x \log p_t(x)$ is the score function. Solving this ODE backward from noise to data produces trajectories with the same marginal probability distributions as the SDE trajectories, but without stochasticity, making sampling deterministic and potentially more efficient. This equivalence arises because the PF ODE is derived by matching the Fokker-Planck equation of the SDE, ensuring identical probability flows. Thus, sampling in diffusion models reduces to numerically integrating this ODE from $t = T$ (pure noise) to $t = 0$ (clean data).
...
# Local Truncation Error and Global Truncation Error
...
# Numerical ODE Solvers for Sampling
