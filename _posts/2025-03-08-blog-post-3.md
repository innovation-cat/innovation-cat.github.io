...
- Predictor: $x_n + h k_1 = x(t_n) + h y'(t_n)$.

- $k_2 = f(t_n + h, x(t_n) + h x'(t_n))$. Taylor-expand $f$ in two variables:
$$f(t_n + h, x(t_n) + h x'(t_n)) = f(t_n, x(t_n)) + h \frac{\partial f}{\partial t} + h x'(t_n) \frac{\partial f}{\partial x} + \frac{h^2}{2} \left( \frac{\partial^2 f}{\partial t^2} + 2 x' \frac{\partial^2 f}{\partial t \partial x} + (x')^2 \frac{\partial^2 f}{\partial x^2} \right) + O(h^3)$$

But since $x'' = \frac{\partial f}{\partial t} + x' \frac{\partial f}{\partial x}$, $k_2 = x' + h x'' + \frac{h^2}{2} x''' + O(h^3)$.

- Then, $x_{n+1}^{\text{approx}} = x(t_n) + \frac{h}{2} (x' + x' + h x'' + \frac{h^2}{2} x''' + O(h^3)) = x(t_n) + h x' + \frac{h^2}{2} x'' + \frac{h^3}{4} x''' + O(h^4)$.
