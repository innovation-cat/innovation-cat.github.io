...
- $k_2 = f(t_n + h, x(t_n) + h x'(t_n))$. Taylor-expand $f$ in two variables:
 $$
 \begin{align}
 f(t_n + h, x(t_n) + h x'(t_n)) = & f(t_n, x(t_n)) + h (\frac{\partial f}{\partial t} + x'(t_n) \frac{\partial f}{\partial x}) + \\ & \frac{h^2}{2} \left( \frac{\partial^2 f}{\partial t^2} + 2 x' \frac{\partial^2 f}{\partial t \partial x} + (x')^2 \frac{\partial^2 f}{\partial x^2} \right) + O(h^3)
 \end{align}
 $$
