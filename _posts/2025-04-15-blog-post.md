...
- **Step 1: Taylor expansion of the CD update.** For a given sample $x_{t_{n}}$ and timestep (t_{n}), CD uses the Euler solver to generate the teacher prediction:

  $$
  \hat{x}_{t_{n-1}} = x_{t_{n}} + (t_{n} - t_{n-1})\,t_n\,s_\theta(x_{t_n},t_n)
  $$
  
  Expanding the target network around $(x_{t_{n}}, t_{n})$ by first-order Taylor expansion

  $$\small
  f_{\theta^-}(\hat{x}_{t_{n-1}}, t_{n-1}) = f_{\theta^-}(x_{t_{n}}, t_{n}) - \Delta t \big[
  \partial_x f_{\theta^-}(x_{t_{n}}, t_{n}) \, t_{n}s_\theta(x_{t_{n}},t_{n}) + \partial_t f_{\theta^-}(x_{t_{n}}, t_{n})
    \big]
  + O(\Delta t^2).
  $$

- **Step 2: Taylor expansion of the CD update.** For a given sample $x_{t_{n}}$ and timestep (t_{n}), CD uses the Euler solver to generate the teacher prediction:
  
  $$\small
  \begin{align}
  f_{\theta^-}(\hat{x}_{t_{n-1}}, t_{n-1}) & = f_{\theta^-}(x_{t_{n}} + (t_{n} - t_{n-1})\,t_n\,s_\theta(x_{t_n},t_n), t_{n-1}+\Delta t) \\[10pt]
  & = f_{\theta^-}(x_{t_{n}}, t_n) + \partial_x f_{\theta^-}(x_{t_{n}}, t_{n})((t_{n} - t_{n-1})\,t_n\,s_\theta(x_{t_n},t_n)) + \partial_t f_{\theta^-}(x_{t_{n}}, t_{n})(-\Delta t) \\[10pt]
  & = f_{\theta^-}(x_{t_{n}}, t_n) + (-\Delta t)(\partial_x f_{\theta^-}(x_{t_{n}}, t_{n})v_{\theta}+\partial_t f_{\theta^-}(x_{t_{n}}, t_{n}))
  \end{align}
  $$
