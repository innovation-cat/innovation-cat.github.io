...
In practice, the **teacher netfork** $ f_{\theta^-} $ is often implemented as an **exponential moving average (EMA)** of the student network $f_\theta$:
...
Empirically, EMA teachers have been found to yield significantly more stable training dynamics and higher-quality generations in both diffusion and consistency models.
Without EMA, the target signal ( \hat{x}*0 ) may fluctuate rapidly, breaking the desired temporal invariance and destabilizing the learning of ( f*\theta(x_t, t) ).
