...
f_{\theta^-}(x_{t-\Delta t}, t-\Delta t) & = f_{\theta^-}(x_t, t) -\Delta t\Big(\nabla_x f_{\theta^-}(x_t, t),\frac{dx_t}{dt} + \partial_t f_{\theta^-}(x_t, t)\Big) + O(\Delta t^2) \\[10pt]
  & = f_{\theta^-}(x_t, t) -\Delta t,\frac{d}{dt}f_{\theta^-}(x_t, t) + O(\Delta t^2),
\end{aligned}
$$
...
=\nabla_x f_{\theta^-}(x_t,t)\,\frac{dx_t}{dt}+\partial_t f_{\theta^-}(x_t,t)
$$

represents the **tangent of $f_{\theta^-}$ along the PF-ODE trajectory**.
