...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul>
    <li><a href="#section1">1. Foundation and Preliminaries</a>
		<ul>
...
- **Elimination of Discretization Error.** 
  ...
  In contrast, $\mathcal{L}{\text{cont}}$ evaluates the tangent term $\tfrac{d}{dt}f{\theta^-}$ analytically via Jacobian–vector products (JVPs), avoiding numerical ODE steps entirely. This yields exact supervision at each time $t$, improving both accuracy and training stability.
