...
$$
\mathcal{L}_{\text{CD}} = \mathbb{E}_{x_0, t, \epsilon} \Big[w(t) \, \big\| f_\theta(x_t, t) - f_\theta(\hat{x}_{t_{n-1}}, t_{n-1}) \big\|_2^2\Big],
$$
...
<h1 id="section2.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.1 Problem Definition</h1> 
...
- **Step 2**: Use the **pretrained diffusion model $s_{\theta}$** to integrate one ODE step to obtain adjacent position.
  ...
  $$
  \hat{x}_{t_{n-1}} = x_{t_{n}} - (t_{n-1} - t_{n})t_n s_{\theta}(x_{t_n}, {t_n})\label{eq:cd}
  $$
