...
date: 2025-07-02
excerpt: "In this post, the author provides a clear and insightful introduction to flow matching, a continuous‑time generative modeling framework. The central idea is to train a time-dependent vector field $v_{\theta}(t,x)$ whose flow transforms a simple prior distribution—such as a standard Gaussian at $t=0$ —into the target data distribution by $t=1$. The article, aiming to establish foundational understanding, walks readers through the key mathematical definitions and motivations behind flow matching, including the notion of velocity fields and the corresponding ordinary differential equation dynamics"
