...
<h1 id="section4" style="color: #1E3A8A; font-size: 27px; font-weight: bold; text-decoration: underline;">4. Diffusion-Based and Flow-Based Models as Generative Priors</h1>  
...
$$
\mathbf{x}_{0\text{-pred}} = [\mathbf{x}*t - \sqrt{1-\bar\alpha_t},\epsilon*\theta(\mathbf{x}_t,t)]/\sqrt{\bar\alpha_t}
$$
...
+ \nabla_{\mathbf{x}*t}\!\log p(\mathbf{y}\mid\mathbf{x}_{0\text{-pred}})\big]
$$

drives samples toward regions consistent with both the learned prior and the measurements.
This stochastic integration **implicitly constrains trajectories to the intersection of the diffusion manifold and the observation manifold**, producing diverse yet measurement-faithful reconstructions.
