...
for k in range(K):
    ...
    # Data-fidelity gradient (w.r.t. x) then push to z via VJP (vector-Jacobian product)
...
- **Pixel-Space MAP (optimize over $x$).** The Objective is .
  ...
  for k in range(iters):
    ...
    # compute ∇_x log p(x) via implicit differentiation / JVPs
    grad_prior = grad_x_logp_x(x, z)          # framework-specific
    x = x + η * (grad_prior - grad_likelihood(x) * sigma2)  # MAP step
  return x
  ```
