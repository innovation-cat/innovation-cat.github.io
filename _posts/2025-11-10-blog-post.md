...
A standard GAN, consisting of a generator $\mathcal{G}(\mathbf{z})$ and a discriminator, does not provide a tractable method for evaluating $p(\mathbf{x})$ for an arbitrary signal $\mathbf{x}$. The discriminator learns a proxy for the data distribution, but it does not yield a normalized probability density.

Therefore, applying a GAN prior does not involve formulating an explicit regularizer $R(\mathbf{x})$ that is then added to the data fidelity term. Instead, the prior is enforced through a **reparameterization of the solution space**. It acts as a powerful **manifold constraint**, fundamentally changing the nature of the optimization problem itself.

A pre-trained GAN generator, $\mathcal{G}: \mathcal{Z} \rightarrow \mathbb{R}^n$, learns a mapping from a simple, low-dimensional latent space $\mathcal{Z}$ (e.g., $\mathbb{R}^d$ where $d \ll n$) to the high-dimensional signal space $\mathbb{R}^n$. The core assumption when using a GAN as a prior is that all plausible signals (e.g., natural images) lie on or very close to the low-dimensional manifold defined by the **range of the generator**. We denote this manifold as $\mathcal{M}$:
...
$$
R_{\text{GAN}}(\mathbf{x}) = \begin{cases} 0 & \text{if } \mathbf{x} \in \mathcal{M} \\ \\
...
$$
\hat{\mathbf{x}} = \arg\min_{\mathbf{x} \in \mathbb{R}^n} \left( \|\mathbf{y} - \mathbf{A}\mathbf{x}\|_2^2 + \lambda R_{\text{GAN}}(\mathbf{x}) \right)
$$
...
The search for the solution $\mathbf{x}$ is transformed into a search in the much lower-dimensional latent space for a code $\mathbf{z}$ that generates a consistent signal.

The objective function is reformulated as:

$$
\hat{\mathbf{z}} = \arg\min_{\mathbf{z}} \|\mathbf{y} - \mathbf{A}(\mathcal{G}(\mathbf{z}))\|_2^2 + \lambda R_{latent}(\mathbf{z})
$$

Once $\hat{\mathbf{z}}$ is found, the final estimate is $\hat{\mathbf{x}} = \mathcal{G}(\hat{\mathbf{z}})$. The optional term $R_{latent}(\mathbf{z})$ can be used to regularize the latent code, for example, by penalizing its L2-norm ($R_{latent}(\mathbf{z}) = \|\mathbf{z}\|_2^2$) to encourage solutions from the high-density region of the prior $p(\mathbf{z})$ (typically a standard normal distribution). GANs can produce exceptionally high-quality and sharp reconstructions, but their latent spaces can be non-convex and discontinuous, making the optimization challenging.


---

<h1 id="section3.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.2 Variational Autoencoders (VAEs): Latent-Variable Priors</h1> 


A VAE consists of an encoder $\mathcal{E}$ and a decoder (generator) $\mathcal{G}$. It learns a structured latent space by maximizing a lower bound on the data log-likelihood. Similar to GANs, the decoder $\mathcal{G}$ can be used as a prior. The optimization can be performed in the latent space:

$$
\hat{\mathbf{z}} = \arg\min_{\mathbf{z}} \|\mathbf{y} - \mathbf{A}(\mathcal{G}(\mathbf{z}))\|_2^2
$$

VAEs typically produce smoother latent spaces than GANs, which can aid optimization. However, their reconstructions tend to be less sharp and can suffer from blurriness, a known artifact of the VAE training objective.
...
Normalizing Flows are a class of generative models constructed from a sequence of invertible transformations. A flow model $f$ provides a bijective mapping between the data space $\mathbf{x}$ and a simple latent distribution $p_Z(\mathbf{z})$ (e.g., Gaussian), i.e., $\mathbf{z} = f(\mathbf{x})$. A key advantage is that the exact log-likelihood of a sample can be calculated using the change of variables formula:

$$
\log p(\mathbf{x}) = \log p_Z(f(\mathbf{x})) + \log \left| \det\left( \mathbf{J}_{f}(\mathbf{x}) \right) \right|
$$

where $\mathbf{J}_{f}$ is the Jacobian of the transformation. This ability to compute $\log p(\mathbf{x})$ directly makes Normalizing Flows a perfect candidate for the MAP estimation framework. The objective function becomes:

$$
\hat{\mathbf{x}} = \arg\min_{\mathbf{x}} \left[ \|\mathbf{y} - \mathbf{A}\mathbf{x}\|_2^2 - \lambda \left( \log p_Z(f(\mathbf{x})) + \log \left| \det\left( \mathbf{J}_{f}(\mathbf{x}) \right) \right| \right) \right]
$$
