...
<h1 id="section6.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">6.3 Decoupling Prior Trajectory and Optimization</h1>

Compared with noisy-space posterior-guided sampling, the **decoupled design**: prior trajectory in $$\mathbf{x}_t$$  +  clean-space local MAP optimization in $$\mathbf{x}_0$$ offers several concrete advantages for real-world inverse problems.

---

**(a) Explicit control over the prior–likelihood trade-off.** In noisy-space schemes, the relative strength of prior and likelihood is often entangled in a single guidance weight schedule and in the discretization of the reverse SDE/ODE. In the local-MAP formulation:

- The **prior strength** is encoded by how tightly the quadratic term in (6.1) constrains $$\mathbf{x}_0$$ around $$\hat{\mathbf{x}}_0^{(k)}$$.  
- The **likelihood strength** is controlled by $$\gamma_k$$ and by how accurately we solve the subproblem (number of CG iterations, stopping criteria, etc.).

This separation makes it straightforward to design **time-dependent schedules**: weaker DC updates at large noise levels (when $$\hat{\mathbf{x}}_0$$ is unreliable), stronger DC updates near $$t=0$$, or even problem-specific annealing strategies, without touching the prior sampler.

---

**(b) Modularity and reusability across tasks and solvers**

The prior trajectory is handled by any off-the-shelf sampler (DDPM, DDIM, DPM-Solver, PF-ODE, etc.), which can be reused **unchanged** across inverse problems. Adapting to a new task amounts to:

- specifying the forward operator $$A$$ and noise model;  
- designing an appropriate clean-space objective (often of the form (6.1), possibly with additional regularizers such as TV).

This is conceptually close to **plug-and-play priors**, but with a learned diffusion prior instead of a hand-crafted denoiser. Crucially, the local-MAP view allows one to plug in **mature optimization technology** (CG, ADMM, proximal splitting) inside the clean-space step, while treating the diffusion prior as a black-box manifold projector.

---

**(c) Improved numerical stability and geometric fidelity**

Noisy-space posterior guidance must carefully tune the step sizes of the likelihood gradient in $$\mathbf{x}_t$$; overly aggressive updates can easily drift off the data manifold and destabilize sampling. In contrast:

- Clean-space subproblems like (6.1) are **convex quadratics** that can be solved with well-understood algorithms and convergence guarantees.  
- DDS shows that, under reasonable assumptions, the implicit update in $$\mathbf{x}_t$$ induced by the clean-space CG step stays close to the **tangent space of the diffusion manifold**, preserving the geometric advantages of manifold-constrained methods such as MCG, but without explicit Jacobian–vector products.  
- When instabilities occur, it is often clear that they originate from the optimization side (e.g., too large $$\gamma_k$$ or too many inner iterations), which can be diagnosed and fixed without modifying the prior.

From this perspective, local-MAP optimization can be seen as a **geometrically faithful, numerically robust approximation** of the ideal manifold-constrained MAP problem.

---

**(d) Efficiency for large-scale inverse problems**

Finally, decoupling also brings significant **computational benefits**:

- The cost of the prior step is comparable to unconditional sampling.  
- The clean-space subproblem only requires **matrix–vector products with $$A$$ and $$A^\top$$**, which are typically available as fast operators (FFT, NUFFT, Radon transform, etc.).  
- No backpropagation through the diffusion network is needed during inference, in contrast to some posterior-guided schemes.

Empirically, DDS demonstrates that this design can achieve **state-of-the-art reconstruction quality** on challenging MRI and CT problems while being orders of magnitude faster than earlier diffusion-based inverse solvers[^dds].

---

In summary, **local-MAP optimization in the clean space** offers a principled and practical alternative to noisy-space posterior-guided sampling. It approximates the solution of the original inverse problem by iteratively solving local surrogate MAP problems on the data manifold, while cleanly separating the roles of the generative prior (manifold geometry) and the measurement model (data consistency). This separation is central not only for theoretical understanding but also for scalable, high-quality reconstructions in large-scale real-world applications.
