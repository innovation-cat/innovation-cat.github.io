...
This objective is also known as the Mean Squared Error (MSE) data fidelity term. While principled, it is insufficient for ill-posed problems, as many different signals $\mathbf{x}$ can yield a similarly small residual $$\|\mathbf{y} - \mathbf{A}\mathbf{x}\|_2^2$$, often resulting in solutions dominated by noise [1-3].
...
Normalizing flows (NFs) can precisely obtain the probability density $p(x)$, of variables  which enables us to directly apply Eq. \ref{eq:map} as the objective funtion. Specifically, NFs define an invertible, differentiable map 
...
In this section, we focus on **posterior-guided sampling**, a paradigm for solving inverse problems from the perspective of sampling. In this paradigm, we embed the inverse problem into the reverse-time dynamics of the diffusion model by replacing the prior score with a **posterior score**:
...
with $$\mathbf{w}_t$$ a standard Wiener process and $$p_t(\mathbf{x}_t)$$ the marginal density at time $$t$$. The evolution of $$p_t$$ is governed by the **Fokker–Planck equation**
...
\quad
\delta \mathbf{x}_{\text{tan}}\in T_{\hat{\mathbf{x}}_0}\mathcal{M},;
...
\nabla_{\mathbf{x}_t}\log p(\mathbf{y}\mid \mathbf{x}_t)
\;\approx\;
J_{\hat{\mathbf{x}}_0}^\top(\mathbf{x}_t)\,
...
- $$\nabla_{\mathbf{x}_0}\log p(\mathbf{y}\mid \mathbf{x}_0)$$ is the **clean-space data-consistency gradient**
  
  

- $$\frac{\partial \hat{\mathbf{x}}_0}{\partial \mathbf{x}_t}$$ is the **Jacobian of the denoiser** (or $x_0$-prediction network) w.r.t. $$\mathbf{x}_t$$.
...
- **Flexible weighting and scheduling.** The scalar $\eta_k$ (and possibly additional scaling of the DC gradient) implicitly defines the **relative strength** of prior vs likelihood at each time step: larger $\eta_k$ refers to stronger enforcement of data consistency at time $t_k$; smaller $\eta_k$ implies that stronger trust in the generative prior at that time.
  ...
  This allows practitioners to design **time-dependent guidance schedules**—for example, weak data-consistency at very noisy times (when measurements are not yet informative) and stronger guidance near (t=0) (when the denoised estimate is more reliable).
...
[1] **Tikhonov A N, Arsenin V Y.** Solutions of ill-posed problems[M]. Washington, DC: Winston, 1977.

[2] **Tarantola A.** Inverse problem theory and methods for model parameter estimation[M]. Philadelphia: SIAM, 2005.
...
[10] **Buades A, Coll B, Morel J M.** A non-local algorithm for image denoising[C]//IEEE Computer Society Conference on Computer Vision and Pattern Recognition. San Diego: IEEE, 2005: 60-65.
...
[13] **Venkatakrishnan S V, Bouman C A, Wohlberg B.** Plug-and-play priors for model based reconstruction[C]//2013 IEEE Global Conference on Signal and Information Processing. Austin: IEEE, 2013: 945-948.

[14] **Romano Y, Elad M, Milanfar P.** The little engine that could: regularization by denoising (RED)[J]. SIAM Journal on Imaging Sciences, 2017, 10(4): 1804-1844.

[15] **Goodfellow I J, Pouget-Abadie J, Mirza M, et al.** Generative adversarial nets[C]//Advances in Neural Information Processing Systems 27. Montréal: Curran Associates, 2014: 2672-2680.

[16] **Bora A, Jalal A, Price E, et al.** Compressed sensing using generative models[C]//International Conference on Machine Learning. Sydney: PMLR, 2017: 537-546.

[17] **Kingma D P, Welling M.** Auto-encoding variational Bayes[C]//2nd International Conference on Learning Representations. Banff, 2014.

[18] **Dinh L, Sohl-Dickstein J, Bengio S.** Density estimation using Real NVP[C]//International Conference on Learning Representations. Toulon, 2017.

[19] **Kingma D P, Dhariwal P.** Glow: Generative flow with invertible 1×1 convolutions[C]//Advances in Neural Information Processing Systems 31. Montréal: Curran Associates, 2018: 10215-10224.

[20] **LeCun Y, Chopra S, Hadsell R, et al.** A tutorial on energy-based learning[M]//Bakir G, Hofmann T, Schölkopf B, et al., eds. Predicting Structured Data. Cambridge, MA: MIT Press, 2006: 191-246.

[21] **Song Y, Sohl-Dickstein J, Kingma D P, et al.** Score-based generative modeling through stochastic differential equations[C]//International Conference on Learning Representations. 2021.

[22] **Ho J, Jain A, Abbeel P.** Denoising diffusion probabilistic models[C]//Advances in Neural Information Processing Systems 33. 2020: 6840-6851.

[23] **Song J, Meng C, Ermon S.** Denoising diffusion implicit models[C]//International Conference on Learning Representations. 2021.

[24] **Song Y, Shen L, Xing L, et al.** Solving inverse problems in medical imaging with score-based generative models[C]//International Conference on Learning Representations. 2022.

[25] **Chung H, Ye J C.** Diffusion posterior sampling for general noisy inverse problems[J]. SIAM Journal on Imaging Sciences, 2023, 16(1): 1-33.

[26] **Chung H, Ryu E, Ye J C.** Improving diffusion models for inverse problems using manifold constraints[J]. IEEE Journal of Selected Areas in Information Theory, 2022, 3(3): 529-544.

[27] **Chung H, Lee S, Ye J C.** Decomposed diffusion sampler for accelerating large-scale inverse problems[C]//Proceedings of the 12th International Conference on Learning Representations. 2024.
