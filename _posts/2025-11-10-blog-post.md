...
2. **Total Variation (TV):** A significant advancement over Tikhonov, the TV regularizer is defined as the L1-norm of the signal's gradient magnitude, this is equivalent to imposing a **Laplacian prior** on the gradient magnitudes.
...
$$
be the transform coefficients. Sparsity priors posit that most entries of $\boldsymbol{\alpha}$ are zero or near-zero. The standard regularizer is the **$\ell_1$-norm**:
$$
R(\mathbf{x}) = \|\mathbf{\Psi}\mathbf{x}\|_1.
$$

This choice can be viewed as imposing a **Laplace prior** on the coefficients, which is well known to promote sparsity. Depending on the application, $\mathbf{\Psi}$ may be:
...
\Longrightarrow \hat{\mathbf{z}} & = \arg\min_{\mathbf{z}} \left( \frac{1}{2\sigma^2} \|\mathbf{y} - \mathbf{A}(\mathcal{G}(\mathbf{z}))\|_2^2 + \lambda R(\mathbf{z}) \right)
\end{align}
$$
...
Given a measurement model $$\mathbf{y}=A\mathbf{x}+\mathbf{n}$$, Bayes’ rule yields the posterior
...
The second term is directly available from the diffusion model’s score estimator. Hence diffusion-family priors offer a **local gradient oracle** for the log-density of the data manifold, enabling two complementary solution paradigms.
...
Bayes’ rule gives the posterior at the clean level
