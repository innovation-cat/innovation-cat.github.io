...
In section [5](#section5), we viewed diffusion-based inverse problem solvers as **posterior-guided samplers** in the noisy space $$\{\mathbf{x}_t\}_{t\in[0,T]}$$: methods such as DPS and MCG inject a likelihood score $$\nabla_{\mathbf{x}_t}\log p(\mathbf{y}\mid \mathbf{x}_t)$$ into the reverse-time dynamics to approximate the posterior score $$\nabla_{\mathbf{x}_t}\log p(\mathbf{x}_t\mid\mathbf{y})$$ and sample from the posterior.

In this chapter we turn to a complementary perspective, exemplified by **Decomposed Diffusion Sampling (DDS)** [^dds], where the inverse problem is addressed via **local optimization in the clean space** $$\mathbf{x}_0$$. Instead of directly constructing a posterior score in $$\mathbf{x}_t$$, DDS alternates between

- a **prior step**, which maps a noisy state $$\mathbf{x}_t$$ to a denoised estimate $$\hat{\mathbf{x}}_0$$ and back, and  
- a **local MAP step in $$\mathbf{x}_0$$**, which optimizes a small clean-space objective that balances data consistency and prior faithfulness.

The result is what we will call a **local-MAP optimization paradigm in the clean space**. The outer loop still walks along a diffusion-style trajectory in time, but each time slice is mediated by an inner optimization in $$\mathbf{x}_0$$. 
...
As discussed in Chapter [4](#section4), directly minimizing this objective is difficult because:

- $$-\log p(\mathbf{x}_0)$$ is only defined **implicitly** through a time-dependent score oracle $$\nabla_{\mathbf{x}_t}\log p_t(\mathbf{x}_t),$$ rather than via a tractable closed form;  
- evaluating or differentiating $$\log p(\mathbf{y}\mid \mathbf{x}_0)$$ is easy, but combining it with the implicit prior in a single optimization in $$\mathbf{x}_0$$ is non-trivial.

The **local-MAP viewpoint** used by DDS circumvents these obstacles via two key ideas.
...
**(b) Iterating local surrogates ≈ optimizing the global posterior.** Solving \ref{eq:local_map} at a single time step only gives a **local** improvement. However, along the diffusion trajectory:
...
3. By repeatedly updating $$\mathbf{x}_0$$ via these surrogates and lifting it back to $$\mathbf{x}_{t_{k-1}}$$, we obtain a sequence whose clean endpoints $$\mathbf{x}_0^{(K)}, \mathbf{x}_0^{(K-1)}, \dots, \mathbf{x}_0^{(0)}$$ approximate a **descent trajectory of the true negative log-posterior**.

This is closely analogous to **majorization–minimization / proximal point methods** in classical optimization:

- At iteration $$k$$, construct a **simple upper bound** (majorizer) of the difficult term $$-\log p(\mathbf{x}_0)$$ around the current point (here: the quadratic in (6.1));  
- Minimize this bound plus the data term;  
- Iterate. Under mild assumptions, the sequence of iterates converges to a stationary point of the original objective.

DDS further leverages the **geometric structure of diffusion manifolds**: the Jacobian  

$$
\frac{\partial \hat{\mathbf{x}}_0}{\partial \mathbf{x}_t}
$$

acts approximately as a **projection onto the tangent space of the data manifold** at $$\hat{\mathbf{x}}_0$$. This implies that the effect of the clean-space update (6.1) on $$\mathbf{x}_t$$ mimics a **manifold-constrained gradient step** on the data term, i.e., an update of the form

$$
\mathbf{x}_0 \leftarrow
\mathbf{x}_0 - \eta\,\Pi_{T_{\hat{\mathbf{x}}_0}\mathcal{M}}
\nabla_{\mathbf{x}_0}\tfrac{1}{2}\|\mathbf{y}-A\mathbf{x}_0\|_2^2,
$$

where $$\mathcal{M}$$ denotes the data manifold and $$\Pi_{T_{\hat{\mathbf{x}}_0}\mathcal{M}}$$ is the tangent-space projector.

In the limit of small time steps and accurate local solutions, iterating (6.1) along the diffusion trajectory thus approximates solving the **manifold-constrained MAP problem**

$$
\min_{\mathbf{x}_0 \in \mathcal{M}}
\frac{1}{2\sigma_y^2}\|\mathbf{y}-A\mathbf{x}_0\|_2^2,
$$

which is precisely the inverse problem with a **hard generative manifold prior**. In this sense, **local optimization in $$\mathbf{x}_0$$ is not ad-hoc**: it is a principled way to approximate the original inverse problem’s MAP solution using only local information from the diffusion prior.
...
1. **Denoising / prior projection.**  From the current noisy state $\mathbf{x}_t$, we compute a clean estimate
   ...
   via an $x_0$-prediction network or Tweedie’s formula. Geometrically, $\hat{\mathbf{x}}_0$ can be interpreted as a local projection of $\mathbf{x}_t$ onto the data manifold.
...
This realizes a clean-space variant of the “prior + data-consistency” decomposition: the prior is used to set up a well-posed low-dimensional ML problem, which is then solved using standard linear algebra.
...
Let $\mathcal{M}_0$ denote the data manifold of clean images. At a given time $t$, the noising process generates a “blurred” manifold $\mathcal{M}_t$ consisting of points $\mathbf{x}_t$ lying in a tubular neighborhood of $\mathcal{M}_0$. A single DDS step at time $t$ can be viewed as:
...
From the unified perspective developed in Chapters 3–5, clean-space local-MAP sampling sits at the intersection of:
...
- Noisy-space methods operate directly on $\mathbf{x}_t$ and require Jacobian–vector products through the denoiser network to transport the likelihood gradient into noisy space;  
...
- DDS can be viewed as a **time-continuous PnP framework**, where the denoiser is the diffusion model’s $x_0$-prediction network and the outer loop follows a decreasing noise schedule;  
