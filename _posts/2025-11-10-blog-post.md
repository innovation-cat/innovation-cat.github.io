...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul>
	...
	<li><a href="#section4">4. Diffusion Models: Score-Based Priors</a></li>
...
<h1 id="section4" style="color: #1E3A8A; font-size: 27px; font-weight: bold; text-decoration: underline;">4. Diffusion Models: Score-Based Priors</h1>  
...
1. **Likelihood Score:** For Gaussian noise, 

   $$\log p(\mathbf{y}\mid \mathbf{x}) \propto -\|\mathbf{y}-\mathbf{A}\mathbf{x}\|_2^2 / (2\sigma^2)$$ 
...
```python
# Inputs: score s_theta(x_t, t), schedule {t_T ... t_0}, step sizes {Δt},
#         likelihood grad ∇_x log p(y|x_t) = -grad_likelihood(x_t) * σ^2 (if calibrated)
initialize x_T ~ N(0, I)     # or from your forward noising of a warm start
for t in reversed(schedule):
    prior_score = s_theta(x_t, t)                  # ≈ ∇_{x_t} log p_t(x_t)
    like_score  = -(A.T @ (A @ x - y)) / sigma2 * sigma2t  # calibrate to p(y|x_t)
    drift = prior_score + like_score
    x_{t-Δt} = x_t + Δt * drift                    # use improved solvers as desired
    x_t = x_{t-Δt}
return x_0
```
