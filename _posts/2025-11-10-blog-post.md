...
$$
\mathcal{G}: \mathcal{Z}  \rightarrow \mathbb{R}^n\,\qquad \text{where} \mathcal{Z} \in \mathbb{R}^d$ and $d \ll n
$$ 
...
```python
# x = G(z);  data term ℓ(Ax,y);  latent prior Rz(z)=½||z||^2
z = init_normal()
for k in range(K):
    x = G(z)
    loss = data_fidelity(A @ x, y) + λ * 0.5 * ||z||^2
    z = z - η * ∇_z loss
return G(z)
```
