...
| :---: | :--- | :--- | :--- | :--- |
| **Smoothness Priors** | Signals are predominantly smooth or piecewise-constant. High-frequency variations are noise. | • Tikhonov: $\|\mathbf{L}\mathbf{x}\|_2^2$ <br> • Total Variation (TV): $\|\nabla \mathbf{x}\|_1$ | • Simple, convex, and computationally efficient. <br> • Effective for signals that are inherently smooth or blocky. | • **Tikhonov**: Oversmooths, blurring sharp edges. <br> • **TV**: Can create "staircasing" artifacts. <br> • Both are too simple for complex textures. |
| **Sparsity Priors** | Signals have a sparse representation in a suitable transform domain (e.g., Wavelets, DCT). | • L1-norm of transform coefficients: $\|\mathbf{\Psi}\mathbf{x}\|_1$ <br> • Foundational to Compressed Sensing. | • Strong theoretical guarantees for recovery under certain conditions. <br> • Excellent for signals with a known sparsifying basis. | • Relies on finding a suitable transform $\mathbf{\Psi}$. <br> • Not effective for signals without a sparse representation. <br> • Can introduce bias in coefficient amplitudes. |
| **Self-Similarity & Low-Rank Priors** | Natural signals contain repetitive patterns. Grouped similar patches form a low-rank matrix. | • **Methods**: NL-Means, BM3D <br> • **Regularizer**: Nuclear Norm of patch matrix, $\|\mathbf{P}_{\mathbf{x}}\|_*$ | • Captures complex non-local structures and textures effectively. <br> • Achieved state-of-the-art results for many years. | • Computationally very expensive due to patch searching and grouping. <br> • Can introduce patch-based artifacts. <br> • Difficult to formulate as a simple, explicit regularizer. |
| **Learning-Based Implicit Priors** | The manifold of natural signals can be implicitly defined by the action of a powerful denoiser. | • **Plug-and-Play (PnP)**: Replaces proximal operator with a denoiser $\mathcal{D}(\cdot)$. <br> • **Regularization by Denoising (RED)**: Defines prior gradient as $\nabla R(\mathbf{x}) = \mathbf{x} - \mathcal{D}(\mathbf{x})$. | • Highly flexible and modular; leverages SOTA denoisers (incl. deep nets). <br> • Excellent empirical performance without needing to train a full generative model. | • A "black box" prior. <br> • Convergence guarantees can be weak, especially for PnP with complex denoisers. <br> • Performance is entirely dependent on the quality and domain of the pre-trained denoiser. |  
...
This completes one **outer iteration**. Starting from an initial sample $$\mathbf{x}_{t_K} \sim \mathcal{N}(\mathbf{0}, I)$$, we repeat Steps 1–3 for $$k = K, K-1, \dots, 1$$. The final clean estimate
...
<h2 id="section6.3" style="color: #1E40AF; font-size: 24px; font-weight: bold; text-decoration: underline;">6.3 Why Decoupling Prior Trajectories and Local MAP Updates Helps</h2>

The discussion above raises a natural question: if posterior-guided sampling already targets the correct posterior distribution in principle, why do we need a separate local-MAP view that repeatedly goes back to $x_0$? The key point is that **local-MAP methods change *how* we approximate the posterior**, not just *which* posterior we target. This brings several concrete advantages that are tightly linked to operating in the clean space and to decoupling the prior trajectory from the optimization steps.

---

<strong>(a) Faithful use of the likelihood in the clean space</strong>  

Posterior-guided sampling updates the noisy state $\mathbf{x}_t$ using an approximate likelihood score

$$
\nabla_{\mathbf{x}_t} \log p(\mathbf{y}\mid \mathbf{x}_t)
\approx
\left(\frac{\partial \hat{\mathbf{x}}_0}{\partial \mathbf{x}_t}\right)^\top
\nabla_{\hat{\mathbf{x}}_0} \log p(\mathbf{y}\mid \hat{\mathbf{x}}_0),
$$

where $\hat{\mathbf{x}}_0 = \hat{\mathbf{x}}_0(\mathbf{x}_t, t)$ is the Tweedie-style denoised estimate. In practice this Jacobian is usually dropped, both to avoid Jacobian–vector products and because computing it reliably is difficult. The resulting update is therefore only a **heuristic approximation** to the true posterior score.

In contrast, local-MAP methods formulate at each time $t$ a clean-space objective of the form
...
\arg\min_{\mathbf{x}_0}
\left[
\frac{1}{2\sigma^2}\|\mathbf{y} - \mathbf{A}\mathbf{x}_0\|_2^2
+ \frac{1}{2\gamma_t^2} \|\mathbf{x}_0 - \hat{\mathbf{x}}_t\|_2^2
\right],
$$

where $\hat{\mathbf{x}}_t$ is the current prior prediction (e.g., Tweedie denoising of $\mathbf{x}_t$). 

Here the **likelihood term is evaluated exactly in the clean space**:

$$
\nabla_{\mathbf{x}_0} \log p(\mathbf{y}\mid \mathbf{x}_0)
= -\frac{1}{\sigma^2} \mathbf{A}^\top (\mathbf{A}\mathbf{x}_0 - \mathbf{y}),
$$

without any approximation through $\mathbf{x}_t$ or through the Jacobian of the denoiser. All the approximation is pushed into the **local quadratic surrogate for the prior** (the second term), while the likelihood remains the original one. This makes the method a much more direct proxy for the MAP objective in Section 1.2, and clarifies what is being approximated: we approximate the prior locally, not the likelihood.

Once $\mathbf{x}_0^{(t)}$ is updated by solving this local problem, we simply re-noise it to obtain $\mathbf{x}_{t-1}$ using the known forward diffusion kernel. In this sense, **data-consistency is always enforced in the space where the forward operator $\mathbf{A}$ is defined**, which is exactly the space in which the original inverse problem lives.

---

<strong>(b) Manifold-aware updates without explicit Jacobians</strong>  

From a geometric perspective, the denoiser (or score network) defines a data manifold in the clean space. For a given noisy state $\mathbf{x}_t$, the corresponding denoised estimate $\hat{\mathbf{x}}_t$ can be seen as a point on (or very close to) this manifold. Posterior-guided updates that ignore the Jacobian effectively move $\mathbf{x}_t$ in directions that are not guaranteed to remain tangent to this manifold, which may lead to drift away from the learned data manifold.

In the local-MAP formulation, the quadratic prior term
$$
\frac{1}{2\gamma_t^2}\|\mathbf{x}_0 - \hat{\mathbf{x}}_t\|_2^2
$$
acts as a **trust region centered at the manifold point** $\hat{\mathbf{x}}_t$. The optimizer is free to move in directions that improve data consistency, but any move far away from $\hat{\mathbf{x}}_t$ is heavily penalized. In particular:

- When the radius $\gamma_t$ is small, the solution $\mathbf{x}_0^{(t)}$ is constrained to a **small neighborhood of the prior manifold** around $\hat{\mathbf{x}}_t$.
- Within this neighborhood, the manifold can be approximated locally by its tangent space, and the quadratic penalty implicitly restricts updates to directions that do not deviate excessively from this tangent space.
- This happens **without ever having to compute $\partial \hat{\mathbf{x}}_0 / \partial \mathbf{x}_t$ explicitly**: the geometric constraint is implemented by the local quadratic term rather than by an explicit Jacobian projection.

Thus, local-MAP methods retain the geometric intuition that “likelihood gradients should be projected onto the tangent space of the data manifold”, but they realize this projection implicitly via a proximal-like penalty in $\mathbf{x}_0$, rather than via expensive Jacobian–vector products at the noisy state $\mathbf{x}_t$.

---

<strong>(c) A clean separation of roles: prior trajectory vs. optimization</strong>  

A key conceptual advantage of local-MAP methods such as DDS is that they **separate the roles of the diffusion prior and the measurement model**:

- The **diffusion prior** is responsible for generating a sequence of anchors and uncertainty levels
  $$
  \bigl\{ (\mathbf{x}_t, \hat{\mathbf{x}}_t, \gamma_t) \bigr\}_{t=T}^{0}
  $$
  that encode “where the data manifold is” and “how confident the model is” at each noise level.
- The **local optimization module** is responsible for solving, at each time step, the clean-space problem
  $$
  \min_{\mathbf{x}_0}
  \left[
  \frac{1}{2\sigma^2}\|\mathbf{y} - \mathbf{A}\mathbf{x}_0\|_2^2
  ...
  \right],
  $$
  using classical tools such as conjugate gradients or Krylov subspace methods.

This decoupling brings several practical benefits that are specific to the local-MAP view:

1. **Reuse of classical large-scale solvers.**  
   The inner problem at each $t$ is a (typically convex) quadratic in $\mathbf{x}_0$ for linear $\mathbf{A}$. This allows us to plug in decades of work on numerical linear algebra (CG, preconditioning, multi-grid, etc.), without ever differentiating through the diffusion model with respect to $\mathbf{A}$. The diffusion prior is a black-box provider of $\hat{\mathbf{x}}_t$ and $\gamma_t$; all the heavy lifting of enforcing data consistency is done by standard optimization techniques.

2. **Independent tuning of “trajectory” and “optimization accuracy”.**  
   The diffusion step size (i.e., the schedule from $t$ to $t-1$) controls how we traverse the prior trajectory, while the number of iterations (or tolerance) of the local solver controls how accurately we enforce the data term at each step. This is fundamentally different from posterior-guided sampling, where the prior and likelihood are tightly coupled inside a single noisy update. Here we can, for example, use **very few diffusion steps but many inner iterations**, or vice versa, depending on computational budget and the difficulty of the forward operator.

3. **Deterministic, optimization-style convergence behavior.**  
   Posterior-guided sampling follows a stochastic reverse SDE/ODE and is primarily analyzed in terms of convergence in distribution. By contrast, local-MAP methods behave much more like **deterministic optimization algorithms**: each step aims to decrease a local approximation of the global MAP objective. Especially in the linear-Gaussian setting, one can interpret the sequence
   $$
   \{\mathbf{x}_0^{(t)}\}_{t=T}^{0}
   $$
   as a coarse-to-fine series of proximal updates that gradually sharpen the posterior. This makes the method conceptually closer to classical inverse-problem solvers, while still benefiting from a powerful learned prior.

In summary, the local-MAP perspective does not compete with the posterior-guided sampling view at the level of the underlying Bayesian formulation—they both approximate the same conditional distribution $p(\mathbf{x}_0\mid \mathbf{y})$. Instead, it offers a **different algorithmic factorization**: the diffusion prior defines a sequence of local quadratic surrogates around the data manifold, while clean-space optimization enforces data consistency within these surrogates. This clean separation of roles is precisely what makes local-MAP methods attractive for complex inverse problems with sophisticated forward operators.
