...
\;+\; \underbrace{\frac{\tau^{p+1}}{(p+1)!}\,\epsilon^{(p+1)}(\lambda_{t_i}+\xi_\tau)}_{\text{Lagrange remainder}}
$$

where $\epsilon_i := \epsilon_\theta(x_{t_i},\lambda_{t_i})$ and derivatives are w\.r.t. $\lambda$: $\epsilon^{(m)}_i := \partial_\lambda^m \epsilon_\theta(x_{\lambda},\lambda)\!\mid_{\lambda=\lambda_{t_i}}$

### Step 2 — Pull out the exponential weight and normalize the interval

Plug (D) into (C) and integrate term-by-term:
...
$$
\star \;=\; h^{m+1}\int_0^1 e^{\,h(1-\theta)}\,\theta^m\,d\theta.
\tag{E}
$$

### Step 3 — Meet the $\varphi$-functions

The **$\varphi$-functions** used in exponential integrators are defined by
...
(standard results in the exponential-integrator literature). ([维基百科][1], [na.math.kit.edu][2])
...
where $\widetilde{R}_{p+1}$ collects a bounded combination of $\epsilon^{(p+1)}$ on $[\lambda_{t_i},\lambda_{t_{i-1}}]$.
Multiply by $\sigma(t_{i-1})$ and put everything back into (A):
...
This is the **mother formula**: “linear exact” plus a **$\varphi$-weighted** polynomial in $\lambda$ of the network output. The only approximation lies in truncating the Taylor series (the remainder scales like $h^{p+2}$). This construction is exactly the same mechanism used by exponential integrators for semi-linear ODEs, now specialized to the diffusion PF-ODE in log-SNR coordinates. ([arXiv][3], [na.math.kit.edu][2])

> **Sanity check.** As $h\to 0$, $h\,\varphi_1(h)=e^h-1 = h + \tfrac{h^2}{2}+\cdots$, so the leading nonlinear contribution is $-\sigma_{i-1}\,h\,\epsilon_i + \mathcal{O}(h^2)$, i.e., it vanishes linearly with the step size—as it must for a consistent one-step method.
