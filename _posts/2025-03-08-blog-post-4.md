...
4. Plug into the formula \ref{eq:45} and update:
   ...
   $$
   \begin{align}
   x_{t_{i-1}} & = \frac{s(t_{i-1})}{s(t_i)}\,x_{t_i}\;-\; \sigma(t_{i-1})\,\left( h\,\varphi_1(h)\,\epsilon_{\theta}(x_{t_i}, t_i) + h^2\,\varphi_2(h)\,\epsilon'_{\theta}(x_{t_i}, t_i) \right) \\[10pt]
...
2. Stage-1 state at $s_{2i-1}$ (first-order exponential Euler). Propagate from $t_{i-1}$ to $s_{2i-1}$ using a 1st-order exponential update driven by $\epsilon_\theta(\tilde x_{t_{i-1}},t_{i-1})$:
   ...
   This is the standard closed-form “linear-part exact, nonlinear part frozen” advance to the first internal point.
...
3. Residual $D_{2i-1}$ at the first stage. Measure how much the model output changes when moving to $u_{2i-1}$ at $s_{2i-1}$:
   ...
   \epsilon_\theta(u_{2i-1},s_{2i-1}) - \epsilon_\theta(\tilde x_{t_{i-1}},t_{i-1}).
   $$
...
4. Stage-2 state at $s_{2i}$ (second-order with a correction). Now propagate to the second internal point $s_{2i}$ using a 2nd-order formula that reuses $D_{2i-1}$ as a cheap estimate of derivatives:
   ...
   $$
   \begin{aligned}
   u_{2i} \leftarrow\;&\frac{\alpha_{s_{2i}}}{\alpha_{t_{i-1}}}\,\tilde x_{t_{i-1}}\;-\;\sigma_{s_{2i}}\Big(e^{r_2 h_i}-1\Big)\,\epsilon_\theta(\tilde x_{t_{i-1}},t_{i-1})\\
   &\quad-\;
...
5. Residual $D_{2i}$ at the second stage. Evaluate the model again at $u_{2i}$ and take a residual w\.r.t. the base point:
   ...
   \epsilon_\theta(u_{2i},s_{2i})-\epsilon_\theta(\tilde x_{t_{i-1}},t_{i-1}).
   $$
...
6. Final third-order update to $t_i$. Assemble the one-step third-order solution using the base value and the second residual:
   ...
   Using $e^{h}-1=h\varphi_1(h)$ and $\frac{e^{h}-1}{h}-1=h\varphi_2(h)$, this line is exactly the $\phi$-function “skeleton”

   $$
   h\varphi_1(h)\,\epsilon_i \;+\; h^2\varphi_2(h)\,\epsilon'_i \;+\; h^3\varphi_3(h)\,\epsilon''_i
   $$

   with $\epsilon'_i,\epsilon''_i$ implicitly encoded via $\mathbf{D}_{2i}$.
