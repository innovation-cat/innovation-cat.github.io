...
& = \frac{s(t_{i-1})}{s(t_i)}\,x_{t_i}
\;-\; \sigma(t_{i-1})\,(e^h-1)\,\epsilon_{\theta}(x_{t_i}, t_i)\label{eq:49}
\end{align}
$$
...
we only need to consider the first term ($k=2$). According to taylor expansion $$e^h-1-h=h^2/2+\mathcal{O}(h^3)$$

$$
\begin{align}
\mathbf{LTE}_{1} & = x_{t_{i-1}} - \tilde x_{t_{i-1}} \approx \sigma(t_{i-1})h^2\varphi_2(h)\epsilon^{'}_{\theta}(x_{t_i}, t_i) \\[10pt]
& = \sigma(t_{i-1})h^2\epsilon^{'}_{\theta}(x_{t_i}, t_i)\frac{e^h-1-h}{h^2} \\[10pt]
& \approx \sigma(t_{i-1})h^2\epsilon^{'}_{\theta}(x_{t_i}, t_i)\frac{1}{2}  \\[10pt]
& = \mathcal{O}(h^2)
\end{align}
$$
...
$$
\begin{align}
\mathbf{LTE}_{2} & = x_{t_{i-1}} - \tilde x_{t_{i-1}} = \sigma(t_{i-1})\sum_{k=2}^{\infty}\left( h^k\varphi_k(h)\epsilon^{(k-1)}_{\theta}(x_{t_i}, t_i) \right) \\[10pt]
& = \sigma(t_{i-1})h^2\varphi_2(h)\left( \frac{\epsilon_{\theta}(x_{t_s}, t_s)- \epsilon_{\theta}(\tilde x_{t_s}, t_s)}{rh} \right) + \sigma(t_{i-1})\sum_{k=3}^{\infty}\left( h^k\varphi_k(h)\epsilon^{(k-1)}_{\theta}(x_{t_i}, t_i) \right) \\[10pt]
& = \sigma(t_{i-1})h^2\varphi_2(h)\left( \frac{\mathcal{O}\|\tilde x_{t_s} - x_{t_s}\|}{rh} \right) + \sigma(t_{i-1})\sum_{k=3}^{\infty}\left( h^k\varphi_k(h)\epsilon^{(k-1)}_{\theta}(x_{t_i}, t_i) \right) \\[10pt]
& = \sigma(t_{i-1})h^2\varphi_2(h)\left( \frac{\mathcal{O}(h^2)}{rh} \right) + \sigma(t_{i-1})\sum_{k=3}^{\infty}\left( h^k\varphi_k(h)\epsilon^{(k-1)}_{\theta}(x_{t_i}, t_i) \right) \\[10pt]
& = \mathcal{O}(h^3)
\end{align}
$$
