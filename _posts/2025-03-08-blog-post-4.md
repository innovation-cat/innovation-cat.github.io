...
\;+\; \underbrace{\frac{\tau^{p+1}}{(p+1)!}\,\epsilon^{(p+1)}(\lambda_{t_i}+\xi_\tau)}_{\text{Lagrange remainder}} \right)d\tau \\[10pt]
& = \sum_{m=0}^{p} \int_{0}^{h} e^{\,h-\tau}\,\frac{\tau^m}{m!}\,\epsilon^{(m)}_id\tau + \int_{0}^{h} e^{\,h-\tau}\,\frac{\tau^{p+1}}{(p+1)!}\,\epsilon^{(p+1)}(\lambda_{t_i}+\xi_\tau)d\tau \\[10pt]
& \approx  \sum_{m=0}^{p} h^{m+1}\int_0^1 e^{\,h(1-\theta)}\,\frac{\theta^m}{m!}\,d\theta\, \epsilon^{(m)}_i \\[10pt]
