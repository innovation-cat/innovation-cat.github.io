...
Same as in §4.2, higher orders come from approximating $x'_{0,i},x''_{0,i}$ with a few extra evaluations.
...
3. Approximating First-Order derivatives $$x'_{\theta}(x_{t_i}, t_i)$$ using Finite Differences 
...
4. Plug into the formula \ref{eq:93} and update:

   $$
   \begin{align}
   \tilde x_{t_{i-1}} & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\; + \; s(t_{i-1})\Big[h\hat\varphi_1(h)\,x_{i}+ h^2\,\hat\varphi_2(h)\,x'_{i}\Big] \\[10pt]
   & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;-\; s(t_{i-1})\,(e^{(-h)}-1)\,\left(\epsilon_{i} + \frac{\epsilon_{s}\,-\,\epsilon_{i}}{2r} \right)
   \end{align}
   $$
