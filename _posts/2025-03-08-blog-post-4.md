...
where $$\widetilde{R}_{p+1}$$ collects a bounded combination of $$\epsilon^{(p+1)}$$ on $$[\lambda_{t_i},\lambda_{t_{i-1}}]$$. Multiply by $\sigma(t_{i-1})$ and put everything back into (A):
