...
$$
\int_{0}^{h} e^{\,h-\tau}\,
\epsilon_\theta\big(\lambda_{t_i}+\tau\big)\,d\tau=
\int_0^h e^{\,h-\tau}\,\sum_{m=0}^{p}\frac{\tau^m}{m!}\,\epsilon^{(m)}_i\,d\tau + \mathcal{O}(h^{p+1})
\approx \sum_{m=0}^{p} \frac{\tau^m}{m!}\,\epsilon^{(m)}_i \underbrace{\int_0^h e^{\,h-\tau}\,\tau^md\tau}_{\star}
$$
