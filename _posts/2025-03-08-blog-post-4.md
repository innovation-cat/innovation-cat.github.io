...
$$
\begin{align}
C_1 & = \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} f(s) \, \mathrm{d}s \right) = \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} \frac{s'(s)}{s(s)} \, \mathrm{d}s \right) = \frac{s(t_{i+1})}{s(t_i)} \\[10pt]
C_2(t) & = - \exp\left( \displaystyle \int_{0}^{t_{i+1}} f(s) \, \mathrm{d}s \right)
\displaystyle \int_{t_i}^{t_{i+1}} \frac{g^2(t)}{2} s_{\theta}(x_t, t)
\exp\left( -\displaystyle \int_{0}^{t} f(s) \, \mathrm{d}s \right) \, \mathrm{d}t
\end{align}
$$

The coefficient of the linear term is a constant, which means we can solve it exactly without approximation. The second part is a nonlinear term involving the model's output. To further simplify this nonlinear term, we introduce log-SNR $\lambda(t)$:

$$
\lambda(t) = \log \frac{s(t)}{\sigma(t)} \quad \Longrightarrow \quad  \lambda'(t) = \frac{\mathrm{d}\log s(t)}{\mathrm{d}t} - \frac{\mathrm{d}\log \sigma(t)}{\mathrm{d}t}
$$
