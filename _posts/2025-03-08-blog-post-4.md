...
2. Predict with order-1 to get a provisional end state $x_{t_s}$.
   
   $$
   x_{t_s}=\frac{s(t_{s})}{s(t_i)}x_{t_i} - \sigma(t_{i-1})\,(e^h-1)\,\epsilon_{\theta}(x_{t_i}, t_i)
   $$
   
3. Approximating First-Order and Second-Order Derivatives $\epsilon'_{\theta}(x_{t_i}, t_i)$  Using Finite Differences 
   
   $$
   \epsilon'_{\theta}(x_{t_i}, t_i) \approx \frac{\epsilon_{\theta}(x_{t_s}, t_s)\,-\,\epsilon_{\theta}(x_{t_i}, t_i)}{\lambda_{t_s} - \lambda_{t_i}}
   $$
   
4. Plug into the formula \ref{eq:45} and update:

   $$
   x_{t_{i-1}} = \frac{s(t_{i-1})}{s(t_i)}\,x_{t_i}\;-\; \sigma(t_{i-1})\,h\,\varphi_1(h)\,\epsilon_{\theta}(x_{t_i}, t_i) - \sigma(t_{i-1})\,h\,\varphi_2(h)\,\epsilon'_{\theta}(x_{t_i}, t_i) 
   $$
