...
& = \sigma(t_{i-1})h^2\varphi_2(h)\epsilon^{'}_{\theta}(x_{t_i}, t_i)+\sigma(t_{i-1})h^2\varphi_2(h)\epsilon^{''}_{\theta}(x_{t_i}, t_i) - \;\sigma_{t_i}\;\frac{1}{r_2}\left(\frac{e^{h_i}-1}{h_i}-1\right)\,\mathbf{D}_{2i} 
\end{align}
$$
