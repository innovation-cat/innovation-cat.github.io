...
2. Predict with **Order-1 DPM-Solver** to get a provisional end state $x_{t_s}$  corresponding to $t_s$.
   ...
   $$
   {\bar x_{t_s}}=\frac{s(t_{s})}{s(t_i)}x_{t_i} - \sigma(t_{s})\,(e^{rh}-1)\,\epsilon_{\theta}(x_{t_i}, t_i)
   $$
   
   We use ${\bar x_{t_s}}$ to represent an approximation of the true value $x_{t_s}$.

3. Approximating First-Order derivatives $$\epsilon'_{\theta}(x_{t_i}, t_i)$$ using Finite Differences 
   
   $$
   \epsilon'_{\theta}(x_{t_i}, t_i) \approx \frac{\epsilon_{\theta}({\bar x_{t_s}}, t_s)\,-\,\epsilon_{\theta}(x_{t_i}, t_i)}{\lambda_{t_s} - \lambda_{t_i}} = \frac{\epsilon_{\theta}({\bar x_{t_s}}, t_s)\,-\,\epsilon_{\theta}(x_{t_i}, t_i)}{rh}
   $$
