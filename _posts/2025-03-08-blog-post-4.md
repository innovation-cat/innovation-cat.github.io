...
DPM-Solver [^dpm_solver] is a family of dedicated, high-order ODE solvers tailored specifically for the PF-ODE. Instead of feeding the entire right-hand side into a black-box solver, it (i) analytically solves the linear part to obtain its exact solution; (ii) transforms the integration variable from the time t space to the log-SNR space, thereby reducing the nonlinear part to an exponentially weighted integral of the neural network output. Subsequently, approximating this special integral with low-degree polynomials generates first-, second-, or third-order updates, offering provable convergence in the low-step regime.

### <a id="section4.2.1">4.2.1 Setup: from PF-ODE to a Exponentially Weighted Integral integral</a>

Recall our discussion in [previous post](https://innovation-cat.github.io/posts/2024/11/diffusion-model-1/), for any discrete noising formula, there exists a corresponding PF-ODE

$$
x_t=s(t)x_0+\sigma(t)\varepsilon\,\,\Longrightarrow\,\,\frac{dx_t}{dt}=f(t)x_t-\frac{g^2(t)}{2}s_{\theta}(x_t, t)
$$

Where

$$
f(t)=\frac{s'(t)}{s(t)}\quad , \quad g^2(t)=2\sigma(t)\sigma'(t)-2 \frac{s'(t)}{s(t)} \sigma^2(t)
$$

Solving PF-ODE using **variation of constants**, we get the exact one-step map over $\[t_i, t_{i+1}\]$
...
$$
\begin{align}
C_1 & = \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} f(s) \, \mathrm{d}s \right) = \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} \frac{s'(s)}{s(s)} \, \mathrm{d}s \right) = \frac{s(t_{i+1})}{s(t_i)}\label{eq:23} \\[10pt]
...
The coefficient of the linear term ($C_1$) is easy to calculate, which means we can solve it exactly without approximation. The second part is a nonlinear term involving the model's output. To further simplify this nonlinear term, we introduce log-SNR $\lambda(t)$:
...
Now, we combine equation \ref{eq:23} and equation \ref{eq:32}, given an initial value x_{t_i} at time $t=t_i$, the
solution x_{t_{i-1}} at time $t=t_{i-1}$ of diffusion ODEs in is 

$$
x_{t_{i-1}} = \frac{s(t_{i+1})}{s(t_i)} x_t + s(t_{i+1}) \int_{\lambda_{t_i}}^{\lambda_{t_{i+1}}} e^{-\lambda}  \epsilon_{\theta}(x_{\lambda}, \lambda) \mathrm{d}\lambda
$$
