...
Plug into the formula \ref{eq:93} and update:
   ...
   & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;-\; s(t_{i-1})\,(e^{-h}-1)\,\left(x_{i} + \frac{x_{s}\,-\, x_{i}}{2r} \right)
   \end{align}
   $$
...
Plug into the formula \ref{eq:93} and update:
   ...
   \begin{align}
   \tilde x_{t_{i-1}} & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\; + \; s(t_{i-1})\Big[h\hat\varphi_1(h)\,x_{i}+ h^2\,\hat\varphi_2(h)\,x'_{i}\Big] \\[10pt]
   & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;-\; s(t_{i-1})\,(e^{-h_i}-1)\,\left(x_{i} + \frac{x_{i}\,-\, x_{i+1}}{2(h_{i+1}/h_i)} \right)
   \end{align}
   $$  
