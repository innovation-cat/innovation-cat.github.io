...
4. Plug into the formula \ref{eq:45} and update:
   ...
   & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;-\; \sigma(t_{i-1})\,(e^h-1)\,\left(\epsilon_{i} + \frac{\epsilon_{s}\,-\,\epsilon_{i}}{2r} \right)
   \end{align}
   $$
...
and combine the estimated function values at $t_s$ ($\tilde x_{t_s}$) and $x_{t_i}$ to approximate the derivative. 
