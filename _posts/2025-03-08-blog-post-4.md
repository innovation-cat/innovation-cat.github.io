...
$$
x_{t_{i+1}} = e^{\int_{t_i}^{t_{i+1}}f(s)ds} x_{t_i} - e^{\int_{0}^{t_{i+1}}f(s)ds} \int_{t_i}^{t_{i+1}}  \frac{g^2(t)}{2}s_{\theta}(x_t, t)e^{-\int_{0}^{t}f(s)ds}dt
$$

We first simplify the exponential integral.

$$
e^{\int_{t_i}^{t_{i+1}}f(s)ds} = e^{\int_{t_i}^{t_{i+1}}\frac{s'(t)}{s(t)}ds} = e^{\int_{t_i}^{t_{i+1}}(\log{s(t)})'ds}=\frac{s(t_{i+1})}{s(t_i)}
$$
