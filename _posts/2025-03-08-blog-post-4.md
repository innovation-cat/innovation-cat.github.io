...
We first simplify the exponential integral.

$$
\exp\left( \displaystyle \int_{t_i}^{t_{i+1}} f(s) \, \mathrm{d}s \right)
= \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} \frac{s'(s)}{s(s)} \, \mathrm{d}s \right)
= \exp\left( \displaystyle \int_{t_i}^{t_{i+1}} \bigl( \log s(s) \bigr)' \, \mathrm{d}s \right)
= \frac{s(t_{i+1})}{s(t_i)}
$$
