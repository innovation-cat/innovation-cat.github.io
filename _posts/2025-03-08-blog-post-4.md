...
& = \sum_{k=2}^{3}h^k\varphi_k(h)\epsilon^{(k)}_{\theta}(x_{t_i}, t_i) - \;\frac{1}{r_2}\left(\frac{e^{h}-1}{h}-1\right)\,\mathbf{D}_{2} 
\end{align}
$$
...
& = \underbrace{\left(\frac{h^3}{6}+\mathcal{O}(h^4)}_{h^3\varphi_3(h)} - \underbrace{\Big(\dfrac{h}{2}+\dfrac{h^2}{6}+\dfrac{h^3}{24}+O(h^4)\Big)}_{h\varphi_2(h)}\dfrac{r_2h^2}{2} \right) \epsilon_{i}^{''} + \mathcal{O}(h^4) \\[10pt]
& = \left( \frac{1}{6} - \frac{r_2}{4}\right)h^4\epsilon_{i}^{''} + \mathcal{O}(h^4)
\end{align}
$$
