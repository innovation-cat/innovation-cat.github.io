...
& \mathbb E(x_t^2|x_0) = \mathbb E(x_0^2|x_0) + \int_0^t(2*f(s)*\mathbb E (x_s^2|x_0)+g^2(s))ds + \int_0^tg(s)*\mathbb E(dw_s|x_0)\label{eq:15} \\ \Longrightarrow \ \ \ \ 
& \mathbb E(x_t^2|x_0) = \mathbb E(x_0^2|x_0) + \int_0^t(2*f(s)*\mathbb E (x_s^2|x_0)+g^2(s))ds\label{eq:16} 
\end{align}
$$
...
$$
f(t)=\frac{s'(t)}{s{t}}, \ \ \ \ \ \ g^2(t)=2*(\sigma(t)\sigma'(t)-\frac{s'(t)}{s{t}}*\sigma^2(t))
$$

Similarly, given SDE expression, we can also derive the corresponding discrete forward processing, which satisfies:

$$
s(t)=\phi(t)=e^{\int_0^tf(s)ds}, \ \ \ \ \ \ \sigma^2(t)=\phi^2(t)*\int_0^t \frac{g^2(s)}{\phi^2(s)}ds
$$
