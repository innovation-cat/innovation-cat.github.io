...
$$
\begin{align}
& var'(x_t)=m_2'(x_t)-2m(x_t)m'(x_t)\label{eq:18} \\[10pt] \Longrightarrow \ \ \ \ 
& var'(x_t)=2f(t)m_2(x_t)+g^2(t)-2f(t)m^2(x_t)\label{eq:19} \\[10pt] \Longrightarrow \ \ \ \ 
& var'(x_t)=2f(t)(m_2(x_t) - m^2(x_t))+g^2(t)\label{eq:20} \\[10pt] \Longrightarrow \ \ \ \ 
&  var'(x_t)=2f(t)var(x_t)+g^2(t)\label{eq:21} 
\end{align}
$$

Equation \ref{eq:21} is the first-order linear equations, by multiplying the integrating factor $e^{-2\int_0^tf(s)ds}$, let $\phi(t)=e^{\int_0^tf(s)ds}$, we can get the solution.
...
$$
dx_t=\big( f(t)x_t-g^2(t)\nabla_{x_t} \log p(x_t) \big)dt + g(t)d\bar w_t\label{eq:26}
$$

Here, $\bar w_t\$, like $w_t\$, is a standard Wiener process, except that the direction of $w_t\$ is reversed from T to 0 (or from 1 to 0 from a continuous perspective). $\nabla_{x_t} \log p(x_t)$ is known as score, score refers to the gradient (with respect to $x_t$) of the log-probability density at any time step $t$. 
...
$$
dx_t=\big( f(t)x_t-\frac{1}{2}g^2(t)\nabla_{x_t} \log p(x_t) \big)dt\label{eq:27}
$$
