...
**Stochastic Interpolants (SI)** generalize this idea by adopting the same path-first philosophy but extending it to include stochastic noise
...
The core idea of **Stochastic Interpolants (SI)** is to **start from a path of random variables** that connects two endpoint distributions, rather than starting from a forward diffusion process or a likelihood objective.
...
$$
\rho_t(x) = \mathbb P(X_t = x), \qquad t \in [0,1],
$$
