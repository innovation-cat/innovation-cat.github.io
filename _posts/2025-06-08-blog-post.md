---
title: 'High-Order PF-ODE Solver in Diffusion Models'
date: 2025-06-08
...
permalink: /posts/2025/06/ODE-Solver/
tags:
  - ODE
  - Diffusion Model
  - Numerical Analysis
  ...
  <div class="toc-content">
    <ul class="toc-list">
      ...
      <li class="toc-item-main">
        <a href="#sectionPart1" class="toc-link-main">1. Foundation and Preliminary</a>
      </li>

      <li class="toc-item-main">
        <a href="#sectionPart2" class="toc-link-main">2. Claaisic Numerical ODE Solver</a>   
        <ul class="toc-sublist">
          <li><a href="#section2.1">2.1 One-step vs Multi-step</a>
            <ul class="toc-sublist">
              <li><a href="#section2.1.1">2.1.1 One-step Methods</a></li>
              <li><a href="#section2.1.2">2.1.2 Multi-step Methods</a></li>
              <li><a href="#section5.3">5.3 Output Granularity: From Scalar to Full Gate</a></li>
            </ul>
          </li>
          <li><a href="#section2.2">2.2 2.2 Implicit vs Explicit</a>
            <ul class="toc-sublist">
              <li><a href="#section2.2.1">2.2.1 Explicit Methods</a></li>
              <li><a href="#section2.2.2">2.2.2 Implicit Methods</a></li>
            </ul>
          </li>
          <li><a href="#section2.3">2.3 Local Truncation Error and Global Truncation Error</a>
            <ul class="toc-sublist">
              ...
              <li><a href="#section2.3.2">2.3.2 Example with Heun Method</a></li>
            </ul>
          </li>
      ...
      <li class="toc-item-main">
        <a href="#sectionPart3" class="toc-link-main">3. Pseudo-Numerical Methods for Diffusion Models</a> 
        <ul class="toc-sublist">
          <li><a href="#section3.1">3.1 PLMS: Pseudo Linear Multistep</a></li>
          <li><a href="#section3.2">3.2 PRK: Pseudo Runge–Kutta</a></li>
          <li><a href="#section3.3">3.3 Putting them together: the PNDM scheduler</a></li>
        </ul>      
      </li>

      <li class="toc-item-main">
        <a href="#sectionPart4" class="toc-link-main">4. High-Order and Semi-Analytic Solvers</a>   
        <ul class="toc-sublist">
          <li><a href="#section4.1">4.1 DEIS</a></li>
          <li><a href="#section4.2">4.2 DPM-Solvers</a>
            <ul class="toc-sublist">
              <li><a href="#section4.2.1">4.2.1 From PF-ODE to a Exponentially Weighted Integral integral</a></li>
              <li><a href="#section4.2.2">4.2.2 From the Integral to High-Order Updates</a></li>
              <li><a href="#section4.2.3">4.2.3 Single-step DPM-Solver</a></li>
              <li><a href="#section4.2.4">4.2.4 LTE and GTE Analysis</a></li>
            </ul>
          </li>
          <li><a href="#section4.3">4.3 DPM-Solvers++</a>
            <ul class="toc-sublist">
              <li><a href="#section4.3.1">4.3.1 Setup in Data-Prediction View</a></li>
              <li><a href="#section4.3.2">4.3.2 Single-step DPM-Solver++</a></li>
...
Diffusion sampling can be viewed as integrating the probability flow ODE (PF-ODE), yet treating it as a generic initial-value problem often yields a suboptimal speed–quality trade-off—especially in low-NFE regimes where the PF-ODE is semi-linear and can behave stiffly. This post revisits core numerical-analysis notions (one-step vs. multi-step, explicit vs. implicit, and the roles of local/global truncation error) to explain why off-the-shelf integrators may underperform when directly applied to diffusion sampling. Building on the structure of PF-ODEs, we then survey diffusion-tailored solvers in two families: pseudo-numerical methods that raise the order on model outputs while keeping a schedule-aware one-step map (PLMS/PNDM), and semi-analytic/high-order approaches that integrate the linear part exactly and approximate the nonlinear term with structure-aware quadrature (DEIS, DPM-Solver/++ and UniPC). The goal is a practical, unified understanding of when these samplers work—and why “just use RK4” is rarely the best answer.


---

<h1 id="section1" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">1. Foundation and Preliminary</h1>
...
<h1 id="section2" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">2. Numerical ODE Solver</h1>
...
<h1 id="section2.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.1 One-step vs Multi-step</h1>
...
<h1 id="section2.1.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.1.1 One-step Methods</h1>
...
<div style="display: block; margin: 0 auto; text-align: center;">
  <img src="/assets/images/2025-06-08-blog-post/1.png" alt="outline" style="display: block; margin: 0 auto;" width="900" height="700">
  <div style="margin-top: 8px; font-size: 16px; color: #666; font-style: Times New Roman;">
     Figure 1: Approximation of Area by One-step ODE Solvers with Different Orders
  </div>
</div>

<br>

However, many parameters of high-order methods (such as step size and constants) usually need to be set manually. In addition, high-order methods involve complex calculation processes and have poorer stability than low-order methods. Therefore, in practical applications, it is necessary to balance multiple factors such as efficiency and accuracy to select a suitable solver.


---

<h1 id="section2.1.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.1.2  Multi-step Methods</h1>
...
<div style="display: block; margin: 0 auto; text-align: center;">
  <img src="/assets/images/2025-06-08-blog-post/2.png" alt="outline" style="display: block; margin: 0 auto;" width="900" height="700">
  <div style="margin-top: 8px; font-size: 16px; color: #666; font-style: Times New Roman;">
     Figure 2: Approximation of Area by Different Multi-step Methods
  </div>
</div>
...
<h1 id="section2.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.2 Implicit vs Explicit</h1>
...
<h1 id="section2.2.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.2.1  Explicit Methods</h1>
...
<h1 id="section2.2.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.2.2 Implicit Methods</h1>
...
<h1 id="section2.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.3 Local Truncation Error and Global Truncation Error</h1>
...
<h1 id="section2.3.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.3.1  Example with Euler Method</h1>
...
<strong style="color: blue;">Deriving Local Truncation Error:</strong>
...
<strong style="color: blue;">Deriving Global Truncation Error:</strong>
...
<h1 id="section2.3.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">2.3.2  Example with Heun Method</h1>
...
<h1 id="section2.4" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.4 The Limitation of Numerical ODE Solvers for diffusion Sampling</h1>
...
<h1 id="section3" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">3. Pseudo-Numerical Methods for Diffusion Models</h1>
...
<strong style="color: blue;">Keypoint: </strong> treat the linear part exactly, raise the order on the neural part. This is the unifying principle behind the PNDM family.

---

<h1 id="section3.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">3.1 PLMS: Pseudo Linear Multistep (Linear multi-step on model outputs)</h1>
...
<h1 id="section3.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">3.2 PRK: Pseudo Runge–Kutta (in-step multi-stage on model outputs)</h1>
...
<h1 id="section3.3" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">3.3 Putting them together: the PNDM scheduler</h1>
...
<div class="modern-deep-card">
  <p class="card-content">
    ...
    Pseudo-numerical samplers = *high-order on model outputs + one schedule-aware state map*. Read through section 2 with this in mind and you’ll see PLMS is the <strong>multistep (AB) track</strong>, PRK the <strong>multi-stage (RK) track</strong>, and the optional AM-style blend is the familiar **predictor–corrector** from classical ODEs—transplanted into diffusion with the integrating-factor lens.
  </p>
</div>
...
<h1 id="section4" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">4. High-Order and Semi-Analytic Solvers</h1>
...
<h1 id="section4.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">4.1 DEIS</h1>

---

---

<h1 id="section4.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">4.2 DPM-Solvers</h1>
...
<h1 id="section4.2.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.2.1 Setup: from PF-ODE to a Exponentially Weighted Integral integral</h1>
...
<div class="modern-deep-card">
  <p class="card-content">
    <span class="label-text">Takeaway:</span> 
    PF-ODE has a <strong>semi-linear</strong> structure. Solving the linear drift exactly and approximating only the model-dependent part—under a <strong>log-SNR reparameterization</strong>—is what enables high order with very few NFEs.
  </p>
</div>
...
<h1 id="section4.2.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.2.2 \(\varphi\)-functions inside: From the integral to high-order updates</h1>
...
<strong style="color: blue;">Step 1: Local Taylor model of the network output</strong>
...
<strong style="color: blue;">Step 2: Pull out the exponential weight and normalize the interval</strong>
...
<strong style="color: blue;">Step 3: Meet the $\varphi$-functions</strong>
...
<strong style="color: blue;">Step 4: Assemble the unified series</strong>
...
<div class="modern-deep-card">
  <p class="card-content">
    <span class="label-text">Takeaway:</span> 
    DPM-Solver converts PF-ODE’s structure into twp parts: Linear exact , plus <span class="math">φ</span>-weighted polynomial is the entire story. The <span class="math">φ<sub>k</sub></span> compress the 
    <span class="hl-blue">exponential weight</span> and the 
    <span class="hl-blue">Taylor polynomial</span> into stable, closed-form coefficients, enabling high order in few steps.
  </p>
</div>
...
<h1 id="section4.2.3" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.2.3 Single-step DPM-Solver (ε-prediction view)</h1>
...
<strong style="color: blue;">DPM-Solver-1 (Order-1, 1-NFE).</strong>
Constant approximation of $\epsilon$ over $[\lambda_i,\lambda_{i-1}]$:
...
<strong style="color: blue;">DPM-Solver-2 (Order-2, 2-NFEs).</strong>
One **predict–correct** to estimate $\epsilon'_i$.
...
4. Plug into the formula \ref{eq:45} and update:
   ...
   Here, we replace the second term 
   
   $$h^2\varphi_2(h)=e^h-1-h$$

   by

   $$\frac{h}{2}(e^{h}-1)$$ 
   
   which does not affect the method’s **second-order** accuracy. With this approximation, the correction becomes
   
   $$
   \begin{align}
   h^2\varphi_2(h)\epsilon'_{\theta}(x_{t_i}, t_i) & = (e^h-1-h)\cdot \frac{\epsilon_{s}\,-\,\epsilon_{i}}{rh} \\[10pt]
   & \approx \frac{e^h-1}{2}\cdot \frac{\epsilon_s-\epsilon_i}{r},
   \end{align}
   $$
   
   which is algebraically simpler and avoids dividing by (h) while remaining second-order consistent.
...
<strong style="color: blue;">DPM-Solver-3 (Order-3, 3-NFEs).</strong> Two predictions to estimate $$\epsilon'_i,\epsilon''_i$$.
...
<h1 id="section4.2.4" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.2.4 LTE and GTE Analysis</h1>
...
Now, we substitute the above results back into Equation \ref{eq:76}
...
<h1 id="section4.2.5" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.2.5 Relation to DEIS and bridge to DPM-Solver++</h1>
...
<h1 id="section4.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">4.3 DPM-Solver++</h1>
...
<h1 id="section4.3.1" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.3.1  Setup in Data-Prediction View</h1>
...
<h1 id="section4.3.2" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.3.2 Single-step DPM-Solver++</h1>
...
$$
\begin{align}
\tilde x_{t_{i-1}} & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;+\; s(t_{i})h\,\hat\varphi_1(h)\,x_{i} \\[10pt]
& = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\; - \; s(t_{i})\,\left( e^{-h} - 1\right)\,x_{i}
\end{align}
$$


**2nd-order DPM-Solver++(2-NFE).**  we use a similar technique as DPM-Solver-2 to estimate the derivative $x_{i}^{'}$. Specifically, we introduce an additional intermediate time step $t_{s}$ between $t_{i}$ and $t_{i-1}$ 

   $$
   \lambda_{t_s} = t_{\lambda}(\lambda_{t_i}+rh)
   $$
   
Predict with **Order-1 DPM-Solver++** to get the estimated function values at $t_s$ ($\tilde x_{t_s}$)
   
   $$
   {\tilde x_{t_s}}=\frac{\sigma(t_{s})}{\sigma(t_i)}\,x_{t_i} - s(t_{i})\,\left( e^{-rh} - 1\right)\,x_{i}
   $$
...
Approximating First-Order derivatives $$x'_{\theta}(x_{t_i}, t_i)$$ using Finite Differences 
   ...
   $$
   x'_{\theta}(x_{t_i}, t_i) \approx \frac{x_{\theta}({\tilde x_{t_s}}, t_s)\,-\,x_{\theta}(x_{t_i}, t_i)}{\lambda_{t_s} - \lambda_{t_i}} = \frac{x_{\theta}({\tilde x_{t_s}}, t_s)\,-\,x_{\theta}(x_{t_i}, t_i)}{rh}
   $$
   
Plug into the formula \ref{eq:93} and update:

   $$
   \begin{align}
   \tilde x_{t_{i-1}} & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\; + \; s(t_{i-1})\Big[h\hat\varphi_1(h)\,x_{i}+ h^2\,\hat\varphi_2(h)\,x'_{i}\Big] \\[10pt]
   & = \frac{\sigma(t_{i-1})}{\sigma(t_i)}\,x_{t_i}\;-\; s(t_{i-1})\,(e^{-h}-1)\,\left(x_{i} + \frac{x_{s}\,-\, x_{i}}{2r} \right)\label{eq:100}
   \end{align}
   $$
...
If we take the intermediate point at the midpoint, that is $r=1/2$, the second-order DPM-Solver simplifies to
   ...
   $$
   \tilde x_{t_{i-1}}  = \frac{s(t_{i-1})}{s(t_i)}\,x_{t_i}\;-\; s(t_{i-1})\,(e^{-h}-1)\,x_{s}
   $$
   

---

<h1 id="section4.3.3" style="color: #1D4ED8; font-size: 22px; font-weight: bold; text-decoration: underline;">4.3.3 Multistep DPM-Solver++ (2M)</h1>


At each step (from $t_i$ to $t_{i-1}$), the proposed singlestep ([$4.3.2](#section4.3.2)) solver needs two sequential function evaluations of the neural network $x_{\theta}$. Moreover, the intermediate values are only used once and then discarded. Such method loses the previous information and may be inefficient. By drawing on the multi-step numerical  ODE methods ([$2.1.2](#section2.1.2)), historical information can be saved and reused. For instance, when solving 
$x_{t_{i-1}}$ with the 2M method, the information of $x_{t_i}$ and $x_{t_{i+1}}$ are utilized.

Approximating First-Order derivatives $$x'_{\theta}(x_{t_i}, t_i)$$ using Finite Differences 
   
   $$
   x'_{\theta}(x_{t_i}, t_i) \approx \frac{x_{\theta}({\tilde x_{t_i}}, t_i)\,-\,x_{\theta}(x_{t_{i+1}}, t_{i+1})}{\lambda_{t_{i}} - \lambda_{t_{i+1}}} = \frac{x_{\theta}({\tilde x_{t_i}}, t_i)\,-\,x_{\theta}(x_{t_{i+1}}, t_{i+1})}{h_{i+1}}
   $$
...
<h1 id="section4.4" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">4.4 UniPC</h1>



---

<h1 id="section5" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">5. References</h1>
