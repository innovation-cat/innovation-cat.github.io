...
**Stochastic Interpolants (SI) provides a profound generalization and unification of this "path-first" concept, establishing a meta-framework capable of accommodating both deterministic and stochastic dynamics.** The central tenets of SI are:

1.  **Flexibility in Path Definition:** SI defines the probability path connecting endpoints $(x_0, x_1)$ through a general class of interpolating functions, $(\alpha_t, \beta_t)$. The power of this framework lies in its ability to realize the evolution along this path via two distinct types of dynamical systems:
    - **Deterministic Path (ODE):** A vector field $v(x,t)$ is learned. The ODE dynamics it defines ensure, via the **transport equation**, that the evolution of the sample density aligns perfectly with the predefined probability path. This represents the core and most direct application of the SI framework.
...
The randomness $\zeta$ gives you freedom to design **noisy paths** (not only deterministic straight lines).

This construction induces a family of **marginal distributions**

$$
p_t(x) = \mathbb P(X_t = x), \qquad t \in [0,1],
$$

which we call the **interpolant path of densities**.


In practice, $\Phi$ is usually specified by a **simple analytic formula** with two pieces:
...
### (a) Two-sided interpolants

$$
X_t \;=\; I(t, X_0, X_1)\;+\;\Gamma(t)\,Z.
$$

* $I(t, X_0, X_1)$: a **deterministic curve** between the endpoints.

  * The simplest choice: a **linear path**

    $$
    I(t, X_0, X_1) = (1-t)X_0 + tX_1.
    $$
  * More complex curves are also possible (e.g. geodesics in latent space).
* $\Gamma(t)$: a **time-dependent noise scale**.

  * If $\Gamma(t) \equiv 0$, the path is **deterministic** (this is exactly **Flow Matching / Rectified Flow**).
  * If $\Gamma(t) \neq 0$ for $t \in (0,1)$, the path is **stochastic**—particles wiggle around the deterministic curve as they move from $X_0$ to $X_1$.

Thus two-sided SI subsumes both deterministic flows and noisy interpolations.

---

### (b) One-sided interpolants

Another important case (which recovers diffusion-style noising) is
...
* Here the path starts from a **Gaussian noise distribution** at $t=0$ (since $\alpha(0)=0, \sigma(0)>0$), and ends at the **data distribution** at $t=1$ (since $\alpha(1)=1, \sigma(1)=0$).
* This is exactly the forward diffusion family: the VP/VE SDEs correspond to different choices of the schedules $\alpha(t), \sigma(t)$.
* Thus **diffusion models** appear as a **special case of SI** with a one-sided noisy interpolant.
...
## <a id="section6.4">6.4 Connections contrasts with FM (RF) and SDE-Based Diffusion Model</a>




### <a id="section6.4.1">6.4.1 Stochastic Interpolants (SI) vs Flow Matching</a>
...
### <a id="section6.4.2">6.4.2 Stochastic Interpolants (SI) vs Score-based Diffusion Models</a>

Both Score-based Diffusion Models and Stochastic Interpolants acknowledge that the same probability path $p_t$ can be realized using either a reverse stochastic differential equation (SDE) or a probability flow ordinary differential equation (PF-ODE), with both sharing identical marginal distributions. The key distinction lies in their paradigms: Score-based Diffusion Models adopt a "process-first" approach (starting with a forward SDE and reverse SDE, then deriving the corresponding PF-ODE and training method), whereas SI employs a "path-first" paradigm (first defining an interpolant path and its density, then uniformly deriving a family of ODE/SDE implementations with two explicit regression objectives).

**Score-based Diffusion Models**: Introduces the **Process-first paradigm** in generative modeling. The central object (first step) is a *forward stochastic process* (typically an SDE or a discrete-time Markov chain) that maps data to Gaussian noise.


**SI**: Introduces the **Path-first paradigm** in generative modeling. The central object is to construct a probability path that maps any two distribution.
