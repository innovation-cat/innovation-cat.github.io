...
& = -\int \varphi(x)\,\nabla_x\!\cdot\!\big(p_t(x)u_t(x)\big)\,dx + \int \nabla_x \cdot \big(\varphi(x)p_t(x)v_t(x)\big)\,dx  \\[10pt]
...
Equate the two expressions and integrate by parts (boundary term vanishes by compact support/decay):
...
Alternatively, one can realize the same path through a stochastic differential equation
   ...
   $$
   dX_t = \Big(u_t(X_t) + \tfrac12 \beta(t)\,\nabla\log p_t(X_t)\Big)\,dt \;+\; \sqrt{\beta(t)}\,dW_t,
   $$
...
### <a id="section6.3.1">6.3.1 Fokker–Planck (transport + diffusion)</a>
