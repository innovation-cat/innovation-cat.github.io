...
Instead of a fixed linear interpolation, Flow Matching begins by defining a more general **conditional probability path**, $p_t(x \mid x_0, x_1)$, that connects any pair of points $x_0 \sim p_0$ (noise) and $x_1 \sim p_1$ (data). This path is a time-dependent probability distribution that satisfies the boundary conditions:

- at $t=0$, it concentrates on the starting point: $p_0(x \mid x_0, x_1) = \delta_{x_0}(x)$.
- at $t=1$, it concentrates on the endpoint: $p_1(x \mid x_0, x_1) = \delta_{x_1}(x)$.

Associated with this probability path is a **conditional vector field**, $v_t(x \mid x_0, x_1)$, which represents the instantaneous velocity required for a particle at position $x_t$ at time $t$ to stay on the trajectory defined by the path.

A common and intuitive choice for this path is the **Gaussian probability path**, defined as:

$$
p_t(x|x_0, x_1) = \mathcal{N}(x | s(t) x_1, \sigma(t)^2I).
$$
