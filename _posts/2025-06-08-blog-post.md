...
- **Flow Matching (FM)**
  * Introduces the **path-first paradigm** in generative modeling.
  * The focus is on **deterministic interpolants**: construct a smooth path of random variables connecting two endpoint distributions without adding noise.
  * The central object is the **velocity field** along this path.

- **Stochastic Interpolants (SI)**
  * Extends the **path-first paradigm** further.
  * Allows interpolants to be **stochastic**, not just deterministic.
  * SI is best viewed as a **general framework**, within which FM appears as the **noise-free special case**.
