...
Flow Matching begins by defining a more general **marginal probability path**, $p_t(x_t)$, that connects any pair of points $x_0 \sim p_0$ (noise) and $x_1 \sim p_1$ (data). This path is a time-dependent probability distribution that satisfies the boundary conditions:
...
Under standard well-posedness assumptions on $v_t$ (for example, globally Lipschitz in $x$, the continuity equation has a **unique distributional solution** for the given initial data. Therefore,

$$
\boxed{ \quad \mu_t \equiv p_t \quad \text{for all } t\in[0,1]. \quad }
$$

In words: **the ODE driven by $v_t$ produces *exactly* the prescribed SI path \Phi(t) of marginals**.
