...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul>
	...
	<li><a href="#section3">3. Origins of the Idea: From Rectified Flow</a>
		<ul>
			<li><a href="#section3.1">3.1 The Intuition of the Shortest Path</a></li>
			<li><a href="#section3.2">3.2 The Birth of Rectified Flow</a></li>
			<li><a href="#section3.3">3.3 Reflow: Straightening the Path Further</a></li>
		</ul>
	</li>
...
At the heart of Normalizing Flows (NFs) [^Nice] [^nvp] [^Glow] lies a simple but powerful principle: the **change-of-variables** formula. It connects the density of a data point $z$ in one space to the density of its transformed representation in another $x$.
...
The question raised at the end of the previous section — *can we directly learn the path between distributions?* — found its first concrete answer in **Rectified Flow (RF)**. Rather than simulating a noisy process step by step, RF proposed a remarkably simple intuition: what if we could connect the data distribution and the noise distribution by the **shortest, straightest path**?

---

## <a id="section3.1">3.1 The Intuition of the Shortest Path</a>
...
Thus, instead of relying on stochastic simulation, RF frames generation as **learning a deterministic vector field** that drives samples along these straight trajectories.

---

## <a id="section3.2">3.2 The Birth of Rectified Flow (RF)</a>

Lipman et al. (2022) formalized this idea by defining a **conditional probability path** \$\pi\_t(x \mid x\_0, x\_1)\$ induced by the interpolation above. The learning objective is to regress a vector field \$v\_\theta(x,t)\$ that matches the ground-truth velocity:
...
Training therefore becomes a **supervised regression problem**, with no need to simulate stochastic noise trajectories. Once trained, the generative process is defined as solving the ODE

$$
\frac{dx}{dt} = v_\theta(x,t), \quad x(0) \sim p_{\text{noise}},
$$

which transports noise samples into the data distribution.

---

## <a id="section3.3">3.3 Reflow: Straightening the Path Further</a>
