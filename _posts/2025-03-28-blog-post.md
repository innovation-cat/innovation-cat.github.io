...
**Core Idea**: These methods avoid the explicit construction of the  matrix $QK^T \in \mathbb{R}^{n\times n}$ by reformulating the attention equation. The key insight is to reorder the computation $\text{softmax}(QK^{T}/\sqrt{d})V$. By leveraging mathematical properties, such as **kernel methods** or low-rank projections. Specifically, If there exists a feature mapping (kernel function) $\phi: \mathbb{R}^d \to \mathbb{R}^r$ such that

$$
\exp\!\left(\tfrac{Q K^\top}{\sqrt{d}}\right) \approx \phi(Q) \phi(K)^\top,
$$

then the exponential interaction can be expressed as an inner product in the feature space. This is the cornerstone of linearized attention. Substituting the kernelized representation into the attention weights yields
...
$$
\text{Attn}(q_i) = 
\frac{\phi(q_i)^\top \Big(\sum_{j=1}^n \phi(k_j) v_j^\top\Big)}
{\phi(q_i)^\top \Big(\sum_{j=1}^n \phi(k_j)\Big)}.
$$


Define two key **precomputations** that depend only on keys and values:

$$
K' = \sum_{j=1}^n \phi(k_j) v_j^\top \;\;\in \mathbb{R}^{r \times d},
$$
...
The attention output for each query then reduces to

$$
\text{Attn}(q_i) = \frac{\phi(q_i)^\top K'}{\phi(q_i)^\top z}.
$$

Thus, the cost per query is $O(rd)$, and the entire sequence requires $O(nrd)$ operations—linear in sequence length.

Different instantiations of linearized attention differ primarily in the choice of feature map $\phi(\cdot)$:
