...
## <span style="color:#3498DB;">Case A: Naïve Attention (store $A$ or $S$), No Checkpointing</span>



** Activations** include $Q,K,V,O,Z,H,Y$ plus the full attention score or probability matrix:
...
** Memory**
  ...
  - Each $N \times d$ tensor ($Q,K,V,O,Y$):  262{,}144 \times 128 \times 4 \approx 0.125 GiB.
  - Each $N \times h$ tensor ($Z,H$): 0.500 GiB.
...
#### Case B: Memory-Efficient Attention (no materialized $A/S$), No Checkpointing

* Use an “online softmax” or tiling scheme (as in FlashAttention).
* Stored activations: $X,Q,K,V,O,Z,H,Y$ only.
* Memory:

  * $6$ tensors of shape $N \times d$ → 0.75 GiB.
  * $2$ tensors of shape $N \times h$ → 1.00 GiB.
* **Peak activation memory ≈ 1.75 GiB.**

---

#### Case C: Case B + Block-Level Gradient Checkpointing

* Only checkpoint the block input $X$ and output $Y$; everything else is recomputed in backward.
* Stored activations: $X,Y$ only.
* Memory:

  * $2$ tensors of shape $N \times d$ → 0.25 GiB.
* **Peak activation memory ≈ 0.25 GiB.**
* Cost: requires recomputing $Q,K,V,O,Z,H$ during backward pass.
