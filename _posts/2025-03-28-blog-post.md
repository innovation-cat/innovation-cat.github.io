...
- **Step 1: Pre-computation of the $$\text{rowsum}(A \odot {d_A})$$**. The most complex term in the standard backward pass is $${d_S} = A \odot ({d_A} - \text{rowsum}(A \odot {d_A}))$$. The FlashAttention algorithm begins with a crucial mathematical insight that simplifies the rowsum term. Let us define a vector $D \in \mathbb{R}^{N}$ where $$D_i = [\text{rowsum}(A_i \odot {d_{A_i}})]$$.

  **Theorem:** The vector $D$ can be computed as $D = \text{rowsum}(\text{dO} \odot O)$.

  **Proof:** We aim to prove that for any row $i$: $[\text{rowsum}(A \odot {d_A})]_i = [\text{rowsum}(O \odot {d_O})]_i$.
