...
- **Step 2: accumulate row scalars and $d_V$.**
  ...
  Calculate $d_A$ is simple. Load $d_{O_i}$ and $V_j$ from HBM into SRAM, for each block d_{A_{ij}}, we 
  
  $$
  d_{A_{ij}} = d_{O_i} {V_j}^T
  $$

  Once $d_{A_{ij}}$ is done, we can derive $d_{S_{ij}}$.
