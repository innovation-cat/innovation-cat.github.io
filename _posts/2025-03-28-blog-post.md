...
| Symbol / Abbreviation | Definition & Meaning |
|:---------------------:|:---------------------:|
| \(N\) | Sequence length (number of tokens) |
| \(d\) | Head dimension |
| \(d_v\) | Value dimension (often \(d_v = d\)) |
| \(Q \in \mathbb{R}^{N \times d}\) | Query matrix |
| \(K \in \mathbb{R}^{N \times d}\) | Key matrix |
| \(V \in \mathbb{R}^{N \times d_v}\) | Value matrix |
| \(S = QK^\top/\sqrt d\) | Attention score matrix |
| \(P = \mathrm{softmax}(S)\) | Attention probability matrix |
| \(O = PV\) | Attention output |
| \(B_r\) | Row tile size (blocking size for \(Q\)) |
| \(B_c\) | Column tile size (blocking size for \(K,V\)) |
| \(i\) | Row index (query index or row in a row tile) |
| \(j\) | Column tile index (key/value tile) |
| \(T_c = \lceil N / B_c \rceil\) | Number of column tiles |
| \(\tilde m_i^{(j)}\) | Row maximum inside tile \(j\) |
| \(\tilde \ell_i^{(j)}\) | Row sum of exponentials inside tile \(j\) |
| \(\tilde u_i^{(j)}\) | Tile-local unnormalized weighted sum of values |
| \(m_i^{(j)}\) | Global row maximum after tile \(j\) |
| \(\ell_i^{(j)}\) | Global row exp-sum after tile \(j\) |
| \(\tilde O_i^{(j)}\) | Global unnormalized weighted sum after tile \(j\) |
| \(O_i = \tilde O_i^{(T_c)}/\ell_i^{(T_c)}\) | Final normalized row output |
| \(dO\) | Gradient of output \(O\) |
| \(dQ, dK, dV\) | Gradients of \(Q, K, V\) |
| \(dS\) | Gradient of score matrix \(S\) |
| RNG mask | Dropout random mask (reconstructed deterministically) |
| HBM | **High Bandwidth Memory** — off-chip GPU memory, large capacity (GBs), high bandwidth but higher latency |
| SRAM | **Static Random Access Memory** — on-chip memory, very fast, very small (KB–MB) |
| SMEM | **Shared Memory** — CUDA’s on-chip shared memory (a form of SRAM) |
| Registers | Fastest on-chip storage inside GPU cores, very limited capacity |
| Occupancy | Ratio of active threads to maximum supported threads (GPU utilization) |
| Warp | Basic execution unit on NVIDIA GPUs (32 threads in lockstep) |
| Tensor Core | Specialized GPU unit for high-throughput matrix multiplications (FP16/BF16/FP8) |
| TMA | **Tensor Memory Accelerator** — Hopper feature for efficient HBM↔SMEM transfers |
| WGMMA | **Warp Group Matrix Multiply-Accumulate** — Hopper instruction for warp-group GEMMs |
| FP32 | 32-bit floating point precision |
| FP16 | 16-bit half precision floating point |
| BF16 | 16-bit bfloat16 (FP32 exponent range, lower mantissa precision) |
| FP8 | 8-bit floating point (Hopper), extreme throughput with higher quantization error |
| log-sum-exp trick | Numerical trick: subtract row max before exponentiation to avoid overflow |
| Time complexity | FLOPs order; both standard and FlashAttention are \(O(N^2 d)\) |
| Space complexity | Memory usage; standard \(O(N^2)\), FlashAttention \(O(N d)\) |
| I/O complexity | HBM↔SMEM transfer volume; FlashAttention-V1 is provably I/O-optimal |
