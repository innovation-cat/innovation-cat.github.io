...
- **Step 2: accumulate row scalars and $d_V$.**
  ...
  Please note that we **do not** save $\textcolor{blue}{\mathbf{d_{A_{ij}}}}$. That's because, like matrix $A$, $d_A$ requires O(N \times N) space memory. Once $d_{A_{ij}}$ is done, we can derive $d_{V_{ij}}$, again tile-by-tile.
