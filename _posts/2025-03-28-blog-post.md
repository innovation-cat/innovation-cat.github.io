...
In the previous article, we examined architectural strategies for <strong style="color: blue;">stability</strong>, contrasting how U-Nets and DiTs differ in their design philosophies and failure modes. In this article, we shift the focus to <strong style="color: blue;">efficiency</strong> — how diffusion architectures can be restructured to <strong style="color: blue;">accelerate computation [F]</strong> and <strong style="color: blue;">reduce memory usage [M]</strong>, both during training and inference.
...
<h1 id="section2.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">1.2 Scope of This Article</h1>
...
Throughout the article, we mark each technique with [F] and/or [M] to clarify its dominant benefits.

### 1.3 Focus on UNet and DiT Architectures

Diffusion models today are largely built on two families of backbones:

* **UNet-based architectures**, as popularized by Stable Diffusion, where multi-scale convolutional processing with long skip connections dominates.
* **DiT (Diffusion Transformer)-based architectures**, where homogeneous Transformer blocks handle generation without explicit multi-scale skip structures.

These two designs represent different philosophies: UNets emphasize hierarchical feature fusion, while DiTs rely on deep, uniform attention-based stacks. Both introduce unique challenges and opportunities for efficiency. This article explores optimizations tailored to each, as well as cross-architecture strategies that apply to both.

### 1.4 Dimensions of Optimization

We organize the efficiency landscape into four major parts:

1. **Representation-Level Efficiency**: Reducing the problem size through latent spaces, cascades, and multi-resolution design.
2. **Backbone and Building Blocks**: Lightweight convolutions, streamlined feed-forward networks, normalization refinements, and dynamic computation strategies in UNet/DiT backbones.
3. **Efficient Attention**: From sparse/local to kernelized and FlashAttention, attention remains the primary bottleneck in both compute and memory.
4. **System-Level Memory Efficiency**: Training-time methods such as checkpointing and ZeRO, and inference-time methods such as KV cache optimization and quantization.
