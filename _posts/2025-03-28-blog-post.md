...
$$
\textbf{FLOPs}_{\textbf{conv}} = H \cdot W \cdot K^2 \cdot C_{\text{in}} \cdot C_{\text{out}}.
$$

The number of parameters is:

$$
\textbf{Param}_{\textbf{conv}} = K^2 \cdot C_{\text{in}} \cdot C_{\text{out}}
$$
...
- **depthwise convolution**: that applies a single spatial filter per input channel. specifically, each input channel is filtered independently with a spatial kernel of size $ K \times K \times 1 $. The output remains $ C_{\text{in}} $ channels. The computational cost (in FLOPs) and the number of parameters are:
  ...
  $$
  \textbf{FLOPs}_{\text{dw}} =  H' \cdot W' \cdot K^2 \cdot C_{\text{in}},\qquad \textbf{Param}_{\text{dw}} = K^2 \cdot C_{\text{in}}
  $$
...
- **pointwise convolution**: A 1×1 convolution is applied to project the $ C_{\text{in}} $-channel feature map into $ C_{\text{out}} $ channels. The computational cost (in FLOPs) and the number of parameters are:
  ...
  $$
  \textbf{FLOPs}_{\text{pw}} =  H' \cdot W' \cdot C_{\text{in}} \cdot C_{\text{out}},\qquad \textbf{Param}_{\text{pw}} = C_{\text{in}} \cdot C_{\text{out}}
  $$
...
$$
\begin{align}
  & \textbf{FLOPs}_{\text{seq}} = \textbf{FLOPs}_{\text{dw}} + \textbf{FLOPs}_{\text{pw}} = H' \cdot W' \cdot \left( K^2 \cdot C_{\text{in}} + C_{\text{in}} \cdot C_{\text{out}} \right) \\[10pt]
  & \textbf{Param}_{\text{seq}} = \textbf{FLOPs}_{\text{dw}} + \textbf{FLOPs}_{\text{pw}} = K^2 \cdot C_{\text{in}} + C_{\text{in}} \cdot C_{\text{out}}
\end{align}
$$
...
$$
\frac{\textbf{FLOPs}_{\text{seq}}}{\textbf{FLOPs}_{\text{conv}}} = \frac{1}{C_{\text{out}}} + \frac{1}{K^2}
$$
