...
## <span style="color:#3498DB;">Step 1: Gradients $d_A$ and $d_V$ through the output linear combination</span>

Since $O = A V$, the differentials are governed by standard matrix multiplication rules.
The gradients with respect to $V$ and $A$ are

$$
dV = A^\top \cdot dO,\qquad
dA = dO \cdot V^\top.
$$

If dropout is applied to the attention probabilities during training, i.e., $A' = (A \odot \mathcal{M})/p$ with dropout mask $\mathcal{M}$ and keep probability $p$, then the gradient through $A$ becomes

$$
dA = (dO \cdot V^\top) \odot (\mathcal{M}/p).
$$

## <span style="color:#3498DB;">Step 2: Gradients $d_S$ through the softmax function</span>

This is the most intricate step due to the nature of the softmax derivative. Let $ S \in \mathbb{R}^{N \times N} $ be a matrix of input logits. The softmax function is applied **row-wise** to $S$ to produce the output matrix $A \in \mathbb{R}^{N \times N}$, where each element is given by:

$$
A_{ij} = \frac{\exp(S_{ij})}{\sum_{k=1}^{N} \exp(S_{ik})}.
$$

Given the upstream gradient $dA = \frac{\partial L}{\partial A}$, our goal is to compute the gradient of the loss $ L $ with respect to the input logits $ S $, denoted as $ dS = \frac{\partial L}{\partial S} $.
...
$$
dS_{ij} = \frac{\partial L}{\partial S_{ij}} = \sum_{m=1}^{N} \sum_{n=1}^{N} \frac{\partial L}{\partial A_{mn}} \cdot \frac{\partial A_{mn}}{\partial S_{ij}}.
$$
...
Consequently, for an upstream gradient $g = dA$, the gradient with respect to the scores is

$$
ds = \bigl(g - (a^\top g)\mathbf{1}\bigr) \odot a.
$$

Vectorizing over all rows yields

$$
dS = \bigl(dA - \operatorname{rowsum}(dA \odot A)\bigr) \odot A,
$$

where $\operatorname{rowsum}(X)$ denotes the vector of row-wise sums, broadcast across columns.

For masked entries of $S$, the corresponding entries of $dS$ are set to zero. If the bias $B$ is learnable, then the gradient with respect to the bias is given by

$$
dB = dS \quad (\text{restricted to unmasked positions}).
$$
