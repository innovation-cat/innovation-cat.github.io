...
<table align="center">
  ...
  <!-- Core symbols -->
  <tr>
    <td rowspan="9" align="center">Core symbols</td>
    <td>$$N$$</td>
    <td>Sequence length (number of tokens)</td>
  </tr>
  <tr><td>$$d$$</td><td>Head dimension</td></tr>
  <tr><td>$$d_v$$</td><td>Value dimension (often $$d_v = d$$)</td></tr>
  <tr><td>$$Q \in \mathbb{R}^{N \times d}$$</td><td>Query matrix</td></tr>
  <tr><td>$$K \in \mathbb{R}^{N \times d}$$</td><td>Key matrix</td></tr>
  <tr><td>$$V \in \mathbb{R}^{N \times d_v}$$</td><td>Value matrix</td></tr>
  <tr><td>$$S = QK^\top/\sqrt d$$</td><td>Attention score matrix</td></tr>
  <tr><td>$$P = \mathrm{softmax}(S)$$</td><td>Attention probability matrix</td></tr>
  <tr><td>$$O = PV$$</td><td>Attention output</td></tr>

  <!-- Streaming softmax -->
  <tr>
    <td rowspan="11" align="center">Streaming softmax</td>
    <td>$$B_r$$</td><td>Row tile size (blocking size for $$Q$$)</td>
  </tr>
  <tr><td>$$B_c$$</td><td>Column tile size (blocking size for $$K,V$$)</td></tr>
  <tr><td>$$i$$</td><td>Row index (query index or row in a row tile)</td></tr>
  <tr><td>$$j$$</td><td>Column tile index (key/value tile)</td></tr>
  <tr><td>$$T_c = \lceil N / B_c \rceil$$</td><td>Number of column tiles</td></tr>
  <tr><td>$$\tilde m_i^{(j)}$$</td><td>Row maximum inside tile $$j$$</td></tr>
  <tr><td>$$\tilde \ell_i^{(j)}$$</td><td>Row sum of exponentials inside tile $$j$$</td></tr>
  <tr><td>$$\tilde u_i^{(j)}$$</td><td>Tile-local unnormalized weighted sum of values</td></tr>
  <tr><td>$$m_i^{(j)}$$</td><td>Global row maximum after tile $$j$$</td></tr>
  <tr><td>$$\ell_i^{(j)}$$</td><td>Global row exp-sum after tile $$j$$</td></tr>
  <tr><td>$$\tilde O_i^{(j)}$$</td><td>Global unnormalized weighted sum after tile $$j$$</td></tr>
  <tr><td>$$O_i = \tilde O_i^{(T_c)}/\ell_i^{(T_c)}$$</td><td>Final normalized row output</td></tr>

  <!-- Backpropagation -->
  <tr>
    <td rowspan="4" align="center">Backpropagation</td>
    <td>$$dO$$</td><td>Gradient of output $$O$$</td>
  </tr>
  <tr><td>$$dQ, dK, dV$$</td><td>Gradients of $$Q, K, V$$</td></tr>
  <tr><td>$$dS$$</td><td>Gradient of score matrix $$S$$</td></tr>
  <tr><td>RNG mask</td><td>Dropout random mask (reconstructed deterministically)</td></tr>

  <!-- Hardware -->
  <tr>
    <td rowspan="9" align="center">Hardware</td>
    <td><b>HBM</b></td><td><i>High Bandwidth Memory</i> — off-chip GPU memory, large capacity (GBs), high bandwidth but higher latency</td>
  </tr>
  <tr><td><b>SRAM</b></td><td><i>Static Random Access Memory</i> — on-chip memory, very fast, very small (KB–MB)</td></tr>
  <tr><td><b>SMEM</b></td><td><i>Shared Memory</i> — CUDA’s on-chip shared memory (a form of SRAM)</td></tr>
  <tr><td><b>Registers</b></td><td>Fastest on-chip storage inside GPU cores, very limited capacity</td></tr>
  <tr><td><b>Occupancy</b></td><td>Ratio of active threads to maximum supported threads (GPU utilization)</td></tr>
  <tr><td><b>Warp</b></td><td>Basic execution unit on NVIDIA GPUs (32 threads in lockstep)</td></tr>
  <tr><td><b>Tensor Core</b></td><td>Specialized GPU unit for high-throughput matrix multiplications (FP16/BF16/FP8)</td></tr>
  <tr><td><b>TMA</b></td><td><i>Tensor Memory Accelerator</i> — Hopper feature for efficient HBM↔SMEM transfers</td></tr>
  <tr><td><b>WGMMA</b></td><td><i>Warp Group Matrix Multiply-Accumulate</i> — Hopper instruction for warp-group GEMMs</td></tr>

  <!-- Precision -->
  <tr>
    <td rowspan="5" align="center">Precision & numerical</td>
    <td><b>FP32</b></td><td>32-bit floating point precision</td>
  </tr>
  <tr><td><b>FP16</b></td><td>16-bit half precision floating point</td></tr>
  <tr><td><b>BF16</b></td><td>16-bit bfloat16 (FP32 exponent range, lower mantissa precision)</td></tr>
  <tr><td><b>FP8</b></td><td>8-bit floating point (Hopper), extreme throughput with higher quantization error</td></tr>
  <tr><td>log-sum-exp trick</td><td>Numerical trick: subtract row max before exponentiation to avoid overflow</td></tr>

  <!-- Complexity -->
  <tr>
    <td rowspan="3" align="center">Complexity</td>
    <td>Time complexity</td><td>FLOPs order; both standard and FlashAttention are $$O(N^2 d)$$</td>
  </tr>
  <tr><td>Space complexity</td><td>Memory usage; standard $$O(N^2)$$, FlashAttention $$O(N d)$$</td></tr>
  <tr><td>I/O complexity</td><td>HBM↔SMEM transfer volume; FlashAttention-V1 is provably I/O-optimal</td></tr>
</table>
