...
For each column tile $j$, Load $K_j$ and $V_j$ from HBM into SMEM/registers, compute local score block $S_{ij}=Q_iK_j^\top$. Within the SM, compute tile-local statistics:
  ...
  \tilde \ell_i^{(j)}=\sum_{k\in j} e^{\left( S_{ik}-\tilde m_i^{(j)} \right)},\quad
  \tilde u_i^{(j)}=\sum_{k\in j} e^{\left( S_{ik}-\tilde m_i^{(j)} \right)} V_k
  $$
...
Update global online softmax statistics (kept per row in registers/SMEM), discard $S_{ij}$ immediately after use—never stored to HBM.
  ...
  \ell_i^{(j)} &= e^{\left( m_i^{(j-1)}-m_i^{(j)} \right)}\ell_i^{(j-1)} + e^{\left( \tilde m_i^{(j)}-m_i^{(j)} \right)}\tilde \ell_i^{(j)}
  \end{aligned}
  $$
...
$$
\begin{align}
O_i^j & = \frac{\sum_{k \in {1,\dots,j-1}} e^{\left( S_{ik}-m_i^{(j)} \right)} V_k + \sum_{k \in {j}} e^{\left( S_{ik}-m_i^{(j)} \right)} V_k}{l_i^j} \\[10pt]
& =  \frac{e^{\left( m_i^{j-1} - m_i^j \right)}\,l_i^{j-1}}{l_i^j}\,O_i^{j-1} + \frac{e^{\left( \tilde m_i^{(j)} - m_i^j \right)}}{l_i^j}\,\tilde u_i^{(j)}
\end{align}
$$
...
The FlashAttention backward algorithm can be divided into 

- **Pre-computation of the $D = [\text{rowsum}(P \odot \text{dP})]$**. The most complex term in the standard backward pass is $\text{dS} = P \odot (\text{dP} - \text{rowsum}(P \odot \text{dP}))$. The FlashAttention algorithm begins with a crucial mathematical insight that simplifies the rowsum term. Let us define a vector $D \in \mathbb{R}^{N}$ where $D_i = [\text{rowsum}(P \odot \text{dP})]_i$.

**Theorem:** The vector $D$ can be computed as $D = \text{rowsum}(\text{dO} \odot O)$.

**Proof:** We aim to prove that for any row $i$: $[\text{rowsum}(P \odot \text{dP})]_i = [\text{rowsum}(\text{dO} \odot O)]_i$.

Let's start with the left-hand side for row $i$:

$$
[\text{rowsum}(P \odot \text{dP})]_i = \sum_{j=1}^{N} P_{ij} \cdot (\text{dP})_{ij}
$$

We substitute the definition $\text{dP} = \text{dO}V^T$, where $(\text{dP})_{ij} = \sum_{k=1}^{d} (\text{dO})_{ik} \cdot V_{jk}$:

$$
= \sum_{j=1}^{N} P_{ij} \left( \sum_{k=1}^{d} (\text{dO})_{ik} V_{jk} \right)
$$

By swapping the order of summation:

$$
= \sum_{k=1}^{d} (\text{dO})_{ik} \left( \sum_{j=1}^{N} P_{ij} V_{jk} \right)
$$

The inner summation, $\sum_{j=1}^{N} P_{ij} V_{jk}$, is precisely the definition of the matrix product $(PV)_{ik}$, which equals $O_{ik}$. Substituting this back, we get:

$$
= \sum_{k=1}^{d} (\text{dO})_{ik} \cdot O_{ik}
$$

This final expression is exactly the definition of $[\text{rowsum}(\text{dO} \odot O)]_i$. Thus, the identity is proven. $\blacksquare$

This identity is powerful: it allows us to compute a key component for the `dS` gradient using only matrices of size $O(Nd)$, completely bypassing the need for the $O(N^2)$ matrices `P` and `dP`. This vector `D` is computed once at the beginning of the backward pass.
