...
### Step 1 — Partition Q, K, V

- Split $Q$ with size $N \times d$ into **row tiles** of size $B_r\times d$.
- Split $K$ and $V$ into **column tiles** of size $B_c\times d$ and $B_c\times d_v$.
- Assign each row tile of $Q$ to one SM (Streaming Multiprocessor). For example, $\text{SM}_i$ is responsible for all computations related to $Q_i$, this block will remain resident in on-chip memory for the duration of its computation.

### Step 2 — Stream K/V tiles

For each column tile $j$:

- Load $K_j,V_j$ from HBM into SMEM/registers.
- Compute local score block $S_{ij}=Q_iK_j^\top$.
- Within the SM, compute tile-local statistics:
...
- Update global online softmax statistics (kept per row in registers/SMEM):

  $$
  \begin{aligned}
  m_i^{(j)} &= \max(m_i^{(j-1)}, \tilde m_i^{(j)}), \\
  \ell_i^{(j)} &= e^{m_i^{(j-1)}-m_i^{(j)}}\ell_i^{(j-1)} + e^{\tilde m_i^{(j)}-m_i^{(j)}}\tilde \ell_i^{(j)}, \\
  \tilde O_i^{(j)} &= e^{m_i^{(j-1)}-m_i^{(j)}}\tilde O_i^{(j-1)} +
                      e^{\tilde m_i^{(j)}-m_i^{(j)}}\tilde u_i^{(j)} .
  \end{aligned}
  $$
   
- Discard $S_{ij}$ immediately after use—never stored to HBM.


### Step 3 — Finalize output

After all $K/V$ tiles are streamed:

$$
O_i = \frac{\tilde O_i^{(T_c)}}{\ell_i^{(T_c)}} .
$$

Write $O_i$ back to HBM. Only the per-row statistics $(m_i,\ell_i)$ are saved for backward.

**Key properties:**

* Exact computation, identical to dense attention.
* Memory traffic: only $Q,K,V,O$ move to/from HBM; $S$ and $P$ never do.
* Peak memory: $O(Nd)$ instead of $O(N^2)$.
