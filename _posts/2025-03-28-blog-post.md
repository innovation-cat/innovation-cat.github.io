...
- **Step 2: accumulate row scalars and $d_V$.**
  ...
  Calculate $d_A$ is simple. Load $d_{O_i}$ and $V_j$ from HBM into SRAM, for each block $\mathbf{d_{A_{ij}}}$, it can be obtained through dot product.
  
  $$
  d_{A_{ij}} = d_{O_{ij}} {V_{ij}}^T
  $$

  Please note that we **do not** save $\mathbf{d_{A_{ij}}}$. That's because, like matrix $A$, $d_A$ requires O(N \times N) space memory. Once $d_{A_{ij}}$ is done, we can derive $d_{V_{ij}}$, again tile-by-tile.
  
  $$
  d_{V_{ij}} = {A_{ij}}^T d_{O_{ij}} 
  $$
  
  For $\mathbf{d_{S_{ij}}}$, we now 
