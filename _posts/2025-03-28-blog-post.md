...
$$
 ...
 the attention mechanism can be approximated as `Q(KᵀV)`. This changes the computational dependency from `n²` to `n`.
    *   **Impact**: This transforms the complexity to `O(n)`, representing a paradigm shift from quadratic to linear scaling. While these are approximations, they often achieve performance comparable to standard attention with massive efficiency gains.
