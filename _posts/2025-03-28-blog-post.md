...
<h1 id="section2.3.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.2 UNET is a natural multi-resolution architecture</h1>


 At level (s) (downsampling by (2^s)), let spatial size be (H_s!=!H/2^s,, W_s!=!W/2^s), channels (C_s). A stack of (L_s) conv blocks costs
[
\text{FLOPs}*s ;\approx; \sum*{\ell=1}^{L_s} H_s W_s k^2 C_{s,\ell}^{\text{in}} C_{s,\ell}^{\text{out}}.
]
Summing across pyramid levels (s=0,\dots,S) yields
[
\text{FLOPs}*{\text{U-Net}} ;\approx; \sum*{s=0}^{S} \text{FLOPs}_s,
]
where each deeper level is **4× cheaper** per conv than level (s-1) (for similar channel widths), allowing many blocks at coarse scales without exploding cost. Skips ({\mathbf{f}*s}) pass high-res features to the decoder to recover details:
[
\mathbf{y}*{s-1} ;=; \phi!\big(,\text{Up}(\mathbf{y}*s);\oplus;\mathbf{f}*{s-1},\big),
]
with (\oplus) concatenation or addition, (\phi) a small conv stack.
**Takeaway:** U-Net’s **efficiency and fidelity** rely on this pyramid: **cheap global processing** at low res + **detail recovery** via high-res skips.
