...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section2">2. Representation-Level Efficiency Design</a>
		<ul>
			<li><a href="#section2.1">2.1 Latent Diffusion</a></li>
			<li><a href="#section2.2">2.2 Multi-Stage Cascades</a></li>
			<li><a href="#section2.3">2.3 Multi-Resolution Strateries</a></li>
		</ul>
	</li>	
    ...
    <li><a href="#section5">5. Memory Efficiency (Leaner)</a>
	  <ul>
...
- **[F] Computation Acceleration**: Reducing FLOPs, bandwidth, or latency without sacrificing output quality.
- **[M] Memory Optimization**: Minimizing GPU memory footprint, both in forward/backward passes during training and in caching/quantization during inference.

Throughout the article, we mark each technique with <strong style="color: blue;">[F] and/or [M]</strong> to clarify its dominant benefits.
...
where $\mathcal{E}$ and $\mathcal{D}$ are encoder–decoder pairs trained with perceptual and adversarial losses. The diffusion process then operates on $\mathbf{z} \in \mathbb{R}^{h \times w \times c}$, with $h = H/f, \; w = W/f$, and $f \in \{4,8\}$ as the downsampling factor. This reduces computation approximately by
...
Another efficiency-oriented design is the use of **multi-stage cascaded models** [^sdxl]. instead of compressing the space, they **split generation into multiple independent diffusion stages**, each operating at progressively higher resolutions. Formally, for a target image $\mathbf{x}_T$:
...
<h1 id="section2.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.3 Multi-Resolution (within a single model)</h1>
...
<h1 id="section2.3.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.1 What benefits does a hierarchical processing bring</h1>

Efficiency is not the only reason to adopt multi-resolution designs. A single-scale network has two major problems:

- **Time/Memory Complexity**: For an image of size $H \times W$:

  - Convolutional FLOPs scale as
    
	$$
     \text{FLOPs}*{\text{conv}} ;\sim; H W , k^2 , C*{\text{in}} C_{\text{out}},
    $$
	
  - Self-attention FLOPs scale quadratically as
    
	$$
     \text{FLOPs}_{\text{attn}} ;\sim; \mathcal{O}( (H W)^2 d).
    $$
    
	Keeping all layers at full resolution is prohibitively expensive.
...
Thus, **hierarchical processing serves a dual role**:

- [F] reduce cost by working at coarse scales,
- [M] save memory by reducing activation size,
- and **capture global context** that is inaccessible at fixed resolution.


U-Net’s encoder–decoder with long skips is **by design** a multi-resolution backbone. However, Vanilla **DiT** operates on a fixed token sequence, Each Transformer block preserves this resolution, making DiT **homogeneous and single-scale**.

<h1 id="section2.3.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.2 DiT: single-scale by default, multi-resolution optional</h1>




Vanilla **DiT** operates on a fixed token sequence (patches of size (p)):

$$
N = \frac{H}{p}\cdot \frac{W}{p}, \quad \mathbf{X}_0 \in \mathbb{R}^{N \times d}.
$$

Each Transformer block preserves this resolution, making DiT **homogeneous and single-scale**. This simplifies design but has two consequences:

- Attention has quadratic cost (\mathcal{O}(N^2 d)), which grows quickly for high-resolution images.
- Global relationships are handled well (since attention is global), but computation is heavy and memory-hungry.

Some transformer variants also attempt to adopt the multi-resolution hierarchical architecture, such as swin transformer [^swin]. There are several different ways to achieve it


1. **Hierarchical Patch Merging (pyramidal DiT).** At stage $s$:
...
2. **Cross-Scale Attention.** Maintain a low-resolution stream $\mathbf{X} \times {\text{low}}$ and let high-res tokens $\mathbf{Q}$ attend to it:
...
3. **Token Pooling / Dynamic Selection.** Select only (M \ll N) tokens based on saliency scores (s_i = g(\mathbf{x}_i)), then run attention/MLPs on the reduced set.

4. **Hybrid U-shaped DiT.** Construct encoder–decoder stages with Transformer blocks at each scale, plus skip connections in token space, mimicking U-Net’s pyramid but replacing convs with attention blocks.
