...
FlashAttention v1 is a landmark algorithm designed to accelerate the attention mechanism in Transformers. Its innovation lies not in changing the mathematical output of attention but in fundamentally restructuring its computation to be "I/O-aware," thereby overcoming the primary performance bottleneck on modern GPUs.


<h1 id="section4.1.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">4.1.1 The Core Problem: Why Standard Attention is Inefficient on GPUs</h1>

To understand FlashAttention, one must first understand the GPU memory hierarchy and its performance characteristics. GPUs have two main types of memory:
...
The standard attention computation, $O = softmax(QK^T)V$, involves several distinct steps that lead to inefficient memory access patterns:
- **Step 1**:  Compute and materialize $S = QK^T$. GPUs compute $S$ tile by tile: a small block of $Q$ and a small block of $K$ are loaded into on-chip SRAM (registers/shared memory), a micro-GEMM produces one tile $S_{i,j}=Q_iK^T_j$, and that tile is immediately written to HBM, Repeat until every $S_{i,j} $has been produced and stored, so that the full matrix $S$ now exists in HBM.

  ![tradition_step_1](/images/posts/2025-03-28-blog-post/tradition_step_1.jpg)

- **Step 2**:  Softmax over $S$(rowwise). Read tiles of $S$ from HBM, to compute a numerically stable softmax, the kernel must scan each row to get the rowwise max, then scan again to accumulate.

  $$
  A_{i, j} = softmax(S_{i, j}) = \frac{e^{s_{i, j}}}{\sum_{k=1}{d}{e^{i, k}}} = \frac{e^{s_{i, j}-m_i}}{\sum_{k=1}{d}{e^{i, k}- m_i}}
  $$

  the resulting probabilities are written back to HBM (again tile by tile).

  ![tradition_step_2](/images/posts/2025-03-28-blog-post/tradition_step_2.jpg)

- **Step 3**:  Multiply $A$ with $V$ to get output $O$. pull a tile of $A$ (from HBM) and a tile of $V$ (from HBM) onto the SM, multiply, accumulate. When a partial output tile is done, write it back to HBM. After all tiles, the full $O$ lives in HBM.

  ![tradition_step_3](/images/posts/2025-03-28-blog-post/tradition_step_3.jpg)

**The root problem**: The huge $N x N$ intermediate matrices ($A$ and $S$) are repeatedly read from and written to the slow HBM. When the sequence length $N$ is large (e.g., 8K), this matrix becomes enormous ($8K x 8K$), and the time spent on memory I/O dwarfs the time spent on actual computation (FLOPs). This makes the operation **memory-bandwidth bound**.
