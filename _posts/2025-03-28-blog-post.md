...
During training, the quadratic time and memory cost of standard attention are 

$$
\mathcal{O}(n^2 d) \quad \text{time}, \qquad \mathcal{O}(n^2) \quad \text{memory},
$$

where $n$ is the sequence length and $d$ the feature dimension — quickly becomes the bottleneck. To alleviate this, researchers have proposed three main families of techniques that accelerate computation during training.

---

## 3.1 Sparse / Local Attention

The quadratic time and memory complexity of the vanilla attention mechanism ($O(n^2 d)$ in time and $O(n^2)$ in memory, where $n$ is the sequence length and $d$ the hidden dimension) makes it infeasible for very long sequences. To address this, a wide family of **sparse attention mechanisms** has been proposed. The key idea is to restrict each query to attend only to a **subset** of keys, thereby reducing both computation and storage. Below we summarize the most prominent variants.
...
- **Reformer [^reformer]**: Reformer replaces the quadratic similarity search with **locality-sensitive hashing (LSH)**. Queries and keys are bucketed via hash codes; attention is restricted to tokens within the same bucket.
...
**Core idea.** Reformulate the softmax attention kernel to avoid explicit construction of the $n \times n$ similarity matrix. This is achieved by kernel feature maps or low-rank projections, enabling a reordering of matrix multiplications.

**Computation.**

$$
\text{softmax}(QK^\top)V \;\approx\; \Phi(Q)\Big(\Phi(K)^\top V\Big),
$$

where $\Phi(\cdot)$ is a positive kernel mapping of dimension $r \ll n$.

**Complexity.**

$$
\mathcal{O}(n d r) \;\approx\; \mathcal{O}(n d).
$$

**Examples.** Performer (FAVOR+), Linformer, Nyströmformer, Linear Transformers.

**Characteristics.**

* True linear-time scaling in sequence length.
* Approximate by construction; accuracy depends on the choice of kernel or projection rank.

---

## 3. Hardware-Aware Exact Attention (FlashAttention)

**Core idea.** Keep the exact softmax formulation, but restructure computation to minimize memory traffic. FlashAttention tiles the sequence into blocks and performs on-chip log-sum-exp normalization, thereby avoiding materialization of the $n^2$ attention matrix.

**Complexity.**

* Arithmetic FLOPs remain

  $$
  \mathcal{O}(n^2 d).
  $$
* However, memory usage drops from $\mathcal{O}(n^2)$ to $\mathcal{O}(n d)$, and training throughput improves dramatically on GPUs due to reduced HBM I/O.

**Examples.** FlashAttention v1/v2/v3, PyTorch SDPA kernels.

**Characteristics.**

* No approximation: numerically identical to standard softmax attention.
* Gains come from I/O-awareness and kernel optimization rather than lower FLOPs.
