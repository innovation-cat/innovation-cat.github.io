...
$$
\textbf{FLOP}_{\textbf{std}} = H \cdot W \cdot K^2 \cdot C_{\text{in}} \cdot C_{\text{out}}.
$$

The number of parameters is:

$$
\textbf{PARAM}_{\textbf{std}} = K^2 \cdot C_{\text{in}} \cdot C_{\text{out}}
$$
...
- **depthwise convolution**: that applies a single spatial filter per input channel. specifically, each input channel is filtered independently with a spatial kernel of size $ K \times K \times 1 $. The output remains $ C_{\text{in}} $ channels:

  $$
  \mathbf{Z}_{i,j,d} = \sum_{k=1}^{K} \sum_{l=1}^{K} \mathbf{W}^{\text{dw}}_{k,l,d} \cdot \mathbf{X}_{i+k-1, j+l-1, d}
  $$

  where $ \mathbf{W}^{\text{dw}} \in \mathbb{R}^{K \times K \times C_{\text{in}}} $. The computational cost (in FLOPs) and the number of parameters are:
  
  $$
  \textbf{FLOP}_{\text{dw}} =  H' \cdot W' \cdot K^2 \cdot C_{\text{in}},\qquad \textbf{PARAM}_{\text{dw}} = K^2 \cdot C_{\text{in}}
  $$


- **pointwise convolution**: A 1×1 convolution is applied to project the $ C_{\text{in}} $-channel feature map into $ C_{\text{out}} $ channels:

  $$
  \mathbf{Y}_{i,j,c} = \sum_{d=1}^{C_{\text{in}}} \mathbf{W}^{\text{pw}}_{1,1,d,c} \cdot \mathbf{Z}_{i,j,d}
  $$

  where $ \mathbf{W}^{\text{pw}} \in \mathbb{R}^{1 \times 1 \times C_{\text{in}} \times C_{\text{out}}} $. The computational cost (in FLOPs) and the number of parameters are:
  
  $$
  \textbf{FLOP}_{\text{pw}} =  H' \cdot W' \cdot C_{\text{in}} \cdot C_{\text{out}},\qquad \textbf{PARAM}_{\text{pw}} = C_{\text{in}} \cdot C_{\text{out}}
  $$
  
The total computational cost (in FLOPs) and the total number of parameters are:

$$
  \textbf{FLOP}_{\text{tot}} =  H' \cdot W' \cdot \left( K^2 \cdot C_{\text{in}} + C_{\text{in}} \cdot C_{\text{out}} \right),\qquad \textbf{PARAM}_{\text{tot}} = K^2 \cdot C_{\text{in}} + C_{\text{in}} \cdot C_{\text{out}}
$$

The relative computational reduction compared to standard convolution is:

$$
\frac{\textbf{FLOP}_{\text{std}}}{\textbf{FLOP}_{\text{tot}}} = \frac{1}{C_{\text{out}}} + \frac{1}{K^2}
$$

For typical values (e.g., $ K=3, C_{\text{out}} \geq 64 $), this yields > 85\% reduction in FLOPs.
