...
$$
d_A = (d_O \cdot V^\top) \odot (\mathcal{M}/p).
$$
...
Given the upstream gradient $d_A = \frac{\partial L}{\partial A}$, our goal is to compute the gradient of the loss $ L $ with respect to the input logits $ S $, denoted as $ d_S = \frac{\partial L}{\partial S} $.
