...
- 🧠 **A3: Spatially Separable Convolution**:    Decomposes a $K \times K$ convolution into a sequence of a $K \times 1$ convolution followed by a $1 \times K$, particularly for factorizing **larger kernels** like $7 \times 7$ to reduce computational cost.
  ...
  $$
  \text{FLOPs}_{\text{group}} = H' \cdot W' \cdot \left( (2K) \cdot \frac{C_{\text{in}} \cdot C_{\text{out}}}{g} \right)\,\qquad \text{Param}_{\text{group}} =  (2K) \frac{C_{\text{in}} \cdot C_{\text{out}}}{g}
  $$
...
$$
\text{FLOPs}_{\text{bottleneck}} 
= H \cdot W \cdot \Big(\frac{2}{r}C^2 + \frac{k^2}{r^2}C^2\Big)\,\quad \text{Params}_{\text{bottleneck}} 
= \frac{2}{r}C^2 + \frac{k^2}{r^2}C^2
$$
