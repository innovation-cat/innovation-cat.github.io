...
- **Step 1: Pre-computation of the $D = [\text{rowsum}(A \odot \text{d_A})]$**. The most complex term in the standard backward pass is $\text{d_S} = A \odot (\text{d_A} - \text{rowsum}(A \odot \text{d_A}))$. The FlashAttention algorithm begins with a crucial mathematical insight that simplifies the rowsum term. Let us define a vector $D \in \mathbb{R}^{N}$ where $D_i = [\text{rowsum}(A_i \odot \text{d_{A_i}})]$.
