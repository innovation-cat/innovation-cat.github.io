...
<div class="qa-block">
  ...
  <p>In diffusion models, the theoretical forward process defines a marginal distribution $p(x_t)$, which describes how data evolve under noise. However, <strong>this marginal is a complicated mixture of Gaussians determined by the unknown data distribution $q(x_0)$</strong>, and thus has no closed form. 

	$$
	p(x_t)=\int p(x_t \mid x_0) p_{\rm data}(x_0) {\mathbf d}x_0
	$$
	
  In practice, all derivations and training objectives rely instead on the <strong>conditional</strong> distribution $q(x_t \mid x_0) = \mathcal N(s_t x_0, \sigma_t^2 I)$, which is analytically tractable. By sampling $x_t$ from this conditional, we can compute losses and gradients without ever needing to explicitly represent the intractable marginal distribution.</p>
