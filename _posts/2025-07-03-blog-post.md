...
we extend \ref{eq:score_based_lens} and generalize to muliple conditions 
...
={\mathrm g}_{\text{cond}} + \sum_{k\in\mathcal P}w_k\cdot{\mathrm g}_{\text{guide}}^k - \sum_{m\in\mathcal N}w_m\cdot{\mathrm g}_{\text{guide}}^m
$$
...
$$
\mathcal{L}=\mathbb{E}_{x_0,t,\epsilon}\,\mathbb{E}{m_k}
...
During inference, we compute an unconditional prediction $\epsilon_u=\epsilon_\theta(x_t,t;\forall k:\varnothing^{(k)})$
and per-channel conditional predictions $\epsilon_k=\epsilon_\theta(x_t,t;c_k \text{ on})$.
Their differences define per-channel guidance directions:

$$
{\mathrm g}_{\text{guide}}^k = {\mathrm g}_{\text{cond}}^k - {\mathrm g}_{\text{uncond}}^k.
$$

We then aggregate them with positive or negative weights to form the final guided denoiser:
...
where $\mathcal P$ and $\mathcal N$ denote sets of positive and negative conditions respectively.
Negative prompts are implemented simply by using negative weights.	
This compositional rule enables multi-modal control—each channel contributes its own directional influence in the shared latent space.
