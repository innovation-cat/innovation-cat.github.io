...
Let's first examine the density function of the unconditional density distribution $p(x)$ and the density function of the conditional distribution $p(x\mid c)$: 
...
Now let's examine the meaning of density-ratio.
...
Let $p_1(x)$ denote a **strong (good)** model and $p_0(x)$ a **weak (bad)** one. AutoGuidance generalizes the CFG update as:

$$
\nabla_x \log p_{\text{total}}(x)
= \nabla_x \log p_1(x) + (w-1)\big(\nabla_x \log p_1(x) - \nabla_x \log p_0(x)\big).
$$

Here, $p_0$ no longer represents an unconditional distribution but any weaker estimator of the same target domain—such as:
