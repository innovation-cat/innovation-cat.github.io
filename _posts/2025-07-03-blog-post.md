...
Recall that CFG can be interpreted as a **density-ratio field** between conditional and unconditional distributions:
...
The unconditional branch $p(x)$ represents the real data manifold, while the conditional branch $p(x\mid c)$ can be viewed as a submanifold of $p(x)$. This is because the task of $p(x)$ is more challenging, as it needs to learn data from all categories, whereas the task of $p(x\mid c)$ is relatively simpler, requiring only the learning of the data distribution for a specific category. From the perspective of density regions, the density region of $p(x)$ envelops that of $p(x\mid c)$.
