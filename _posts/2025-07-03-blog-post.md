...
c^{(k)} & \text{if } m_k=1\ \text{and channel is available},\
\varnothing^{(k)} & \text{otherwise}.
\end{cases}
$$
...
$$
\mathcal{L}=\mathbb{E}*{x_0,t,\epsilon}\ \mathbb{E}*{{m_k}}\ \big|\epsilon-\epsilon_\theta\big(x_t,t;\tilde c^{(1)},\ldots,\tilde c^{(|\mathcal{K}|)}\big)\big|_2^2.
$$
