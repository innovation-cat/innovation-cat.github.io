...
where $\hat{g}(x_t,c)$ is an approximate gradient that enforces the conditional preference, and $\lambda(t)$ controls its strength. The canonical choice is

$$
g(x_t,t) \;=\; \nabla_{x_t}\log p(c \mid x_t),
$$

i.e., move $x_t$ along the gradient that increases the conditional log-likelihood. By Bayes,

$$
\log p(c \mid x_t) \;=\; \log p(x_t \mid c) - \log p(x_t) + \text{const},
$$
...
Thus the guided sampler can be written as

$$
\frac{dx_t}{dt} \;=\; f_\theta(x_t,t)\;+\;\lambda(t)\Big[\underbrace{\nabla_{x_t}\log p(x_t \mid c)}_{\text{pull toward conditional modes}} \;-\; \underbrace{\nabla_{x_t}\log p(x_t)}_{\text{repel generic density}}\Big].
$$

**Different guidance families differ only in how they approximate these two score terms.**
...
$$
\tilde{\epsilon}_t
=(1+w)\,\epsilon_\theta(x_t,c,t)\;-\;w\,\epsilon_\theta(x_t,\varnothing,t)
$$

where $w\ge 0$ is the **guidance scale**. Large (w) increases faithfulness to $c$ (mode-seeking), small $w$ preserves diversity (mode-covering).


---

<h1 id="section2.3.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.3.2 Summary</h1>

Classifier-Free Guidance (CFG) represents a pivotal evolution of the inference-time guidance paradigm first introduced by Classifier Guidance (CG).
