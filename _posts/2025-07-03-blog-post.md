...
CFG provides an elegant and simple way for guidance generation, allowing us to achieve a good balance between conditional matching and fidelity in controllable generation. However, CFG also has certain limitations and shortcomings.


1. **Off-manifold Sampling.** CFG performs a linear combination

  $$
  {\mathrm g}_{\text{total}} \;=\; {\mathrm g}_{\text{uncond}} \;+\; w\cdot \left({\mathrm g}_{\text{cond}} - {\mathrm g}_{\text{uncond}} \right)
  $$
  
  which is an *extrapolation* beyond the region spanned by conditional and unconditional predictions. As a result, each denoising step moves samples **outside the data manifold**, producing unrealistic intermediate states and numerically unstable trajectories. Over multiple steps, these deviations accumulate and the generated trajectory drifts away from the true data distribution.
  
2. **Oversaturation and Over-amplified Color Contrast.** The extrapolation term scales the component of the prediction **parallel to the conditional direction**, effectively acting as a gain amplifier on image intensity.
This amplifies high-frequency and high-magnitude responses—leading to

* excessively bright or dark regions,
* unnatural color saturation, and
* visual “plasticity” or loss of photometric realism.
  APG identifies this as the main cause of *oversaturation* artifacts in high-guidance generations.
