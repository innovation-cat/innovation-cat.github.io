...
Replacing the conditional score by the conditional one yields:

$$
\mathrm{d}x_t=\Big[f(t)x_t - g^2(t)\underbrace{\nabla_{x_t}\log p_t(x_t)}_{\text{unconditional score}} - g^2(t)\underbrace{\nabla_{x_t}\log p_t(c|x_t)}_{\text{classifier gradient}} \Big]\mathrm{d}t  + g(t)\,\mathrm{d}\bar w_t\label{eq:cg}
...
<h1 id="section2.2.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.2.1 Computing the Classifier Gradient</h1>
