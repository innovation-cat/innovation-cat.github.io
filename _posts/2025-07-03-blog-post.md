...
<h1 id="section3.3.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.3.2 Efficient Block Design</h1>
...
1. **Task Consistency.** 
   ...
   The unconditional model in CFG solves a **different task** with conditional model. As a result, its score field $\nabla_x \log p_0(x)$ differs not only in **quality** but also in **semantics** from that of the conditional branch $\nabla_x \log p_1(x\mid c)$. This **task discrepancy** leads to mismatched gradients: when the two score fields are combined, the sampling trajectory can **overshoot** or **drift** from the true conditional manifold, producing oversimplified or skewed compositions.
...
3. The "Bad Model" Has More Relevant Failure Modes
   ...
   The unconditional model, by contrast, does not share the same error structure, since it was never trained to model $p(x|c)$ at all. Hence, its errors are largely orthogonal to the conditional model’s, leading to noisy or even misleading guidance signals
