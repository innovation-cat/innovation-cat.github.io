  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section2">2. Steering the Diffusion Process with Global Guidance</a>
		<ul>
			...
			<li><a href="#section2.4">2.4 Multi-Condition Guidance in Diffusion Models</a></li>
			<li><a href="#section2.5">2.5 Limitations of Standard Classifier-Free Guidance (CFG)</a></li>
		</ul>
	</li>	
	<li><a href="#section3">3. Advances in Classifier-Free Guidance</a></li>
...
**Goal.** We want to steer a **fixed** diffusion model at **inference time** — without retraining — so that the generated sample $x_0$ better matches a user condition $c$ (text, class label, image cue, etc.). All methods in this family can be seen as injecting a **condition-dependent force field** into the reverse denoising dynamics. 

Recalling that in SDE diffusion models, we drive the data from a noise distribution to the real data distribution based on the score. When we use conditions for controllable generation, our score becomes a conditional score.

$$
\nabla_{x_t}\log p(x_t) \quad \Longrightarrow \quad \nabla_{x_t}\log p(x_t \mid c)
$$


  
where $p_t(x_t)$ is the marginal density at time $t$, and $c$ is condition signal, by Bayes' rule, the conditional score can be decomposed into two components.

$$
\nabla_{x_t}\log p(x_t \mid c) 
= \nabla_{x_t}\log p(x_t)  \;+\; \nabla_{x_t}\log p(c \mid x_t)\label{eq:score_based_lens}
$$


This shows that conditional score equals unconditional score plus an additional drift term. However, in practice we cannot compute $\nabla_{x_t}\log p(c\mid x_t)$ precisely — we must approximate it through a classifier (in Classifier Guidance) or through the difference between conditional and unconditional predictions (in Classifier-Free Guidance). These approximations are often biased or under-scaled relative to the true conditional gradient. Therefore, we introduce a tunable scalar $ w $ to rescale the guidance strength, $w$ is also known as **guidance scale**.

$$
\nabla_{x_t}\log p(x_t \mid c) 
= \nabla_{x_t}\log p(x_t)  \;+\; w\cdot \nabla_{x_t}\log p(c \mid x_t)\label{eq:guidance}
$$


Equation \ref{eq:guidance} is derived from the perspective of score prediction; in fact, we can generalize this conclusion to other objectives: For all four common parameterizations—**$\epsilon$-prediction**, **$v$-prediction**, **score-prediction**, and **$x_0$-prediction** — guidance reduces to the **same mixing rule** :

$$
{\mathrm g}_{\text{total}} \;=\; {\mathrm g}_{\text{uncond}} \;+\; w\cdot {\mathrm g}_{\text{guide}},
\qquad {\mathrm g}\in \{\epsilon,\,v,\,s,\,x_0\}\label{eq:general_guide}
$$
...
This is why it is called classifier guidance. Intuitively, the gradient from the classifier creates a force that pushes noisy samples toward the target class region in feature space.

**Advantages and limitations.** Classifier Guidance was a seminal step because it proved that diffusion trajectories can be steered at inference time via external gradients. However, its drawbacks are significant:

- It requires training a separate classifier across all noise levels, incurring high cost.

- The need for per-step backpropagation makes sampling slow.

- Control is limited to discrete categories rather than continuous or semantic attributes.

Despite these issues, Classifier Guidance provided the conceptual foundation for all subsequent “classifier-free” approaches.
...
Thus the **same network** learns two behaviors: **Conditional branch**: $\epsilon_\theta(x_t,c,t)$, and  **Unconditional branch**: $\epsilon_\theta(x_t,\varnothing,t)$. This duality is the key to estimating $\nabla_{x_t}\log p(c\mid x_t)$ **without** a classifier.  By Bayes,

$$
\nabla_{x_t}\log p(c\mid x_t) = \nabla_{x_t}\log p(x_t\mid c) - \nabla_{x_t}\log p(x_t) 
$$

This means CFG replaces the external estimator with an **internal contrast** between the model’s conditional and unconditional predictions. Generalize it to other objectives.


$$
{\mathrm g}_{\text{guide}} = {\mathrm g}_{\text{cond}} - {\mathrm g}_{\text{uncond}}, \qquad g\in \{\epsilon,\,v,\,s,\,x_0\}.  
$$


Substituting it into Eq. \ref{eq:general_guide}, we obtain that during the inference phase, the total direction of CFG is


$$
\begin{align}
{\mathrm g}_{\text{total}} \;& =\; {\mathrm g}_{\text{uncond}} \;+\; w\cdot {\mathrm g}_{\text{guide}} \\[10pt]
& =\; {\mathrm g}_{\text{uncond}} \;+\; w\cdot \left({\mathrm g}_{\text{cond}} - {\mathrm g}_{\text{uncond}} \right)
\end{align}
$$


**Significance.** CFG retains the theoretical insight of Classifier Guidance but achieves it without any auxiliary model or gradient computation. It supports continuous, multimodal, and high-dimensional conditions—text embeddings, layout vectors, or style codes—making it the de facto standard in modern systems such as Stable Diffusion, Imagen, and SDXL.
...
In practice, Real deployments combine multiple conditions to control complementary aspects of content and geometry. Formally, we consider a set of conditions consists of **positive** conditions $\mathcal{P}$ and **negative** conditions $\mathcal{N}$
...
To enable flexible combinations at inference, the model must see all possible subsets of channels during training. This is achieved through independent per-channel dropout, i.e., randomly replacing each condition $c_k$ by its null token $\varnothing^{(k)}$ with probability $q_k(t)$:

$$
\mathcal{L}=\mathbb{E}_{x_0,t,\epsilon}
\big\|\epsilon-\epsilon_\theta(x_t,t;\tilde {\mathcal{C}})\big\|_2^2,
$$

where 

$$
\tilde {\mathcal{C}} \;=\; \underbrace{\{\tilde c_1,\ldots,\tilde c_K\}}_{\text{positive conditions } \mathcal{P}} \;\cup\; \underbrace{\{\tilde c^-_1,\ldots,\tilde c^-_M\}}_{\text{negative conditions } \mathcal{N}}
$$


where each $\tilde c_k \in \tilde {\mathcal{C}}$ equals $c_k$ or $\varnothing^{(k)}$ depending on the dropout mask $m_k$. This ensures the model can handle any subset of available conditions at test time.
...
During inference, we extend \ref{eq:score_based_lens} and generalize to muliple conditions 


$$
\nabla_{x_t}\log p(x_t \mid \mathcal{C}) 
= \nabla_{x_t}\log p(x_t)  \;+\; \nabla_{x_t}\log p(\mathcal{C} \mid x_t)\label{eq:score_based_lens_multiple}
$$

Assuming conditional independence, using Bayes' rule to decompose the classifier gradient, we obtain
...
\;\approx\;\sum_{k=1}^K\nabla_{x_t}\log p_t(c_k\mid x_t)\;-\;\sum_{m=1}^M\nabla_{x_t}\log p_t(c^-_m\mid x_t)\label{eq:multi_conditions}
$$

Combining Eq. \ref{eq:multi_conditions} and Eq. \ref{eq:score_based_lens_multiple}, the total direction is 


$$
{\mathrm g}_{\text{total}}
={\mathrm g}_{\text{uncond}} + \sum_{k\in\mathcal P}w_k\cdot{\mathrm g}_{\text{guide}}^k - \sum_{m\in\mathcal N}w_m\cdot{\mathrm g}_{\text{guide}}^m
$$

where 

$$
{\mathrm g}_{\text{guide}}^k = {\mathrm g}_{\text{cond}}^k - {\mathrm g}_{\text{uncond}}^k
$$


where $w_k$ is guidance scale for the $k$-th condition, we use negative weight for negative condition.


---

<h1 id="section2.5" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">2.5 Limitations of Standard Classifier-Free Guidance (CFG)</h1>
...
1. **Probabilistic Perspective: Distortion of the Conditional Distribution.**

   Under ideal conditions, a diffusion model with $ w = 1 $ samples from the true conditional distribution $ p(x\mid c) $, generating diverse and natural results. However, increasing the guidance weight $ w $ effectively modifies this distribution into a new, biased one:
...
2. **Score-Function Perspective: Gradient Amplification and Manifold Departure.**

   In the score-based interpretation, CFG modifies the denoising direction as
...
<h1 id="section3" style="color: #1E3A8A; font-size: 28px; font-weight: bold; text-decoration: underline;">3. Advances in Classifier-Free Guidance</h1>

In the previous section, we identified key limitations of standard CFG. To overcome these challenges, a series of CFG variants have emerged, each targeting different aspects of the problem. All these approaches together represent a coherent evolution toward **adaptive**, **geometry-aware**, and **distribution-consistent** guidance.
...
<h1 id="section3.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.1 Adaptive Guidance (AG): Dynamic Switching Based on Alignment</h1>
...
<h1 id="section3.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.2  β-CFG: Adaptive Classifier-Free Guidance</h1>
...
<h1 id="section3.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.3  AutoGuidance: Replace Unconditional branch with Poor Model</h1>

AutoGuidance [^autoguidance] offers a new perspective on what constitutes the guidance itself. It reveals that the unconditional branch in CFG is not a mathematically required component, but rather a practical proxy for a weaker model that provides contrastive information.


Recall that the guidance direction of CFG can be interpreted as a **density-ratio field** between conditional and unconditional distributions:

$$
{\mathrm g}_{\text{guide}}\,=\,\nabla_x \log \frac{p(x\mid c)}{p(x)}\,=\,\nabla_x \log p(x\mid c) - \nabla_x \log p(x)
= 
$$


Let's first examine the density function of the unconditional density $p(x)$ and the conditional distribution $p(x\mid c)$: 


- The unconditional branch $p(x)$ represents the real data manifold, while the conditional branch $p(x\mid c)$ can be viewed as a submanifold of $p(x)$. 

- The task of $p(x)$ is more challenging, as it needs to learn data from all categories, whereas the task of $p(x\mid c)$ is relatively simpler, requiring only the learning of the data distribution for a specific category. 

- From the perspective of density regions, the density region of $p(x)$ envelops that of $p(x\mid c)$.

![condition and uncondition](/images/posts/2025-07-03-blog-post/condition_and_uncondition.jpg)

Now let's examine what exactly the meaning of density-ratio.


- In high-density regions, the probability value of $p(x\mid c)$ is very high, while $p(x)$, because it needs to account for other categories, has a relatively lower probability value, resulting in a very high value of this ratio. 

- In low-density regions, the probability values of both $p(x\mid c)$ and $p(x)$ decrease. However, since $p(x)$ is inherently more dispersed, it decreases more slowly. As a result, the ratio  is relatively lower.


**Key concludion**: This ratio continuously decreases as we move away from the high-density regions, meaning that its gradient direction points from the low-density areas with lower ratios toward the high-density areas with higher ratios. Consequently, following this direction will pull the trajectory back from the outer low-probability zones to the more credible high-density regions.

![gradient ratio](/images/posts/2025-07-03-blog-post/gradient_ration.jpg)


The more critical implication of this conclusion is that: this driving force originates from the difference between a good model and a poor model. This systematic discrepancy in errors creates a driving force directed toward high-density regions, enabling movement along this direction to retreat to high-density areas and avoid outliers.


---

<h1 id="section3.3.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.3.1 Reformulation: Guidance Between Two Models of Different Quality </h1>



The essence of CFG lies in the **systematic difference** between a good model and a poor model, rather than the difference between the conditional and unconditional distributions. The conditional and unconditional distributions are merely one way to implement a good model and a poor model.


Let $p_{\text{good}}(x)$ denote a **strong (good)** model and $p_{\text{bad}}(x)$ a **weak (bad)** one. AutoGuidance generalizes the CFG update as:

$$
\nabla_x \log p_{\text{total}}(x)
= \nabla_x \log p_{\text{good}}(x) + (w-1)\big(\nabla_x \log p_{\text{good}}(x) - \nabla_x \log p_{\text{bad}}(x)\big).
$$

Here, $p_{\text{bad}}$ no longer represents an unconditional distribution but any weaker estimator of the same target domain—such as:

- a smaller model (reduced parameters or layers),
- an earlier checkpoint (fewer training steps),
- or a dropout/noisy variant of the main network.

The resulting **guidance vector field**

$$
\Delta(x) = \nabla_x \log p_{\text{good}}(x) - \nabla_x \log p_{\text{bad}}(x)
$$

retains the same geometric meaning as in CFG: it points from a lower-quality model’s density ridge toward the high-quality model’s density ridge, effectively correcting the generative flow in the direction of improved fidelity.

---

<h1 id="section3.3.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.3.2 Bad Model Rather Than Unconditional Branch</h1>

Autoguidance argue that using a **weaker** version of the main conditional denoiser as the bad model is more appropriate than using an unconditional model for several theoretical and empirical reasons:


1. **Task Consistency.** 
  
   The unconditional model in CFG solves a **different task** with conditional model. As a result, its score field $\nabla_x \log p(x)$ differs not only in **quality** but also in **semantics** from that of the conditional branch $\nabla_x \log p(x\mid c)$. This **task discrepancy** leads to mismatched gradients: when the two score fields are combined, the sampling trajectory can **overshoot** or **drift** from the true conditional manifold, producing oversimplified or skewed compositions.

   In contrast, AutoGuidance uses a **weaker version** of the same conditional model, trained with **identical conditioning and data distribution**, only with limited capacity or training time. Thus, the two denoisers differ purely in **quality**, not in **task**. This ensures their gradients are **aligned in direction but different in magnitude**, providing a clean geometric correction.
  
2. **Decoupling Quality from Alignment.** 

   An important insight of autoguidance is that CFG itself achieves **higher quality** compared to direct conditional sampling, as it effectively removes outliers in low-density regions. This allows CFG to be primarily responsible for quality improvement during design, while conditional matching and alignment can be optimized by the good model itself.
  
3. **The "Bad Model" Has More Relevant Failure Modes.**
  
   A weaker conditional model tends to make similar errors in the same regions, but more severely. This correlation of error patterns is crucial: when both models err in the same areas, their difference vector effectively estimates the direction of improvement.

   The unconditional model, by contrast, does not share the same error structure, since it was never trained to model $p(x\mid c)$ at all. Hence, its errors are largely orthogonal to the conditional model’s, leading to noisy or even misleading guidance signals


4. Applicability to Unconditional Generation
  
   This is a direct and powerful advantage. CFG is, by definition, inapplicable to unconditional generation tasks because there is no "conditional" and "unconditional" distinction to be made. Autoguidance, however, is condition-agnostic. It can be naturally applied to unconditional models by simply training a weaker unconditional model to guide the main one. 



---

<h1 id="section3.4" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.4  CFG++: Interpolation rather than Extrapolation</h1>
...
$$
\begin{align}
& c_i = \text{“a photo of }\tau\text{ dog”},\\[10pt]
& c_i' = \text{“a portrait of }\tau\text{ person”},\\[10pt]
