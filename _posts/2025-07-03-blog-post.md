...
$$
\mathcal{L}=\mathbb{E}{x_0,t,\epsilon},\mathbb{E}{m_k}
\big|\epsilon-\epsilon_\theta(x_t,t;\tilde c_1,\ldots,\tilde c_{|\mathcal K|})\big|_2^2,
$$

where each $\tilde c_k$ equals $c_k$ or $\varnothing^{(k)}$ depending on the dropout mask $m_k$. This ensures the model can handle any subset of available conditions at test time.
...
$$
\epsilon_{\text{total}}
=\epsilon_u + \sum_{k\in\mathcal P}w_k,d_k

\sum_{m\in\mathcal N}w_m^-!d_m,
$$
...
$$
\frac{dx_t}{dt}=f_\theta(x_t,t)+\lambda(t)\,g(x_t,\,t,\,\mathcal C)
$$

where $g(x_t,\,t,\,\mathcal C)$ is guidance direction of multiple conditions set $\mathcal C$
