...
Instead of conditioning generation on textual semantics, SDEdit controls the **structural layout** of the image through the **initial state** of the reverse diffusion process.
Given an input image $x_0^{\text{in}}$ (e.g., a line drawing, depth map, or degraded photo), SDEdit first **adds Gaussian noise** until a moderate noise level $t_0$ and then **runs the reverse SDE/ODE** from $t_0 \to 0$ using a pretrained denoiser.
The result $x_0^{\text{out}}$ retains the large-scale composition of $x_0^{\text{in}}$ while restoring fine details and realism 
...
Let the pretrained score function $$s_\theta(x_t,t)\approx\nabla_{x_t}\log p_t(x_t)$$ describe the learned data manifold. SDEdit initializes the reverse process at an intermediate noise level:
...
From $t_0 \to 0$, it integrates either **VE-SDE:**

$$
  x\!\leftarrow\!x+\varepsilon^2s_\theta(x,t)+\varepsilon z,\qquad
  \varepsilon=\sqrt{\sigma^2(t)-\sigma^2(t-\Delta t)},; z!\sim!\mathcal N(0,I),
$$

or **VP-SDE:**
  $$
  x_{n-1}=\tfrac{1}{\sqrt{1-\beta(t_n)\Delta t}}
  !\left(x_n+\beta(t_n)\Delta t,s_\theta(x_n,t_n)\right)
  +\sqrt{\beta(t_n)\Delta t},z_n .
  $$
...
This variant performs **whole-image refinement**, appropriate for sketch-to-image synthesis, global stylization, or structure-preserving restoration.
Because the process begins at an intermediate diffusion state rather than pure noise, it **preserves large-scale geometry** while letting the model hallucinate plausible fine details.
...
- **Step 1: Initialization.** Inject noise only after restoring the frozen region:
   $$
   x\leftarrow (1-\Omega)\odot x_0+\Omega\odot x^{(g)}+\sigma(t_0)z .
   $$

- **Step 2: Iterative reverse integration.** At every step (t!\to!t-\Delta t):

  - On editable pixels $(\Omega)$: apply the normal reverse SDE/ODE update as in Algorithms 2 or 4.
  - On frozen pixels $(1-\Omega)$: suppress the score term and decay the noise variance $\sigma^2(t)\downarrow0$, so that
    
	$$
     (1-\Omega)\odot x_t \to (1-\Omega)\odot x_0 \quad \text{as } t!\to!0.
    $$
