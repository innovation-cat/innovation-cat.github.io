...
where $Q$ come from image space, whereas $K$ and $V$ come from text space. The **attention map** $A=\mathrm{softmax}(QK^\top/\sqrt{d_k})$ determines **“where each token looks”** — i.e., spatial locations in the image associated with that token.
