...
- **Stage II: Null-Text Optimization.** Because DDIM inversion relies on the model’s **imperfect noise prediction** and a **discretized deterministic ODE**, the forward (inversion) and backward (sampling) trajectories are not mathematically identical. Small **approximation errors in $\epsilon_\theta(x_t,t,c)$**, **guidance nonlinearity (CFG)**, and **numerical discretization** accumulate over timesteps, causing the reconstructed image $\hat{x}_0$ to deviate slightly from the original $x_0$.
  ...
  $$
  \widehat x_{t-1}\,\leftarrow\,\operatorname{DDIMstep}(x_t;\mathcal O_t) \;=\; \sqrt{\bar\alpha_{t-1}}\;\widehat x_0^{(t)} \;+\; \sqrt{1-\bar\alpha_{t-1}}\;\epsilon_{\text{total}}(x_t,t)
  $$
  ...
  \boxed{
  \begin{aligned}
  \widehat\epsilon_t(\mathcal O_t) &;=; \epsilon^{\text{CFG}}*\theta(x_t,t;c,\mathcal O_t,w),\
  \widehat x_0^{(t)}(\mathcal O_t) &;=; \frac{x_t - \sqrt{1-\bar\alpha_t},\widehat\epsilon_t(\mathcal O_t)}{\sqrt{\bar\alpha_t}},\
  \operatorname{DDIMstep}(x_t;\mathcal O_t) &;=; \sqrt{\bar\alpha*{t-1}};\widehat x_0^{(t)}(\mathcal O_t) ;+; \sqrt{1-\bar\alpha_{t-1}};\widehat\epsilon_t(\mathcal O_t).
  \end{aligned}}
  $$

  **Per-step reconstruction loss.** NTI enforces the **reference-path consistency**:
  
  $$
  \mathcal L_{\text{rec}}({\mathcal O_t}) ;=; \sum_{t=1}^{T} \Big|,\operatorname{DDIMstep}(x_t^\star;\mathcal O_t) ;-; x_{t-1}^\star \Big|_2^{2}.
  $$

  **Temporal smoothness (regularization).** To avoid step-wise jitter and improve stability,
  $$
  \mathcal L_{\text{smooth}}({\mathcal O_t}) ;=; \lambda \sum_{t=2}^{T}\big|,\mathcal O_t - \mathcal O_{t-1},\big|_2^{2}.
  $$

  **(Optional) Pixel-/Perceptual-space nudges.** After decoding ( \widehat x_0 !=! \text{VAE.decode}(x_0^{\wedge}) ) from the last step, one may add

  $$
  \mathcal L_{\text{img}} ;=; \mu_1,|\widehat x_0 - x_0|_1 ;+; \mu_2,\text{LPIPS}(\widehat x_0, x_0),
  $$

  kept small to avoid leaking information outside the diffusion dynamics. **Total objective and optimization.**
