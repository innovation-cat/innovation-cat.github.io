...
$$\mathcal{K}={\text{text},\text{layout},\text{seg},\text{depth},\text{normal},\text{ref}}$$ 

For channel $k\in\mathcal{K}$, denote its input by $c^{(k)}$ and its **channel-specific null** by $\varnothing^{(k)}$. The (unguided) probability-flow ODE is
[
...
with (\mathcal{C}) the set of active channel conditions (positive and/or negative).
