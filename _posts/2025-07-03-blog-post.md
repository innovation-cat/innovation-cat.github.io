...
$$
\tilde{\epsilon}_{\text{guide}}
=\epsilon_\theta(x_t,\varnothing,t) + w(t)\,(\epsilon_\theta(x_t,c,t)\;-\epsilon_\theta(x_t,\varnothing,t))
$$
...
In practice, however, **the conditioning signal is rarely singular**. Real tasks routinely combine **heterogeneous** constraints—for example, text semantics, spatial layout, depth or segmentation maps, brand/style codes, safety filters, and even hard constraints such as color gamut or watermark removal. Moreover, conditions may be **positive** (encouraging the presence of attributes) or **negative** (suppressing undesired content). These signals can be **statistically independent**, **weakly dependent**, or **directly conflicting** (e.g., a style that clashes with layout geometry).

Classical guidance mechanisms (e.g., classifier guidance and classifier-free guidance, CFG) are usually presented for a single condition. Extending them **systematically** to multiple, potentially competing conditions requires a careful treatment of (i) the geometry of guidance directions, (ii) the calibration and scheduling of guidance strengths across noise levels, and (iii) the resolution of interference when directions disagree.

This section formalizes **multi-condition guidance** as a principled augmentation of the probability-flow dynamics and lays the foundation for independent as well as dependent/antagonistic condition sets.
...
In **(\epsilon)-prediction** (VP-style forward process), we know the standard CFG identity yields

[
\nabla_{x_t}\log p_t(c_k\mid x_t) = \nabla*{x_t}\log p(x_t\mid c)-\nabla_{x_t}\log p(x_t);\approx;
...
+\sum_{k=1}^K w_k(t)\big(\epsilon_{c_k}-\epsilon_u\big)
-\sum_{m=1}^M w^-_m(t)\big(\epsilon_{c^-_m}-\epsilon_u\big)
}
\tag{7.1}
$$

and plug (\epsilon*{\text{guid}}) into any sampler (DDIM, Heun/RK, DPM-Solver, etc.). The same construction holds for **(x_0)-** and **(v)-prediction** by replacing (\epsilon) with the model’s native output and using **differences in that parameterization**.
