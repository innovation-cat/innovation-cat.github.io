  ...
  <div class="toc-content">
    <ul class="toc-list">
      ...
      <li class="toc-item-main">
        <a href="#sectionPartI" class="toc-link-main">Part I — Foundation and Preliminary</a>
      ...
      <li class="toc-item-main">
        <a href="#sectionPartII" class="toc-link-main">Part II — Stabilization in U-Net Architectures</a>   
        <ul class="toc-sublist">
          <li><a href="#section4">4. What Goes Wrong with Naïve Skip Connections?</a></li>
      ...
      <li class="toc-item-main">
        <a href="#sectionPartIII" class="toc-link-main">Part III — Stabilization in DiT Architectures</a> 
        <ul class="toc-sublist">
      ...
      <li class="toc-item-main">
        <a href="#sectionPartIV" class="toc-link-main">Part IV — Conclusion</a>     
      </li>


      <li class="toc-item-main">
        <a href="#sectionV" class="toc-link-main">Part V — References</a>
      </li>
...
In our previous [article](https://innovation-cat.github.io/posts/2024/12/diffusion-model-training/), we examined how diffusion training stability can be improved from the perspective of **loss functions and optimization strategies**. Yet, stability is not only a matter of training schedules or objectives — the **network architecture itself** plays a decisive role in determining whether gradients vanish, explode, or propagate smoothly across noise levels. 
...
<h1 id="section1.3" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">1.3 Architecture–Noise Schedule Coupling</h1>
...
<div class="modern-deep-card">
  <p class="card-content">
    <span class="label-text">The Core Problem:</span> The Global and Local Dilemma. A single diffusion model faces a challenging balancing act. During the denoising process, it must simultaneously:
    
    <ul>
        <li><strong>Establish Global Coherence:</strong> Correctly interpret the prompt to create a plausible composition, with proper object placement, anatomy, lighting, and color harmony. This is a <strong>low-frequency</strong> task.</li>
        <li><strong>Render Fine Details:</strong> Generate intricate, high-frequency textures like skin pores, fabric weaves, hair strands, and sharp edges. This is a <strong>high-frequency</strong> task.</li>
    </ul>
...
<h1 id="sectionPartII" style="color: #1E3A8A; font-size: 30px; font-weight: bold; text-decoration: underline;">Part II — Stabilization in U-Net Architectures</h1>



In diffusion / flow-matching U-Nets, the long skip pathway (encoder → decoder at the same resolution) is simultaneously a **quality driver** and a **stability risk**. It is a quality driver because it carries high-resolution spatial detail; it is a stability risk because it can become an **uncontrolled bypass** that injects noise, overwhelms semantic features, and amplifies forward/backward oscillations.
...
In the classic implementations of DDPM, a typical “plain” U-Net skip can be written as either addition or concat.
...
- <strong style="color: blue;">Amplitude mismatch.</strong> This mismatch will lead to two consequences: skip dominance and variance coupling
  ...
  **Skip Dominance.** Shallow encoder features can dominate decoder semantics:
  ...
  so the model effectively learns a “shortcut” that bypasses deeper semantic computation: it can reduce the loss mainly through the skip branch, so gradient descent allocates most learning signal there and “starves” the decoder. 
  ...
  $$
  \underbrace{\nabla_{d'}\mathcal{L}=\frac{\partial \mathcal{L}}{\partial d'} = W_d^\top \frac{\partial \mathcal{L}}{\partial y}}_{\text{Decoder gradient flow}},\quad \underbrace{\nabla_{s'}\mathcal{L}=\frac{\partial \mathcal{L}}{\partial s'} = W_s^\top \frac{\partial \mathcal{L}}{\partial y}}_{\text{Skip gradient flow}}
  $$
...
- <strong style="color: blue;">Representation mismatch.</strong> Skip features $s$ and decoder features $d$ typically live at different abstraction levels:
  ...
  * **decoder feature $d$**: global layout, semantics, denoising backbone (lower-frequency dominated)
  ...
  Naïve fusion forces incompatible subspaces to couple directly. The resulting training signal contains “conflict gradients”: updates that help one stream may harm the other, slowing convergence and making optimization noisy.
...
- <strong style="color: blue;">Conditioning mismatch.</strong> conditions should decide routing, not only content. In modern diffusion systems, conditions $z$ (text, control maps, style, reference images) should influence:
  ...
  * **how** to fuse (Fuse)
  ...
  If the skip pathway is not condition-aware, the model is forced to “encode everything” into bottleneck or cross-attention only, often producing brittle control and unstable training when multiple conditions are present.
...
with increasing granularity (scalar → channel gate → spatial mask → full gate).
...
Skip weighting is best understood as a compact “control system” for U-Net routing. In unified notation,
...
<div style="display: block; margin: 0 auto; text-align: center;">
  ...
  <div style="margin-top: 8px; font-size: 16px; color: #666; font-style: Times New Roman;">
     Figure 6: A unified view of skip weights / gates as an input-to-output mapping with broadcastable granularity.
  </div>
</div>
...
<ul class="hanging-list">
  <li><strong>Scalar gate</strong>: $w\in\mathbb{R}$ &nbsp;&nbsp;→ broadcast to $(1,1,1,1)$</li>
  <li><strong>Channel gate</strong>: $w\in\mathbb{R}^{C_\ell}$ &nbsp;&nbsp;→ reshape to $(1,C_\ell,1,1)$</li>
  <li><strong>Spatial gate</strong>: $w\in\mathbb{R}^{H_\ell\times W_\ell}$ &nbsp;&nbsp;→ reshape to $(1,1,H_\ell,W_\ell)$</li>
  <li><strong>Full gate</strong>: $w\in\mathbb{R}^{C_\ell\times H_\ell\times W_\ell}$ &nbsp;&nbsp;→ reshape to $(1,C_\ell,H_\ell,W_\ell)$</li>
</ul>

This is why many “different” skip weighting methods are actually the same recipe: they differ mainly in <em>what goes into $u$</em> and <em>what shape is emitted</em>.
...
<strong style="color: blue;">Step V. Verify with diagnostics: “where does the learning signal go?”</strong>  
...
In the unified formulation, the entire “weight / gate design” problem reduces to a mapping:
...
This is flexible and condition-adaptive, but it introduces a new failure mode: the gate itself can become high-gain and destabilize long-skip dynamics. Hence, the key engineering is not “which MLP,” but <em>how</em> to constrain $\eta(\cdot)$ and where to apply the gate (granularity/layers/timesteps).
...
- <strong style="color: blue;">A. Variance / energy preserving schedules.</strong>   For additive fusions, repeated merges can drift feature norms because 
  ...
  This turns $(w_d,w_s)$ into “direction” parameters rather than “gain” parameters, preventing uncontrolled amplification even when weights change across $(t,\ell)$. This idea appears naturally in the variance-aware merge variant
...
- <strong style="color: blue;">B. Layer-wise (hierarchy-aware) schedules.</strong>   A large portion of stability issues can be addressed by prescribing a per-level coefficient $\kappa_\ell$ (or $\alpha_\ell$):
  ...
  <li><strong>Temperature-controlled sigmoid</strong>: $\kappa_\ell=\sigma\big(a(\ell-b)\big)$, useful when you want a smooth “transition depth.”</li>
  </ul>

  This family is conceptually aligned with “explicitly scale down long skip edges” (e.g., $\kappa_\ell$ or $\kappa_\ell(t)$) emphasized by the ScaleLong-style viewpoint. 
...
- <strong style="color: blue;">C. Timestep / SNR-aware schedules.</strong>  Because diffusion spans low-SNR (very noisy) to high-SNR (nearly clean) regimes, a constant skip strength cannot be optimal across time. 
  ...
  **Design principle.** A monotone schedule is often sufficient: **suppress** skip in low SNR (avoid noise leakage), and **enable** skip in high SNR (recover detail)
  ...
  <ul class="hanging-list">
  <li><strong>Sigmoid in log-SNR</strong>: with $(\beta,\tau)$ controlling sharpness and the “turn-on” point.
...
So the “granularity choice” answers one question: **what axes are allowed to vary?**
...
- <strong style="color: blue;">Scalar Gates.</strong> A scalar gate is the most stable and cheapest option: it uniformly rescales the entire skip stream. This includes layer-wise coefficients $$\alpha_\ell$$ and time-dependent scalars $$w_s(t,\ell)$$ . Scalar gates are especially attractive when the primary goal is **stability** (preventing skip dominance, bounding feature norms, reducing gradient oscillation). When the fusion is additive, variance-aware rescaling (normalizing by $$\sqrt{1+\alpha_\ell^2}$$) keeps magnitudes bounded , and also mitigates the “variance drift” issue across repeated merges .
...
- <strong style="color: blue;">Channel Gates.</strong> Channel-wise gating is often a sweet spot: it is substantially more expressive than a scalar, while remaining much more stable than spatial/full masks :
  ...
  **Why it works well in diffusion:** channels tend to specialize (detail vs structure), so a channel gate can suppress “noisy detail channels” at low SNR without discarding all skip information . This directly targets timestep mismatch and reduces conflict between streams.
...
- <strong style="color: blue;">Frequency-selective transforms:</strong> Frequency-selective transforms implement $$\phi_s$$ by filtering or re-scaling specific spectral bands of the skip feature $$s$$. 
  ...
  Concretely, $$\beta_{\ell}(\omega)$$ is a piecewise mask that downscales a selected frequency region by a factor $$s_\ell<1$$ (and keeps the rest unchanged). Thus FreeU implements “frequency-selective attenuation” on the skip stream without changing the backbone parameters. 
...
- <strong style="color: blue;">Attention fusion:</strong> A major source of instability in skip pathways is the **semantic gap**: encoder skip features can be “too local / too detailed”, while decoder features become increasingly semantic. Attention-based fusion makes the merge *selective* rather than unconditional.
...
Without $z$-aware routing, the model must force all control into a single bottleneck/cross-attention interface, which often leads to: brittle composition of multiple conditions, unstable gradients when conditions compete, and unpredictable “over-control” or “under-control”
...
- <strong style="color: blue;">Stability principles for routing interfaces.</strong>  
  A routing interface is “stable” when it preserves the pretrained backbone behavior at initialization and limits gradient interference across pathways. Common patterns:

  <ul class="hanging-list">
    <li><strong>Identity-preserving initialization:</strong> start from “no-op” routing so the base U-Net behavior is unchanged (reduces early oscillations).</li>
...
- <strong style="color: blue;">ControlNet: stable conditional injection via zero-initialized connectors.</strong> [^controlnet]  
  ...
  so the base model’s behavior is preserved initially, and optimization learns a *residual route* rather than rewriting the skip pathway.
  ...
  and in practice it is often paired with an explicit routing scale (a special case of $w_s(t,\ell,z)$) to avoid “control dominance”.
...
- <strong style="color: blue;">SCEdit-style controllable skip tuners: editing via localized skip routing.</strong> [^scedit]  
  ...
  where $T_\ell$ is a lightweight learnable module (few parameters) and $g_{\ell}(t,z)$ is a routing gate that decides **where/when** edits should take effect. This makes “editing strength” an explicit part of $w_s(t,\ell,z)$ and avoids uncontrolled global drift.

  In a stability-centric reading, SCEdit is not “just controllability”: it is a concrete way to implement **condition-aware routing** that is (i) identity-friendly at initialization, and (ii) restricted to selected layers/timesteps to reduce interference with the base semantic computation.
...
- <strong style="color: blue;">Training-free routing: SkipInject / style injection as $z$-conditioned transforms.</strong> [^skipinject] [^sid]  
  ...
  with mixing strengths $\lambda_{\ell,t}$ acting as explicit routing schedules. From a stability viewpoint, these methods bypass optimization entirely and instead exploit the fact that skip pathways already encode rich, spatially aligned structure that can be “re-routed” at inference time.

---

In summary, section [5](#section5) to section [7](#section7) gave us the “local operator toolkit” for stabilizing skip fusion. This chapter adds the systems-level lesson: **conditions should shape the routing policy of skip pathways**. ControlNet, T2I-Adapter, and SCEdit illustrate three stable interface patterns—branch injection, lightweight aligned adapters, and localized skip tuners—that implement $z$-aware $w/\phi/\mathrm{Fuse}$ while preserving identity at initialization and limiting gradient interference. This perspective makes “controllability modules” directly relevant to architectural stability: they are not only about adding more conditions, but about adding them *without destabilizing* the skip operator.
...
- <strong style="color: blue;">Up/downsampling choices and aliasing control.</strong>
  ...
  The choice of up/downsampling (strided conv vs pooling vs resampling filters) affects the spectral content of features. If downsampling introduces aliasing, skip features can carry “incoherent high-frequency artifacts” that the decoder cannot reconcile stably. Anti-aliased resampling or carefully designed strided convs can improve stability, especially when combined with frequency-aware (\phi_s).
...
<h1 id="sectionPartIII" style="color: #1E3A8A; font-size: 30px; font-weight: bold; text-decoration: underline;">Part III — Stabilization in DiT Architectures</h1>




In diffusion / flow-matching **DiTs**, there are no long skip highways like U-Nets; instead, the backbone is a deep stack of homogeneous Transformer blocks, so the stability pressure is concentrated on **per-block numerical stability under deep stacking**. Consistent with the “unified operator” style used to open Part II, we describe each DiT block as a **residual + multi-branch update operator**:
...
where we treat **residual scaling** and **gating** as the primary “amplitude regulator” of $$\Delta_\ell$$. This view is consistent with the DiT training philosophy that emphasizes identity-preserving residual behavior at initialization and controlled deviations thereafter [^dit].
...
A clean way to make residual increments “scale-safe” is to insert an explicit **depth scaling**:
...
- **Depth-dependent scaling**: $$\beta_\ell \propto 1/\sqrt{L}$$ (or slowly varying with $$\ell$$).  
  This matches the intuition that summing $$L$$ increments of comparable scale should not cause $$\|h_L\|$$ to grow as $$\sqrt{L}$$ unchecked; related “depth-safe” residual parameterizations have been studied in very-deep Transformer scaling work [^deepnet].
...
- **Small learnable scaling (LayerScale-style)**: initialize a per-channel scale to a tiny value (e.g., $$10^{-6}$$ to $$10^{-4}$$), then learn it [^cait].  
  This starts the network extremely close to identity and gradually “opens up” capacity as training stabilizes.

These are **architecture-level** stabilizers: they do not depend on $$t$$ or $$z$$, and they primarily control *depth accumulation risk*.

---

<h1 id="section10.3" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">10.3 Identity-preserving initialization: Zero-init and “start-as-identity” gates</h1>
...
<h1 id="section10.4" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">10.4 Learned gates as “dynamic amplitude controllers”</h1>
...
In DiTs, modulation is not merely “conditioning injection”—it is the **global controller** that shapes the statistics of the signal entering each operator. In our notation, modulation is encapsulated by
...
In practice, this acts like an “automatic warm-up” at the architectural level: the network begins in a stable regime without requiring extremely conservative optimization hyperparameters [^dit].
...
- If $$\|Q\|$$ or $$\|K\|$$ grows, logits $$QK^\top/\sqrt{d_h}$$ can become large, making softmax nearly one-hot → **gradient starvation** and brittle routing.
...
From our unified lens, these are not “extra tricks”—they are ways of making $$F_\ell^{(\text{attn})}$$ better behaved so the residual update $$\Delta_\ell$$ remains well-conditioned.
...
In diffusion transformers, gated FFNs are particularly compatible with the “residual amplitude control” story: gating makes the effective residual update more selective and less likely to explode on hard/noisy timesteps.
...
This motivates thinking in the unified equation rather than treating attention/MLP “in isolation.”
...
Many text-to-image DiT variants explicitly add cross-attention modules into DiT-like blocks for conditioning (e.g., PixArt-α) [^pixart].
...
A particularly influential design is to keep **separate weights for each modality stream** while sharing the attention interaction, enabling both “own-space processing” and cross-modal exchange [^sd3_mmdit].

**Stability profile:**

- improves text understanding and compositional binding by allowing image ↔ text token interaction at every layer,
...
<h1 id="sectionPartIV" style="color: #1E3A8A; font-size: 30px; font-weight: bold; text-decoration: underline;">Part IV — Conclusion</h1>
...
<h1 id="sectionPartV" style="color: #1E3A8A; font-size: 30px; font-weight: bold; text-decoration: underline;">Part V — References</h1>
...
[^score_sde_code]: Song Y. score_sde_pytorch (official implementation). GitHub repository, 2020–2021. (Contains the widely used “skip_rescale” practice.)

[^improved_diffusion_code]: OpenAI. improved-diffusion (reference U-Net implementation). GitHub repository, 2021. (Includes common “zero_module / zero-init residual branch” practice.)
...
[^pixart]: Chen J, Yu J, Ge C, et al. PixArt-α: Fast Training of Diffusion Transformer for Photorealistic Text-to-Image Synthesis[J]. arXiv preprint arXiv:2310.00426, 2023.
