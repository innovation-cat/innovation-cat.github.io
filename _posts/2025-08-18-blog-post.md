...
\Longrightarrow \quad & \int_t\int_{x_t} p_t(x_t) \times \| v_{\theta}(t, x_t) \| \times \big( \int_z\| \underbrace_{v_t(x_t|z) \| \times \frac{p_t(x_t \mid z)p_{\text data}(z)}{p(x_t)}dz \big)}_{\text{Eq. 33}} dx_tdt\label{eq:27}   \\[10pt]
