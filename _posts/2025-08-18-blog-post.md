...
\Longrightarrow \quad & \int_t\int_{x_t} p_t(x_t) \times \| v_{\theta}(t, x_t) \| \times \big( \int_z\| \underbrace_{v_t(x_t|z) \| \times \frac{p_t(x_t \mid z)p_{\text data}(z)}{p(x_t)}}_{\text{Eq. 33}}dz \big) dx_tdt\label{eq:27}   \\[10pt]
...
\Longrightarrow 
\quad & \mathcal L_{\text{FM}}(\theta)=\mathbb E_{\scriptscriptstyle  t\sim U(0,1), z\sim p_{\text data}(z), x_t\sim p_t(x_t|z)}\big(\| v_{\theta}(t, x_t) \|^2 -2\| v_{\theta}(t, x_t) \| \| u_t(x_t) \| + \| u_t(x_t) \|^2 \big) \quad  \\[10pt] 
...
Where $C$ is a constant, which means that minimizing $\mathcal L_{\text{FM}}$ is equal to minimizing $\mathcal L_{\text{CFM}}$ with represpect to $\theta$.
