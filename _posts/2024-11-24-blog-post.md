...
Let $t$ be the current time and $t-1$ the next (earlier) time, a **backward** Euler–Maruyama step for the reverse SDE reads

$$
x_{t-1} \;\approx\; x_t \;+\; 
\Big(\frac{1}{2}\beta(t)x_t + \beta(t)s_{\theta}(x_t, t)\Big)\,
\;+\;\sqrt{\beta(t)}\;z,\quad z\sim\mathcal N(0,I),
$$

where we used that moving **backward** in $t$ flips the sign of $dt$ (hence the “$+$” above). Substituting $s_\theta= -\varepsilon_\theta/\sqrt{1-\bar\alpha_t}$ gives

$$
x_{t-1} \;\approx\; x_t \;+\; 
\Big(\frac{1}{2}\beta(t)x_t - \frac{\beta(t)}{1-\bar\alpha_t}\epsilon_{\theta}(x_t, t)\Big)\,
\;+\;\sqrt{\beta(t)}\;z,\quad z\sim\mathcal N(0,I),
$$
