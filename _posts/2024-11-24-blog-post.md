...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
    ...
    <li><a href="#section3">3. Reverse Dynamics: Reverse-Time SDE and PF-ODE</a></li>
...
Let $t$ be the current time and $t-1$ the next (earlier) time, set $\Delta t$ is step size. a **backward** Euler–Maruyama step for the reverse SDE reads
...
$$
x_{t-1}  = \; \left( 1 \;+\; \frac{1}{2}\beta_t \right)(x_t - \frac{\beta_t}{1-\bar\alpha_t}\varepsilon_{\theta}(x_t, t))\,\;+\;\sqrt{\beta_t}\;z
$$

Now compare this with **DDPM’s discrete reverse mean** (Eq. (11) in the DDPM paper):
...
The first-order Taylor expansion of the function $ \sqrt{\alpha_t} = \sqrt{1-\beta_t} $ when $ \beta_t $ is very small, approaching 0, is given by:

$$
\frac{1}{\sqrt{\alpha_t}} = (1-\beta(t)\Delta t)^{-1/2}
\;\approx\; 1+\tfrac12\beta_k\Delta t + O({\Delta t}^2)
\qquad
1-\alpha_k=\beta_k\Delta t.
$$
