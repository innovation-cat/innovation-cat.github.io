...
=\; \underbrace{1+\frac{1}{2}\beta(t)h+O(h^2)}_{\text{first-order Taylor expansion}},
$$
...
$$
\begin{align}
\sqrt{1-\bar\alpha(s)} & =\sqrt{1-\bar\alpha(t-h)} \\[10pt]
& = \sqrt{1-\bar\alpha(t)} - \left( \sqrt{1-\bar\alpha(t)} \right) ^{'}\,h + O(h^2) \\[10pt]
& = \sqrt{1-\bar\alpha(t)} + \frac{\beta(t)\bar\alpha(t)}{2\sqrt{1-\bar\alpha(t)}}h + O(h^2)
\end{align}
$$
