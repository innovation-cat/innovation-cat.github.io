...
where we used that moving **backward** in $t$ flips the sign of $dt$ (hence the “$+$” above). When step size $\Delta t$ is small, we have $\beta_t \approx \beta(t)\,\Delta t$, Substituting $s_\theta= -\varepsilon_\theta/\sqrt{1-\bar\alpha_t}$ gives
...
& \approx\; \left( 1 \;+\; \frac{1}{2}\beta_t \right)x_t +  \left( 1 \;+\; \frac{1}{2}\beta_t \right)\beta_ts_{\theta}(x_t, t)\,\;+\;\sqrt{\beta_t}\;z
\end{align}
$$
