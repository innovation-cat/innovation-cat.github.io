	...
	<li><a href="#section2">2. Evolution of Diffusion Architectures</a>
		<ul>
			...
			<li><a href="#section2.3">2.3 Latent U-Net (Stable Diffusion, SDXL)</a></li>
				<ul>
					<li><a href="#section2.3.3">2.3.3 Evolution to SDXL: Refining the Latent U-Net Formula</a></li>
				</ul>
...
<h1 id="section1.1" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">1.1 Gradient Flow, Conditioning, and Stability</h1>
...
<h1 id="section1.2" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">1.2 Balancing Capacity vs. Robustness</h1>
...
<h1 id="section1.3" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">1.3 Architecture–Noise Schedule Coupling</h1>
...
<h1 id="section2.1" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">2.1 Classical U-Net Foundations</h1>
...
<h1 id="section2.1.1" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.1.1 Encoder: From Local Features to Global Context</h1>
...
<h1 id="section2.1.2" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.1.2 Bottleneck: Abstract Representation</h1>
...
<h1 id="section2.1.3" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.1.3 Decoder: Reconstructing Local Detail under Global Guidance</h1>
...
<h1 id="section2.1.4" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.1.4 Timestep Embedding and Conditioning</h1>
...
<h1 id="section2.1.5" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.1.5 Why U-Net Works Well for Diffusion</h1>
...
<h1 id="section2.2" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">2.2 ADM Improvements (Ablated Diffusion Models)</h1>
...
<h1 id="section2.2.1" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.1 Scaling the U-Net: Wider Channels and Deeper Residual Blocks</h1>
...
<h1 id="section2.2.2" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.2 Multi-Resolution Self-Attention</h1>
...
<h1 id="section2.2.3" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.3 Conditioning via Adaptive Group Normalization (AdaGN)</h1>
...
<h1 id="section2.2.4" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.4  BigGAN-inspired Residual Blocks for Up/Downsampling</h1>
...
<h1 id="section2.2.4" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.5 Rescaling of Residual Connections</h1>
...
<h1 id="section2.2.6" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.2.6 Why ADM Matters for Stability</h1>
...
<h1 id="section2.3" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">2.3 Latent U-Net: The Efficiency Revolution with Stable Diffusion and SDXL</h1>
...
<h1 id="section2.3.1" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.1 The Core Idea: Diffusion in a Compressed Latent Space</h1>
...
<h1 id="section2.3.2" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.2 Architectural Breakdown of the Latent U-Net</h1>
...
<h1 id="section2.3.3" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.3.3 Evolution to SDXL: Refining the Latent U-Net Formula</h1>
...
<h1 id="section2.4" style="color: #1E3A8A; font-size: 25px; font-weight: bold; text-decoration: underline;">2.4 Transformer-based Designs</h1>

While latent U-Nets (Section [2.3](#section2.3)) significantly improved efficiency and multimodal conditioning, they still retained convolutional inductive biases and hierarchical skip pathways. The next stage in the evolution of diffusion architectures explores whether **Transformers** — with their global receptive fields, flexible tokenization, and scaling laws — can serve as the *primary backbone* for diffusion models. This marks a decisive shift from **CNN-dominated designs** to **Transformer-native backbones**, most notably exemplified by the **Diffusion Transformer (DiT)** family and its successors.

---

<h1 id="section2.4.1" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.1 Motivation: Beyond Convolutional Inductive Bias</h1>

Convolution-based U-Nets provide strong locality and translation invariance, but they impose rigid inductive biases:

- **Locality vs. global context**: Convolutions capture local patterns well but require deep hierarchies to model long-range dependencies.
- **Rigid resolution scaling**: U-Nets rely on down/upsampling, which complicates multi-resolution training and often causes instability at very high resolutions.
- **Inefficient scaling laws**: Recent work shows that Transformers scale more predictably with dataset and parameter count, whereas CNNs saturate earlier.
...
<h1 id="section2.4.2" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.2 Diffusion Transformer (DiT)</h1>

The **Diffusion Transformer (DiT)** proposed by Peebles & Xie (2022) is the canonical baseline for pure Transformer backbones:

- **Patch tokenization**: Images (or latent features) are divided into fixed-size patches, linearly projected into token embeddings.  
- **Class and time conditioning**: Instead of AdaGN, DiT uses **Adaptive LayerNorm (AdaLN)**, where timestep and class embeddings are injected via learnable scale and shift parameters.  
- **Global self-attention**: Each Transformer block applies self-attention over all tokens, providing global receptive fields without hierarchical down/upsampling.  
- **Residual scaling for stability**: To prevent gradient explosion in deep stacks, DiT normalizes residual connections and scales updates by 1/√N.  
...
<h1 id="section2.4.3" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.3 Hybrid Designs: Marrying U-Net and Transformer Strengths</h1>

Pure DiT models are powerful but computationally demanding. To balance **efficiency** and **stability**, researchers proposed *hybrid architectures*:

- **U-Net + Transformer Bottleneck**: Convolutional encoder/decoder blocks provide local inductive bias, while Transformer blocks in the bottleneck capture global dependencies.  
- **Cross-attention everywhere**: Latent U-Nets (Section 2.3) already introduced text-to-image cross-attention. Hybrid models generalize this idea by using Transformer blocks at multiple resolutions.  
- **Examples**: SDXL uses U-Net scaffolding but integrates Transformer-like cross-attention modules extensively, while models like **PixArt-α, MMDiT-X** push further toward Transformer dominance.

**Stability note**: Hybrids retain U-Net skip connections, which provide strong gradient flow, while introducing global reasoning through Transformers — a pragmatic compromise.

---

<h1 id="section2.4.4" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.4 Key Architectural Innovations</h1>

Several Transformer-specific techniques were introduced to improve training stability in diffusion:

1. **Adaptive LayerNorm-Zero (AdaLN-Zero)**  
   - Normalization parameters are initialized to zero, allowing conditioning signals (time, class, text) to control feature modulation from the very beginning.  
   - Prevents instability from misaligned conditioning at high noise levels.  

2. **Query-Key Normalization**  
   - Used in large-scale Transformer-based diffusers (e.g., SD3.5, MMDiT-X).  
   - Normalizes Q and K vectors before dot-product attention, reducing variance explosion in deep attention stacks.  

3. **Token merging and compression**  
   - To manage quadratic cost of attention, tokens are merged adaptively (e.g., Token Merging, Token Mixer).  
   - This improves efficiency without sacrificing stability.  

4. **Parallel vs. sequential attention**  
   - Recent designs explore reordering or parallelizing attention and MLP layers to stabilize gradient flow.

---

<h1 id="section2.4.5" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.5 Why Transformers Matter for Diffusion</h1>

The move toward Transformer-based designs signifies a broader shift in generative modeling:

- **Scaling**: Transformers enable billion-scale diffusion models that obey scaling laws more consistently.  
- **Multi-modality**: Text, image, video, and audio tokens can be unified under the same Transformer backbone.  
- **Architectural unification**: With appropriate normalization (AdaLN-Zero, QK-Norm), Transformers can serve as the "universal architecture" across modalities.  
- **Stability trade-off**: While U-Nets are more stable out-of-the-box, Transformers demand careful architectural tricks. The reward is greater flexibility and higher ceiling performance.

---

<h1 id="section2.4.6" style="color: #2563EB; font-size: 23px; font-weight: bold; text-decoration: underline;">2.4.6 Transition to Next Frontiers</h1>

Transformer-based diffusion backbones are not the endpoint but a stepping stone. Current research explores:

- **Video and 3D diffusion** (Section [2.5](#section2.5)), where Transformers naturally handle sequence and spatio-temporal data.  
- **State-space models (S4, Mamba)** as lighter, more stable alternatives to Transformers (see Section [5.5](#section5.5)).  
- **Architecture–schedule co-design** (Section [6](#section6)), where normalization and conditioning layers are explicitly tuned for noise schedules.

Thus, Transformer-based designs represent both a **culmination of scaling trends** and a **launchpad for broader generalization** in generative modeling.
