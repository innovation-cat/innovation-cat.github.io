...
**Conditioning Paradigm Shift: From Global AdaGN to Localized Cross-Attention.** This is the most critical evolution from the ADM architecture. ADM perfected conditioning for **global**, **categorical information (like a single class label)**, whereas Stable Diffusion required a mechanism for sequential, localized, and compositional information (a text prompt).
