...
The idea originates from the Gated Linear Unit (GLU), introduced in language modeling, subsequent work (Shazeer, 2020) proposed several variants that differ only in the choice of activation for the content stream, We summarize the differences between the standard FFN, gated FFN, and its variants in the following table.
