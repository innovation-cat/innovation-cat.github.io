...
### <a id="section2.1.1">Encoder: From Local Features to Global Context</a>
...
### <a id="section2.1.2">Bottleneck: Abstract Representation</a>
...
### <a id="section2.1.3">Decoder: Reconstructing Local Detail under Global Guidance</a>
...
### <a id="section2.1.4">Timestep Embedding and Conditioning</a>
...
### <a id="section2.1.5">Why U-Net Works Well for Diffusion</a>
...
### <a id="section2.2.1">Scaling the U-Net: Wider Channels and Deeper Residual Blocks</a>
...
- **Why it helps:** Deeper and wider blocks improve gradient signal-to-noise in large models, allowing stable optimization even at 256×256 and 512×512 resolutions (ImageNet-scale).
- **Stability mechanism:** Residual pathways and GroupNorm ensure that gradients neither explode nor vanish as they traverse deeper layers, while increased channel width reduces information bottlenecks.

### <a id="section2.2.2">Multi-Resolution Self-Attention</a>
...
This multi-scale approach gives the model a more holistic understanding of the image, preventing structural inconsistencies and improving overall coherence.

### <a id="section2.2.3">Conditioning via Adaptive Group Normalization (AdaGN)</a> 
...
- **Why it helps:** This mechanism integrates class information smoothly, without disrupting feature statistics or exploding channel dimensions.
- **Stability mechanism:** By applying conditioning after normalization, ADM ensures that both timestep and class signals are introduced at controlled magnitudes, reducing the risk of unstable gradient spikes.

### <a id="section2.2.4">Refined Downsampling and Upsampling Operators</a>
...
- **Why it helps:** These choices reduce aliasing and checkerboard artifacts compared to alternatives like pooling or transposed convolutions.
- **Stability mechanism:** Smooth spectral transitions during scale changes prevent oscillations in the optimization dynamics.

### <a id="section2.2.5"> Rescaling of Residual Connections</a>

For very deep networks, it's crucial to maintain well-behaved activations. ADM introduced a simple but effective trick: The output of each residual block was scaled by a constant factor of $\frac{1}{\sqrt{2}}$ before being added back to the skip connection.

This technique helps to balance the variance contribution from the skip connection and the residual branch, preventing the signal from exploding in magnitude as it passes through many layers. This improves training stability for very deep models.

### <a id="section2.2.6"> Why ADM Matters for Stability</a>

Relative to the classical DDPM U-Net, ADM introduced a **set of interlocking improvements**:

* More capacity without destabilizing gradients.
* Attention placed only where it provides value.
* Conditioning injected via adaptive normalization instead of disruptive concatenation.
* Scale changes implemented with numerically stable operators.
* Training practices tuned for high-resolution diffusion.

Together, these refinements established ADM as the **first ImageNet-scale stable diffusion backbone**. It provided a blueprint that later designs — from latent U-Nets to Transformer-based architectures — would inherit: *global reasoning at coarse scales, local fidelity at high resolutions, and conditioning delivered through normalization pathways.*
