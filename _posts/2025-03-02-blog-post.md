...
$y=[y_1, y_2, \dots, y_D]^\top \,\in\,\mathbb{R}^D$, differentiate $y=\tilde x/\sigma$, the **Jacobian** of LN (denoted as $J_{\rm LN}$) with respect to $x$ is
...
$$
J_{\rm LN} \cdot g \;=\; \frac{1}{\sigma}\Big(g-\overline{g}\,\mathbf{1}-\overline{gy}\,y\Big),
\quad \overline{g}=\tfrac{1}{m}\mathbf{1}^\top g,\;\overline{gy}=\tfrac{1}{m}y^\top g.
$$

So LN backprop is equal to orthogonal projection onto $\mathcal{S}$ followed by a scalar scaling $1/\sigma$. 

Similarly, let's analyze Post-LN block (LN **after** the residual sum)
