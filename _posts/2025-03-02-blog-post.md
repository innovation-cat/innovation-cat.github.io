...
<div class="qa-block">
  ...
  <p class="hanging-indent">Diffusion models forced normalization to evolve. Unlike discriminative models, the denoising network operates under <strong>extreme noise regimes</strong>, from nearly clean signals to pure Gaussian noise. This requires the model to <strong>adapt its feature statistics dynamically</strong> depending on the timestep, noise level, or conditioning prompt. Normalization layers became the natural site for this adaptation because:</p>

  <ul class="hanging-list">
    <li><strong>Ubiquity:</strong> every residual block already contains a normalization step, so conditioning can permeate the entire network.</li>
    ...
    <li><strong>Lightweight but global influence:</strong> a small MLP projecting a condition vector can control feature distributions across all layers without altering the convolutional or attention weights directly.</li>
  </ul>

  <p class="hanging-indent">Thus, normalization transitioned into a <strong>controller</strong>: not just stabilizing activations, but embedding semantic and structural conditions into the very statistics of feature maps.</p>
</div>
