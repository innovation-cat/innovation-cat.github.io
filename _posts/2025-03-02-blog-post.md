...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">1️⃣ Conditioning Paradigm Shift: From AdaGN to Cross-Attention</h1>
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">2️⃣ Self-Attention vs. Cross-Attention</h1>
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">3️⃣ Adapting the Training Objective for Latent Space: $v$-prediction</h1>
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">4️⃣ Perceptual Weighting in Latent Space</h1>
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">1️⃣ A Two-Stage Cascade Pipeline: The Base and Refiner Models</h1>
...
$$
\begin{align}
\frac{\partial \mathcal{L}}{\partial x_l} & = \frac{\partial \mathcal{L}}{\partial x_{l+1}} \cdot \frac{\partial x_{l+1}}{\partial x_{l}} = g_{l+1} \cdot J(x_l + F(x_l)) \cdot (I+F'(x_l))\\[10pt]
& \approx (I + F'(x_l)) \cdot \frac{1}{\sigma} \cdot g_{l+1}.
\end{align}
$$
...
$$
|g_0|_2 ;\lesssim; \Big(\prod_{\ell=1}^L \|I+F'_\ell\|*2\Big)
\Big(\prod_{\ell=1}^L \frac{1}{\sigma_\ell}\Big) \|g_L\|*2.
$$

So **the product of LN scalings $\prod (1/\sigma_\ell)$** can force vanishing or exploding gradients. For Pre-LN block 

$$
x_{t+1}=\mathrm{LN}(x_l + F(x_l))
$$
...
= g_{l+1} \;+\; \big(J^{\text{(pre)}}\;l\big)^\top,\big(F'_l(u_l)\big)^\top,g_{l+1}.
$$

**Key consequence.** There is an **identity bypass** term $$g_{l+1}$$ that does **not** pass through any LN Jacobian.
Thus, even though the LN Jacobian does appear **inside the residual branch**, the cross-layer gradient always includes a clean identity path:

$$
g_l \;=\; \underbrace{g_{l+1}}_{\text{bypass}} ;+; \underbrace{(J^{\text{(pre)}}_l)^\top(F'_l)^\top g_{l+1}}_{\text{perturbation}}.
$$

With standard residual scaling (making $|F'_l|$ modest), this keeps the product of layer-wise maps close to an isometry, hence **stable depth**. In the extreme linearized picture with (F'_l) small,

$$
\|g_l\|_2 \approx \|g_{l+1}\|_2,
$$
