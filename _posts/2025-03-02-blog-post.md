...
- Latent U-Net's Approach (Localized Conditioning): Latent U-Nets instead integrate text tokens (from a frozen text encoder, e.g., CLIP) using **cross-attention** at many/most resolutions: Instead of modulating activations, the U-Net directly incorporates the text prompt's token embeddings at multiple layers. A text encoder (e.g., CLIP) first converts the prompt "a cat on a chair" into a sequence of token embeddings: [<start>, a, cat, on, a, chair, <end>]. These embeddings form the **Key (K)** and **Value (V)** for the cross-attention mechanism.

  The U-Net's spatial features act as the **Query (Q)**. At each location in the feature map, the Query can "look at" the sequence of text tokens and decide which ones are most relevant. A region of the feature map destined to become the cat will learn to place high attention scores on the "cat" token, while a region for the chair will attend to the "chair" token.
  
  $$\text{Attn}(Q,\,K,\,V)=\text{softmax}(QK^T/\sqrt{d})V,\quad Q=\text{latent features},\quad K,V=\text{text tokens}$$
