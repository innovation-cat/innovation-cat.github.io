...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section3">3. Scaling & Stability</a>
      <ul>
        <li><a href="#section3.1">3.1 Normalization Evolution: From Stabilizer to Controller</a></li>
		  <ul>
            <li><a href="#section3.1.1">3.1.1 Classical Normalization Techniques in Deep Learning</a></li>
            <li><a href="#section3.1.2">3.1.2 From Stabilizer to Controller: Why Normalization Became the Injection Point</a></li>
            <li><a href="#section3.1.3">3.1.3 U-Net Architectures: GroupNorm and AdaGN</a></li>
            <li><a href="#section3.1.4">3.1.4  The Transformer Paradigm: LayerNorm, Gating, and AdaLN-Zero</a></li>
          </ul>
...
For convolutional architectures like the U-Net, the data is structured as a feature map of shape $(N, C, H, W)$. The core assumption of a CNN is that each channel $C$ represents a specific feature detector (e.g., an edge detector), and its statistical properties should be consistent across the spatial dimensions $(H, W)$.

This makes Group Normalization (GroupNorm) the natural choice. It computes normalization statistics across the spatial dimensions $(H, W)$ within groups of channels, respecting the semantic integrity of each feature channel. Using Layer Normalization here would be nonsensical, as it would average across disparate feature channels, conflating the signals of an "edge detector" with a "color detector."

Building upon this foundation, Adaptive Group Normalization (AdaGN)—also known as Feature-wise Linear Modulation (FiLM)—was introduced. It injects the conditional signal $c$ by having it predict the $\gamma$ and $\beta$ parameters for the GroupNorm layer on the fly:

$${\text {AdaGN}}(x,c)=\gamma(c)⋅{\text {GN}}(x)+\beta(c)$$

This mechanism allows the timestep to control the "intensity" (via γ) and "bias" (via β) of each feature detector's response across its entire spatial canvas. For a small t (low noise), it might learn to amplify channels responsible for fine detail. For a large t (high noise), it might suppress them in favor of channels representing global structure. This architectural choice is a direct consequence of the U-Net's convolutional nature.

The following figure shows how to inject conditional signal using AdaGN in an UNET residual block.


![adaln](/images/posts/2025-03-02-blog-post/adagn.jpg)
