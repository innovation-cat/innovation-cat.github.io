...
Before adaptive normalization became the central mechanism for stability and conditioning in diffusion models, the deep learning community relied on several **classical normalization methods**. These techniques were originally designed as **stabilizers** to enable deep networks to train reliably. Each method differs in **how the mean and variance are computed** across the input tensor $x \in \mathbb{R}^{(N, C, H, W)}$, where $N$ is batch size, $C$ is the number of channels, and $H, W$ are spatial dimensions.

- **Batch Normalization (BN)**: **Computation** (per channel, across batch and spatial dimensions):
...
- **Layer Normalization (LN)**: **Computation** (per sample, across all channels and spatial positions):
...
- **Instance Normalization (IN)**: **Computation** (per sample, per channel, across spatial dimensions):
...
- **Group Normalization (GN)**: **Computation** (per sample, per group of channels + spatial dimensions):

Divide $C$ channels into $G$ groups. For group $g$:
...
$$
\mu_{n,g} = \frac{1}{\frac{C}{G} \cdot H \cdot W} \sum_{c \in g} \sum_{h=1}^H \sum_{w=1}^W x_{n,c,h,w},
\quad 
\sigma_{n,g}^2 = \frac{1}{\frac{C}{G} \cdot H \cdot W} \sum_{c \in g} \sum_{h=1}^H \sum_{w=1}^W (x_{n,c,h,w} - \mu_{n,g})^2
$$
