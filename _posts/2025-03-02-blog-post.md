      ...
      <ul>
        ...
        <li><a href="#section3.3">3.3 Stabilization in DiT Architectures</a>
		  <ul>
            ...
            <li><a href="#section3.3.2">3.2.2 The Signal Pathways: Enabling Deep Stacks in DiT</a></li>
...
- **Decoder feature**: $\mathbf{d}\in\mathbb{R}^{C\times H\times W}$
- **Skip feature**: $\mathbf{s}\in\mathbb{R}^{C\times H\times W}$
- **Fusion output**: $\mathbf{y}$
- **Normalization**: $\mathrm{Norm}(\cdot)$ (GN/AdaGN, etc.)
- **Residual input / function**: $\mathbf{x}, f(\cdot)$
- **Scaling factor**: $\alpha\in\mathbb{R}$ (scalar) or $\alpha\in\mathbb{R}^C$ (per-channel)
- **Gate**: $g_t\in[0,1]^C$, conditioned on time step $t$ or embedding $h(t,\text{cond})$
- **Noise scale**: $\sigma_t$
- **Drop mask**: $m\sim\mathrm{Bernoulli}(p)$
- **Fourier transform**: $\mathcal{F}(\cdot)$, with low/high-frequency splits $$\mathbf{s}_{LP}, \mathbf{s}_{HP}$$

## 💡 Variance and Amplitude Control


In a U-Net, decoder features $\mathbf{d}$ are fused with skip features $\mathbf{s}$ (from the encoder) to form:

$$
\mathbf{y} = \mathbf{d} + \alpha\,\mathbf{s}.
$$

The variance of the fused output is:

$$
\mathrm{Var}(y) = \mathrm{Var}(d) + \alpha^2 \mathrm{Var}(s) + 2\alpha\,\mathrm{Cov}(d,s).
$$

Several issues arise:

- **Variance explosion.** If $\mathbf{d}$ and $\mathbf{s}$ are weakly correlated and both unit variance, then $\mathrm{Var}(y)\approx 1+\alpha^2$. Across many layers, this compounds and causes exploding activations or unstable gradients.

- **Magnitude mismatch.** Shallow skip features often have much larger variance ($\mathrm{Var}(s)\gg \mathrm{Var}(d)$). Even with moderate $\alpha$, the skip dominates, flooding the decoder with unstable signals.

- **Covariance amplification.** When $\mathbf{d}$ and $\mathbf{s}$ are strongly correlated, the cross term inflates variance further.

- **Concatenation with projection.** When $\mathbf{y} = W[\mathbf{d},\mathbf{s}]$, random initialization of $W$ effectively mixes and amplifies both branches, again risking variance blow-up.



**Goal**: The purpose of **Variance and Amplitude Control** is to **keep the fused signal well-scaled and balanced**, ensuring that skip features and decoder features contribute in proportion without overwhelming each other. This prevents numerical instability, preserves semantic/depth balance, and enables smooth gradient propagation.

---

- **Skip Scaling**: The idea of this strategy is to explicitly scale skip features with a tunable factor to control their dominance.


  $$
  \mathbf{y} = \mathbf{d} + \alpha\,\mathrm{Norm}(\mathbf{s})
  \quad\text{or}\quad
  \mathbf{y} = \frac{\mathbf{d} + \alpha\,\mathrm{Norm}(\mathbf{s})}{\sqrt{1+\alpha^2}}
  $$
  
  **Problem solved:** prevents skip features from overwhelming the decoder; balances high- vs low-level contributions.
  **Applicability:** ✅ Generic U-Net problem.

- **Variance Renormalization**: After fusing branches, rescale outputs to keep the variance constant across layers.


  $$
  \mathbf{y} = \frac{\mathbf{d} + \alpha\,\mathbf{s}}{\sqrt{1+\alpha^2}}.
  $$

  (A more precise variant can include correlation $\rho$: $\mathbf{y}=\tfrac{\mathbf{d}+\alpha\,\mathbf{s}}{\sqrt{1+\alpha^2+2\alpha\rho}}$).

  **Problem solved:** mitigates variance growth layer by layer, avoiding activation explosion.
  **Applicability:** ✅ Generic.

- **Skip Normalization**: The idea of this strategy is to normalize skip features before fusion so their distribution matches the decoder.


  $$
  \hat{\mathbf{s}} = \mathrm{Norm}(\mathbf{s}),\quad
  \mathbf{y} = \mathbf{d} + \alpha\,\hat{\mathbf{s}}.
  $$

  **Problem solved:** shallow skip features often have higher variance and unstable statistics; normalization aligns them with deep decoder features.
  **Applicability:** ✅ Generic.


Variance and Amplitude Control strategies address the *universal* stability risks of U-Net skip fusion. By scaling, normalizing, and renormalizing skips, these methods prevent variance explosion, ensure balanced contributions, and keep training stable across depth. These strategies are fundamental for both **generic U-Nets** and **diffusion U-Nets**, where imbalance is exacerbated by noisy inputs.
...
- **Batch Normalization (BN) [^bn]**: normalizes per channel across the batch and spatial dimensions. Highly effective in CNNs trained with large batches (e.g., ResNet on ImageNet).
...
- **Layer Normalization (LN) [^ln]**: normalizes per sample across all channels, making it batch-size independent and ideal for sequence models (Transformers, RNNs).
...
- **Instance Normalization (IN) [^in]**: normalizes per sample per channel, used widely in style transfer and image generation for its per-instance invariance.
...
- **Group Normalization (GN) [^gn]**: divides channels into groups and normalizes within each group. Stable under small batch sizes and well-suited to high-resolution CNNs. Divide $C$ channels into $G$ groups. For group $g$:
...
<h1 id="section3.1.3" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.1.3 U-Net Architectures: GroupNorm and AdaGN
</h1>
...
<h1 id="section3.1.4" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.1.4 The Transformer Paradigm: DiT, Gating, and AdaLN-Zero
</h1>
