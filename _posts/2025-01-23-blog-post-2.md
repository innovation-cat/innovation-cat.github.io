...
Following equation, we can dirrectly build a network $\mu_{\theta}$ to output $\mu_{tar}$. However, $\mu_{\tar}$ is an affine function of $x_t$, which is known at training and test time; there is no need for the network to “reproduce” it. If we regress $\mu_{tar}$ directly, the network wastes capacity relearning a large known term and must also learn the residual that actually depends on the unknown clean content.

Instead of asking the network to output
​
  directly, Ho et al. propose to let it predict the noise that generated 
