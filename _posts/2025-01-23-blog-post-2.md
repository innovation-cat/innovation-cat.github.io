...
### Case A: When $x_0$ is Gaussian (closed forms)
...
4. **Score-prediction** ($\tilde s = -\epsilon/b_t$)
...
### Case B: Non-Gaussian data prior (realistic)

For real-world data, $x_0$ is highly non-Gaussian (e.g., images). Then:

- $\mathbb E[y\mid x_t]$ is a nonlinear denoiser;
- $\mathrm{Var}(y\mid x_t)$ has no closed form;
- **But the total variance decomposition still holds**, and the **qualitative SNR trends** remain the same, since the corruption channel is Gaussian.

**Qualitative behaviors:**

* **ε-prediction:** low-SNR ⇒ small variance (stable); high-SNR ⇒ large variance (unstable).
...
* **Score:** inherits ε’s trend but amplified by $1/b_t$; variance grows with SNR ⇒ high-SNR instability.

---

### Practical takeaway

* Conditional variance tells you the **gradient noise floor**.
* Combined with conditional mean (§5.2), it explains why:

  * ε is biased toward **low-SNR stability** but unstable in high-SNR,
  * $x_0$ is the opposite,
  * $v$ is balanced but weak,
  * score explodes in variance at high-SNR.
* Modern reweighting and preconditioning strategies aim to **suppress variance spikes** while **boosting weak-signal regions**.
