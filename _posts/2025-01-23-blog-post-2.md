...
The performance gap between prediction targets across SNR regimes stems from two core quantities:  
1. **Pearson correlation** $\,\rho(y_t, x_t)\,$ — measures how linearly predictable the target is from $x_t$.  
2. **Conditional mean coefficient** $\,k_y(t)=\mathrm{Cov}(y_t,x_t)/\mathrm{Var}(x_t)\,$ — the slope of the optimal linear predictor, directly tied to gradient magnitude and optimization stability.  

Under the joint Gaussian assumption, both admit closed forms. They share similar trends but reflect different aspects: $\rho$ captures *predictability*, while $k_y$ captures *optimization signal strength*.
...
**Figure 1: Pearson correlation vs SNR (four targets)**  
...
**Figure 2: Conditional mean coefficient vs SNR (four targets)**  
![Conditional mean coefficient vs SNR](/images/posts/post_2/condmean_vs_snr_fusion.png)
