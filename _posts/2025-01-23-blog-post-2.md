...
In $x_0$-prediction, the neural network is trained to directly estimate the clean original data $x_0$ from the noisy input $x_t$ at timestep $t$. Denoted as $\hat{x}_0 = x_{\theta}(x_t, t)$, this approach reparameterizes the predicted mean $\mu_{\theta}$ using the estimated $\hat{x}_0$. Substituting into Equation \ref{eq:8}, the mean becomes:
...
The $\epsilon$-prediction paradigm tasks the network with predicting the noise $\epsilon$ added during the forward process. this approach reparameterizes the predicted mean $\mu_{\theta}$ using the estimated $\epsilon_{\theta}$. Substituting into Equation \ref{eq:8}, the mean becomes:
...
Here, , and the network outputs $\hat{\epsilon} = \epsilon_{\theta}(x_t, t)$. The predicted mean $\mu_{\theta}$ is then derived as:

The corresponding loss is the MSE on the noise:
$$
