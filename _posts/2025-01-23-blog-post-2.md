...
## <a id="section4">4.1 Strategic Loss Weighting</a>
...
Where $w(t)=1$ in vanilla diffusion training. The function $w(t)$ acts as a precision-engineered "equalizer" for our training process. It amplifies the learning signal where it's naturally weak and  suppress the loss when the gradient variance is large to prevent instability. The goal is to design $w(t)$ such that the effective learning task is normalized across all timesteps (or all SNR regions).
...
- **SNR-Based weighting**: This is a simple, elegant, and effective strategy that provides a balanced solution by weighting the loss directly by the SNR: $$ w(t) = \text{SNR}(t) $$
  ...
  It provides a balanced trade-off, ensuring that neither end of the SNR spectrum dominates the training process. 
  
- ** Min-SNR Weighting**: min-snr [^min-snr] sets an upper bound to suppress the high-SNR region.
...
- ** Max-SNR Weighting**: max-snr [^max-snr] sets an lower bound to ensure a minimum learning signal.
