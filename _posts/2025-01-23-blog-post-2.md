...
where $y_t$ is the chosen target, and $x_t = a_t x_0 + b_t \epsilon$ is the noisy input. The key question becomes: **how much information does $x_t$ contain about $y_t$ at different noise levels?** This can be captured directly by the **Mutual Information (MI)** $I(y_t; x_t)$. It measures the reduction in uncertainty about the target $y_t$ after observing the input $x_t$.  
...
<h1 id="section3.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.1 Endpoint Analysis</h1>

Given the noisy input $$x_t = a_t x_0 + b_t \epsilon$$, we analyze the behavior of the diffusion process at its two extremes.

- **$x_0$-Prediction**: For target $y_t = x_0$
...
- **$\epsilon$-Prediction**: For target $y_t = \epsilon$
...
- **$v$-Prediction**: For target $y_t = v = a_t\epsilon - b_tx_0$
...
- **\text{score}-Prediction**: for target $y_t = s_t = -\epsilon / b_t$
...
- **$x_0$-Prediction**: The curve shape of $I(x_0, x_t)$ requires a powerful tool: **I-MMSE** (Information - Minimum Mean Square Error) [^immse] Identity.
  ...
  $$\text{MMSE}(\gamma) = \mathbb{E}[\|X - \mathbb{E}[X|Y]\|^2] = E(\text{Var(X \mid Y)})$ 
  
  is the minimum possible mean square error when estimating the signal $X$ from the observation $Y$. Since MMSE is always non-negative, as it is the expectation of a squared error. Therefore, the derivative of mutual information with respect to SNR is always **non-negative**. This implies that for a standard "signal corrupted by Gaussian noise" model, the mutual information between the signal and the observation is a **monotonically non-decreasing function of the SNR**.
  ...
  In this transformed model, we can identify: $X = x_0$ and $Y = \frac{x_t}{b_t}$, We can now apply the I-MMSE identity by differentiating with respect to SNR:
...
- **$\epsilon$-Prediction**: To analyze the curve shape of $I(\epsilon, x_t)$, we consider the relationship between Mutual Information and Conditional Entropy:
  ...
  **Conclusion:** Since $H(\epsilon|x_t)$ is monotonically increasing with SNR, it follows that $I(\epsilon; x_t)$ is a **strictly monotonically decreasing function of the SNR**.

<h1 id="section3.1.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.1.1 </h1>




<h1 id="section3.1.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.1.2 Overall Curve Shapes</h1>

While analyzing the endpoints provides strong intuition, a formal proof of the shape of the mutual information curves—specifically their monotonicity—requires a more powerful tool. The I-MMSE (Information - Minimum Mean Square Error) [^immse] identity provides the necessary mathematical foundation for this proof.
...
<h1 id="section3.4" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.4 A Deeper Look at Dependency: Mutual Information</h1>

While Pearson correlation in Section [3.1](#section3.1) provided a valuable lens into the *linear* predictability of our targets, it doesn't capture the full picture of statistical dependency. To measure the total relationship—both **linear** and **non-linear** parts, we turn to a more fundamental quantity from information theory: **Mutual Information (MI)**.

The mutual information $I(y_t; x_t)$ quantifies how much uncertainty about the target $y_t$ is reduced by observing the input $x_t$.

$$
I(y_t; x_t) = H(y_t) - H(y_t \mid x_t)
$$

A high MI indicates strong dependency, making the prediction task fundamentally easier, while a low MI signifies weak dependency, meaning the model must learn a more complex transformation.

---

<h1 id="section3.4.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">3.4.1 Case A: When $x_0$ is Gaussian (Simplified Setting)</h1>

A remarkable simplification occurs when we assume $x_0 \sim \mathcal{N}(0, I)$. In this case, the pair $(y_t, x_t)$ is jointly Gaussian for all our targets. For jointly Gaussian variables, the entire statistical dependency is captured by the linear relationship. As a result, mutual information has an elegant closed-form solution directly related to their Pearson correlation coefficient, $\rho$:

$$
I(y_t; x_t) & = H(y_t)+H(x_t)-H(y_t, x_t) = -\frac{1}{2} \log(1 - \rho^2(y_t, x_t))
$$

This elegant formula allows us to directly translate our previous linear analysis from Section 3.1 into a complete, information-theoretic one. Let's analyze each target.

-   **$\epsilon$-prediction**:
    We recall that $\rho^2_{\epsilon, x_t} = \frac{1}{1+\text{SNR}}$. Substituting this into the formula gives:
    
	$$
    I(\epsilon; x_t) = -\frac{1}{2} \log\left(1 - \frac{1}{1+\text{SNR}}\right) = \frac{1}{2} \log\left(1 + \frac{1}{\text{SNR}}\right)
    $$
    
	**Interpretation**:
    -   At **low SNR** (e.g., $\text{SNR} \to 0$), $1/\text{SNR}$ is large, making the MI very **high**. This is because $x_t \approx b_t\epsilon$, so observing $x_t$ provides a great deal of information about $\epsilon$.
    -   At **high SNR** (e.g., $\text{SNR} \to \infty$), $1/\text{SNR} \to 0$, making the MI approach **zero**. Here, $x_t \approx a_t x_0$, and the noise $\epsilon$ is just a tiny perturbation, so $x_t$ contains very little information about it.
    This confirms that the information required to predict $\epsilon$ is concentrated in the low-SNR regime.

-   **$x_0$-prediction**:
    Here, $\rho^2_{x_0, x_t} = \frac{\text{SNR}}{1+\text{SNR}}$. The mutual information is:
    
	$$
    I(x_0; x_t) = -\frac{1}{2} \log\left(1 - \frac{\text{SNR}}{1+\text{SNR}}\right)  = \frac{1}{2} \log(1 + \text{SNR})
    $$
    
	**Interpretation**:
    -   At **low SNR**, the MI is near **zero**. The input $x_t$ is almost pure noise and contains very little information about the original signal $x_0$.
    -   At **high SNR**, the MI is very **high**. The input $x_t$ is a nearly perfect representation of $x_0$, so observing it almost completely resolves the uncertainty about $x_0$.
    This confirms that the information required to predict $x_0$ is concentrated in the high-SNR regime.

-   **$v$-prediction**:
    We found that $\rho_{v, x_t} = 0$. Substituting this gives a striking result:
   
	$$
    I(v; x_t) = -\frac{1}{2} \log(1 - 0^2) = 0
    $$
    
	**Interpretation**:
    Under the Gaussian prior, there is **zero** mutual information between the velocity target $v$ and the input $x_t$ at all noise levels. This is a direct and powerful consequence of a key property of Gaussian distributions: **zero correlation implies statistical independence**. While this seems extreme, it highlights that, from an information-theoretic standpoint in this simplified world, $x_t$ provides no direct information to aid in the prediction of $v$.

-   **Score-prediction**:
    The score is a deterministic, invertible scaling of the noise, $s = -\epsilon/b_t$. A core property of mutual information is that it is invariant to invertible transformations applied to either variable. Therefore:
    
	$$
    I(s; x_t) = I(\epsilon; x_t) = \frac{1}{2} \log\left(1 + \frac{1}{\text{SNR}}\right)
    $$
    
	**Interpretation**:
    The information dynamics for score prediction are identical to those for $\epsilon$-prediction, with the information concentrated at low SNR.

---

<h1 id="section3.5" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.5 Summary and Comparative Table</h1>
...
### <a id="section4.1.1">4.1.3 From σ-space perspective</a>
...
**Zero Terminal SNR (ZTSNR)** [^ZTSNR] is an effective solution to this problem, which enforces the SNR at the final timestep $SNR(T)=0$ to be exactly zero — i.e., the sample at the terminal step should be pure Gaussian noise without any residual data signal.
...
## <a id="section4.3">4.3 Stabilization in σ-space (EDM and successors)</a>
...
> **Important: All subsequent design choices—preconditioning coefficients, loss weights, training distributions—are defined as functions of $\sigma$ rather than $t$**.
...
- **$c_{in}(\sigma)$ (Input Scaling)**: This term aims to counteract the scale variation of the input $x(σ)$. To give $f_{\theta}$'s input a constant variance, $c_{in}(\sigma)$ must cancel out the variance of $x(\sigma)$. Letting the clean data variance be $\sigma_{\text data}^2 = \mathrm{Var}(x_0)$, we have $\mathrm{Var}(x(\sigma)) = \sigma_{\text data}^2 + \sigma^2$. EDM therefore chooses:
...
An important perspective is that the preconditioned network $F_\theta$ acts as a **unified interface** (or container). Regardless of which prediction target we prefer — $x_0$, $\epsilon$, $v$, or the score — all can be obtained from $D_\theta$ by a **fixed linear transformation**.
...
| Score ($\nabla_x \log p(x;\sigma)$) | $s(x;\sigma) = \tfrac{D_\theta(x;\sigma) - x}{\sigma^2}$ | EDM’s denoising–score matching identity.                                                                            |
