	...
	<li><a href="#section4">4. Stabilizing Diffusion Training via Optimization & Objective Design</a>
		<ul>
			...
			<li><a href="#section4.4">4.4 General Engineering Stabilization Techniques</a>
				<ul>
					<li><a href="#section4.4.1">4.4.1 Gradient Cliping and Gradient Normalization</a></li>
...
Given the noisy input $$x_t = a_t x_0 + b_t \epsilon$$, we analyze the behavior of the diffusion process at its two extremes: **High-SNR Limit** and **Low-SNR Limit**.
...
<h1 id="section4.3.1.1" style="color: #64748B; font-size: 20px; font-weight: bold; text-decoration: underline;">From $t$-space to $\sigma$-space</h1>
...
<h1 id="section4.3.1.2" style="color: #64748B; font-size: 20px; font-weight: bold; text-decoration: underline;">EDM Preconditioning Formulation</h1>
...
<h1 id="section4.3.1.3" style="color: #64748B; font-size: 20px; font-weight: bold; text-decoration: underline;">EDM as a Unified Interface for different targets</h1>
...
<h1 id="section4.4.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">4.4.1 Gradient clipping</h1>


One of the most common failure modes in diffusion training is **exploding gradients**, particularly in high-SNR timesteps or early training iterations. Gradient clipping prevents this by enforcing an upper bound on the global norm of the gradient:

$$
g \leftarrow \frac{g}{\max\!\left(1, \frac{\|g\|_2}{\tau}\right)},
$$

where \$g\$ is the raw gradient, and \$\tau\$ is the clipping threshold (e.g., \$\tau = 1.0\$). This ensures \$|g|\_2 \leq \tau\$ at all times.

**Why it works**: prevents large, unstable parameter updates that could derail optimization; also reduces sensitivity to noisy mini-batches.
...
| -------------- | ----------------| ----------------- | ----------------- | ------------------------------------- | ------------------------------------- | ------------------------- | --------------------------------| -------------------------------- |
| **FP32**       | 1/8/23                   | $(2 - 2^{-23}) 2^{127} \approx 3.4028 × 10^{38}$  | $ 2^{-126} \approx 1.1755 × 10^{-38}$  | $2^{-149} \approx 1.4013× 10^{-45}$   | $2^{-23}$       | Baseline; Default training precision | **NVIDIA**: all GPUs<br>**AMD**: all GPUs<br>**Google TPU**: all versions                                       |
| **FP16**       | 1/5/10                   | $(2 - 2^{-10}) 2^{15} \approx 6.5504 × 10^{4}$  | $ 2^{-14} \approx 6.1035 × 10^{-5}$  | $2^{-24} \approx 5.9605× 10^{-8}$   | $2^{-10}$       | High throughput; narrow range, requires loss scaling                  | **NVIDIA**: Pascal P100+, Tensor Cores from Volta V100<br>**AMD**: Instinct MI-series<br>**Google TPU**: v2, v3 |
| **BF16**       | 1/8/7                    | $(2 - 2^{-7}) 2^{127} \approx 3.3895 × 10^{38}$  | $ 2^{-126} \approx 6.1035 × 10^{-5}$  | $2^{-133} \approx 9.1835× 10^{-41}$   | $2^{-7}$       | FP32-like range, coarser mantissa; usually no loss scaling            | **NVIDIA**: A100, H100<br>**AMD**: MI200, MI300<br>**Google TPU**: v2+ (native)<br>**Intel**: Habana Gaudi      |
| **FP8 (E4M3)** | 1/4/3                    | IEEE: $(2 - 2^{-3}) 2^{7} = 240$ <br> NVIDIA extended: 448 | $ 2^{-6} \approx 1.5625 × 10^{-2}$  | $2^{-9} \approx 1.9531 × 10⁻³$   | $2^{-3}$       | Experimental training/inference; requires calibration                 | **NVIDIA**: Hopper H100 Tensor Cores<br>**Intel**: Habana Gaudi2                                                |
| **FP8 (E5M2)** | 1/5/2                    | $(2 - 2^{-2}) 2^{15} = 57,344$ | $ 2^{-14} \approx 6.1035 × 10^{-5}$  | $2^{-16} \approx 1.5259 × 10^{-5}$   | $2^{-2}$       | Experimental training/inference; larger range, lower precision        | **NVIDIA**: Hopper H100 Tensor Cores<br>**Intel**: Habana Gaudi2                                                |




| Format | Bits (sign/exp/mantissa) | Max Positive | Min Positive Normal | Min Positive Subnormal | Relative precision | Typical usage | Hardware support |
|--------|:------------------------:|:------------:|:-------------------:|:----------------------:|--------------------|---------------|-----------------|
| **FP32** | 1 / 8 / 23 | $(2 - 2^{-23}) \cdot 2^{127} \;\approx\; 3.4028 \times 10^{38}$ | $2^{-126} \;\approx\; 1.1755 \times 10^{-38}$ | $2^{-149} \;\approx\; 1.4013 \times 10^{-45}$ | $2^{-23}$ | Baseline; default training precision | **NVIDIA**: all GPUs <br> **AMD**: all GPUs <br> **Google TPU**: all versions |
| **FP16** | 1 / 5 / 10 | $(2 - 2^{-10}) \cdot 2^{15} \;\approx\; 6.5504 \times 10^{4}$ | $2^{-14} \;\approx\; 6.1035 \times 10^{-5}$ | $2^{-24} \;\approx\; 5.9605 \times 10^{-8}$ | $2^{-10}$ | High throughput; narrow range, requires loss scaling | **NVIDIA**: Pascal P100+ <br> Tensor Cores from Volta V100 <br> **AMD**: Instinct MI-series <br> **Google TPU**: v2, v3 |
| **BF16** | 1 / 8 / 7 | $(2 - 2^{-7}) \cdot 2^{127} \;\approx\; 3.3895 \times 10^{38}$ | $2^{-126} \;\approx\; 1.1755 \times 10^{-38}$ | $2^{-133} \;\approx\; 9.1835 \times 10^{-41}$ | $2^{-7}$ | FP32-like range, coarser mantissa; usually no loss scaling | **NVIDIA**: A100, H100 <br> **AMD**: MI200, MI300 <br> **Google TPU**: v2+ (native) <br> **Intel**: Habana Gaudi |
| **FP8 (E4M3)** | 1 / 4 / 3 | IEEE: $(2 - 2^{-3}) \cdot 2^{7} = 240$ <br> NVIDIA extended: 448 | $2^{-6} \;\approx\; 1.5625 \times 10^{-2}$ | $2^{-9} \;\approx\; 1.9531 \times 10^{-3}$ | $2^{-3}$ | Experimental training/inference; requires calibration | **NVIDIA**: Hopper H100 Tensor Cores <br> **Intel**: Habana Gaudi2 |
| **FP8 (E5M2)** | 1 / 5 / 2 | $(2 - 2^{-2}) \cdot 2^{15} = 57{,}344$ | $2^{-14} \;\approx\; 6.1035 \times 10^{-5}$ | $2^{-16} \;\approx\; 1.5259 \times 10^{-5}$ | $2^{-2}$ | Experimental training/inference; larger range, lower precision | **NVIDIA**: Hopper H100 Tensor Cores <br> **Intel**: Habana Gaudi2 |
