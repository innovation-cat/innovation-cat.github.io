...
- **$c_{out}(\sigma)$ (output Scaling)**: Since $c_{skip}$ has handled the linear part, what is left for the expensive U-Net $f_{\theta}$ to learn? It must learn the difference: the **non-linear residual**. By rearranging the pre-conditioning formula, we can see the learning target for $f_{\theta}$: 
  ...
  & = \sqrt{\frac{\sigma\,\sigma_{\text data}}{\sigma_{\text data}^2 + \sigma^2}}
  \end{align}
  $$
...
An important perspective is that the preconditioned network $F_\theta$ acts as a **unified interface** (or container):

* Regardless of which prediction target we prefer — $x_0$, $\epsilon$, $v$, or the score — all can be obtained from $D_\theta$ by a **fixed linear transformation**.

  $$
  \hat x_0 = D_\theta(x;\sigma),\quad
  \hat\epsilon = \tfrac{x - D_\theta(x;\sigma)}{\sigma},\quad
  \hat s = \tfrac{D_\theta(x;\sigma)-x}{\sigma^2},\quad
  \hat v = a(\sigma)\hat\epsilon - b(\sigma)\hat x_0.
  $$

This means EDM does not lock us into a specific objective. Instead, it provides a container in which any target can be realized.
