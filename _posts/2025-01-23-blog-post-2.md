	...
	<li><a href="#section4">4. SOTA Solutions: Balancing SNR in Diffusion Training</a></li>
		<ul>
			...
			<li><a href="#section4.2">4.2 EDM-style Preconditioning</a></li>
				<ul>
					<li><a href="#section4.2.1">4.2.1 From t-space to σ-space</a></li>
					<li><a href="#section4.2.2">4.2.2 EDM Preconditioning Formulation</a></li>
				</ul>
...
To remove scale inconsistencies across different noise levels, Karras et al. [^edm] introduced a pivotal technique: **Network Pre-conditioning**.  The core idea is to use simple, analytically-defined scaling functions to "normalize" the network's inputs and outputs. This ensures the core network $f_{\theta}$ always operates in a **well-behaved "comfort zone"** where its inputs and outputs have roughly constant, near-unit variance.. EDM expresses the pre-conditioned network (denoiser) as:
...
-   **$c_{out}(\sigma)$ and $c_{skip}(\sigma)$ (Output and Skip Scaling)**: This is the most ingenious part of the pre-conditioning framework. The pre-conditioned network $F_{\theta}$ can be redefined as:
...
The choices of $c_{out}$ and $c_{skip}$ **jointly define the final prediction target of the entire network `F_θ` while ensuring the learning task for the inner network `f_θ` remains stable.**
    
    To understand this, let's assume we want the overall network `F_θ` to predict the clean data `x_0`. We then have:
    $$
    x_0 \approx F_{\theta}(x(\sigma), \sigma) = c_{\text{out}}(\sigma) \cdot f_{\theta}(...) + c_{\text{skip}}(\sigma) \cdot x(\sigma)
    $$
    By rearranging this equation, we can see what the inner network `f_θ` is actually tasked with learning:
    $$
    f_{\theta}(...) \approx \frac{x_0 - c_{\text{skip}}(\sigma) \cdot x(\sigma)}{c_{\text{out}}(\sigma)}
    $$
    EDM designs the functional forms of `c_skip(σ)` and `c_out(σ)` (e.g., `c_skip(σ) = σ_data² / (σ² + σ_data²)`) precisely so that this learning target for `f_θ` has near-unit variance across all values of `σ`. This vastly simplifies the task for `f_θ`: it no longer needs to predict a target with a wildly fluctuating scale, but rather a well-behaved function in a stable, "normalized space." The skip connection `c_skip(σ) · x(σ)` provides the "easy part" of the prediction, while `f_θ` focuses on learning the more difficult residual.
---
