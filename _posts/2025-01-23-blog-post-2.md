...
A fundamental principle of statistics states that to minimize the loss function (equation \ref{eq:20}), the optimal model f_{\theta}^{\star} must predict the conditional expectation of the target y given the input $x_t$, i,e., $f_{\theta}^{\star} = \mathbb{E}(y\|x_t)$. To understand why one target $y$ is better than another, we must analyze the properties of its conditional mean.
