...
- **Pearson Correlation Coefficient**: Measuring Linear Task Difficulty
...
<h1 id="section3.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">3.1 Pearson Correlation Coefficient: Measuring Linear Task Difficulty</h1>


Pearson Correlation Coefficient measures how predictable a target \$y\_t\$ is from the noisy input \$x\_t\$ via a *linear* relationship:

$$
\rho_t = \frac{\mathrm{Cov}(y_t, x_t)}{\sqrt{\mathrm{Var}(y_t)\,\mathrm{Var}(x_t)}}\label{eq:18}
$$

- **High correlation:** the target is strongly aligned with \$x\_t\$, making the **linear part** of the regression problem easier.
- **Low correlation:** the target is weakly coupled to \$x\_t\$, making  the **linear part** of the regression problem harder.

To analyze this, we make the standard assumptions that our clean data $x_0$ and noise are normalized to have zero mean and unit variance, and are statistically independent with $\epsilon$. Note that this calculation does **not require**  $x_0$ to follow a Gaussian distribution. 

With $$x_t = a_t x_0 + b_t \epsilon$$ and $$ \text{SNR} = {a_t^2}/{b_t^2}$$, we can derive the correlations for our primary targets:
...
- **$v$-prediction**: Since $\mathrm{Cov}(v,x_t)=\mathrm{Cov}(a_t \epsilon - b_t x_0,a_t x_0 + b_t \epsilon)=0$, Substitute into Eq. \ref{eq:18}
  ...
  orthogonal to $x_t$ for all SNR, indicating a uniform linear difficulty (i.e., no linear signal to leverage).
...
$$
I(y_t; x_t) = -\frac{1}{2} \log(1 - \rho^2(y_t, x_t))
$$
...
-   **$\epsilon$-prediction**:
    ...
    $$
    I(\epsilon; x_t) = -\frac{1}{2} \log\left(1 - \frac{1}{1+\text{SNR}}\right) = -\frac{1}{2} \log\left(\frac{\text{SNR}}{1+\text{SNR}}\right) = \frac{1}{2} \log\left(1 + \frac{1}{\text{SNR}}\right)
    $$
    **Interpretation**:
...
-   **$x_0$-prediction**:
    ...
    $$
    I(x_0; x_t) = -\frac{1}{2} \log\left(1 - \frac{\text{SNR}}{1+\text{SNR}}\right) = -\frac{1}{2} \log\left(\frac{1}{1+\text{SNR}}\right) = \frac{1}{2} \log(1 + \text{SNR})
    $$
    **Interpretation**:
