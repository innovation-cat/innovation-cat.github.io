...
### Case A: When $x_0$ is Gaussian (simplified setting)
...
$$
\mathbb{E}(y_t \mid x_t) = \mathbb{E}(y_t) + \frac{ {\text Cov}(y_t, x_t)}{ {\text Var}(x_t)}(x_t - \mathbb{E}(x_t))\label{eq:20}
$$
...
-  **$ \epsilon $-prediction:** Substitute $y_t=\epsilon$ into Eq. \ref{eq:20}
   ...
   $$
   \begin{align}
   \mathbb{E}[\epsilon \mid x_t] & = \mathbb{E}(\epsilon) + \frac{{\text Cov}(\epsilon, x_t)}{{\text Var}(x_t)}(x_t - \mathbb{E}(x_t)) \\[10pt]
   & = b_tx_t
   \end{align}
   $$
...
-  **$ x_0 $-prediction:** Substitute $y_t=x_0$ into Eq. \ref{eq:20}
   ...
   $$
   \begin{align}
   \mathbb{E}[\epsilon \mid x_t] & = \mathbb{E}(x_0) + \frac{{\text Cov}(x_0, x_t)}{{\text Var}(x_t)}(x_t - \mathbb{E}(x_t)) \\[10pt]
   & = a_tx_t
   \end{align}
   $$
...
### Case B: When $x_0$ is non-Gaussian (realistic case)

For real datasets, $x_0$ is far from Gaussian (images, audio, text embeddings, etc.). Then:

* The conditional distribution $p(x_0 \mid x_t)$ is **highly non-Gaussian**.
* The conditional mean $\mathbb E[x_0 \mid x_t]$ is a **nonlinear denoiser** (e.g., MMSE estimator under AWGN channel).
* Similarly, $\mathbb E[\epsilon \mid x_t] = \tfrac{1}{b_t}(x_t - a_t \mathbb E[x_0 \mid x_t])$ is nonlinear in $x_t$.

Thus, the neat linear forms from Case A **do not hold exactly**. Instead, we can only describe **qualitative trends** based on information-theoretic arguments (I–MMSE identity) and intuitive limits:
...
* **Score-prediction:**
  Training target is $\tilde s=-\epsilon/b_t$. Its conditional mean is
  $\mathbb E[\tilde s \mid x_t] = -\mathbb E[\epsilon\mid x_t]/b_t$.
  Qualitative behavior mirrors ε-prediction: stronger at low-SNR, weaker at high-SNR.
  

> **Takeaway.** Although case B does not have a simple closed-form formula, its qualitative trends is consistent with case A. This explains why in practice different targets emphasize different parts of the SNR spectrum, even though all are formally equivalent.
