...
- **$c_in(\sigma)\cdot x(\sigma)` (Input Scaling)**: This term aims to counteract the scale variation of the input $x(σ)$. To give $f_{\theta}$'s input a constant variance, $c_in(σ)$ must cancel out the variance of $x(σ)$. Letting the clean data variance be $\sigma_{\text data}^2 = Var(x_0)$, we have $Var(x(σ)) = \sigma_{\text data}^2 + \sigma^2$. EDM therefore chooses:
  
  $$
  c_in(σ) = \frac{1}{\sqrt{\sigma^2 + \sigma_{\text data}^2}}
  $$
