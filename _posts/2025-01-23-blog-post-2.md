...
-   **`c_noise(t)` (Noise/Timestep Scaling)**: It's not just the input data that needs scaling. The timestep information provided to the network (often an embedding of `log(SNR)` or `log(σ_t)`) should also be standardized to ensure consistent conditioning.

-   **`c_out(t) · f_θ(...)` (Output Scaling)**: `c_out(t)` performs the inverse operation. The core network `f_θ`, operating in its stable internal environment, produces an output with approximately unit variance. `c_out(t)` is responsible for scaling this output back to the correct magnitude of our desired prediction target `y_t`. For example, if the target is the score, whose scale varies dramatically with `t`, `c_out(t)` will adjust the output of `f_θ` accordingly.

-   **`c_skip(t) · x_t` (Skip Connection Scaling)**: This is a brilliant design, drawing insight from modern architectures like ResNet. We know that many prediction targets (like `x_0` or `ε`) can be expressed as a linear combination of `x_t` and another unknown term (e.g., `x_0 = (x_t - b_t ε) / a_t`). This skip connection provides the `x_t` component of the solution "for free," allowing the core network `f_θ` to focus exclusively on learning the more difficult **residual part** (i.e., estimating the `ε` or `x_0`-related term from `x_t`). This dramatically simplifies the network's learning task.
