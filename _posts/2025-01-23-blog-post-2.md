...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section4">4. Stabilizing Diffusion Training</a></li>
		<ul>
			...
			<li><a href="#section4.4">4.4 Cross-space optimization & architecture tricks</a>
				<ul>
					<li><a href="#section4.4.1">4.4.1 General numerical stability</a></li>
					<li><a href="#section4.4.2">4.4.2 Optimizer configurations</a></li>
					<li><a href="#section4.4.3">4.4.3 Architecture & regularization</a></li>
				</ul>
			</li>
		</ul>
...
# <a id="section4">4. Stabilizing Diffusion Training</a>
...
### Why not uniform in $\sigma$ or uniform in $t$?

If one samples $\sigma$ uniformly in the linear domain, the overwhelming majority of samples lie in the **high-noise regime**, where input images contain little recognizable structure. Training then spends most of its budget fitting essentially uninformative gradients, while the low-noise, high-SNR regime is severely underrepresented.

Uniform sampling in $t$-space fares no better: since $\sigma(t)$ is a highly nonlinear mapping determined by the noise schedule, uniform in $t$ implicitly oversamples some SNR regions and undersamples others. The distribution depends heavily on the specific $\beta(t)$ schedule chosen, making it less robust across tasks.

### The case for log-uniform

A natural correction is to sample **uniformly in $\log\sigma$**. Intuitively, this allocates equal training budget to each *order of magnitude* of noise, ensuring that low-, mid-, and high-noise regions are all represented. This matches the fact that the **effective information content** of a noisy observation varies smoothly in log-SNR space, not in linear $\sigma$.

### Why log-normal instead of strict log-uniform?

EDM goes one step further and samples $\log\sigma$ from a Gaussian distribution, truncated to $[\sigma_{\min}, \sigma_{\max}]$. This has several advantages:

* **Centering around $\sigma_{\text{data}}$:** The residual signal (after preconditioning) is typically most informative when $\sigma \approx \sigma_{\text{data}}$, where the perturbation is neither negligible nor overwhelming. A log-normal places its peak here, focusing training on the most valuable regime.
* **Controlled coverage:** The variance parameter controls the width of the distribution, allowing practitioners to balance focus around $\sigma_{\text{data}}$ with coverage of extreme noise scales.
* **Stable tails:** Unlike strict log-uniform, log-normal avoids over-allocating probability mass to extreme $\sigma$ values, where gradients are unstable or uninformative.
* **Consistency with inference schedules:** Sampling strategies such as the $\rho$-power schedule used in EDM inference approximate uniform spacing in log-$\sigma$. Log-normal training ensures train-test alignment in distributional coverage.

Formally, EDM samples

$$
\log\sigma \sim \mathcal N(P_{\text{mean}}, P_{\text{std}}^2), \quad
\sigma = \exp(\log\sigma)\ \ \text{clipped to }[\sigma_{\min}, \sigma_{\max}],
$$

with $P_{\text{mean}} \approx \log\sigma_{\text{data}}$ and $P_{\text{std}}$ controlling coverage.

The following figure illustrates how different sampling distributions allocate training budget across noise scales:

* **Uniform in $\sigma$:** heavily biased toward the high-noise end.
* **Uniform in log-$\sigma$:** equal attention to each order of magnitude.
* **Log-normal:** concentrated around the most informative regime, with balanced tails.

*(Insert both PDF and CDF comparison plots here for maximum clarity.)*

In short, **σ-sampling is the lever that determines *where* training effort is spent**. By working in log-space and biasing toward the most useful region, EDM ensures balanced, efficient learning across scales.
