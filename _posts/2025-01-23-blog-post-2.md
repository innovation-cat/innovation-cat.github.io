...
-   **$c_{out}(\sigma)$ and $c_{skip}(\sigma)$ (Output and Skip Scaling)**: This is the most ingenious part of the pre-conditioning framework. The choices of $c_{out}$ and $c_{skip}$ **jointly define the final prediction target of the entire network `F_θ` while ensuring the learning task for the inner network `f_θ` remains stable.**
