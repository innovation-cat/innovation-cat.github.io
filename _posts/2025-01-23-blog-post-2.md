...
In $x_0$-prediction, the neural network is trained to directly estimate the clean original data $x_0$ from the noisy input $x_t$ at timestep $t$. Denoted the network as $ x_{\theta}(x_t, t)$, this approach reparameterizes the predicted mean $\mu_{\theta}$ using the estimated $x_0$. Substituting into Equation \ref{eq:8}, the mean becomes:
