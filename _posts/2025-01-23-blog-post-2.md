...
For each denoising step, both forward posterior $q(x_{t-1} \mid x_t, x_0)$ and backward posterior 
$p_{\theta}(x_{t-1} \mid x_t)$ are gaussian distributions, set $q \sim \mathcal{N}(\mu_{q}, \sigma_{q}^2I)$ and $p_{\theta} \sim \mathcal{N}(\mu_p, \sigma_{p}^2I)$. For two Gaussians with identical covariance:
