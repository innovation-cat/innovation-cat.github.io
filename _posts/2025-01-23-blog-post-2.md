...
## A unifying lens: the $(x_0,\epsilon)$ basis and SNR
...
### 2) Where the differences actually come from

For a per-step squared loss \(\mathcal L_t=\|y_t-\hat y_t\|^2\), two Gaussian facts drive training:

1. **Bayes linearity.** \(\mathbb E[y_t\mid x_t]=k(t)\,x_t\). The slope \(k(t)\) and the **irreducible error** \(\mathrm{Var}(y_t\mid x_t)\) depend on the target and on SNR.
2. **Gradient energy allocation.** Near the optimum,
   $$
   \frac{\partial \mathcal L_t}{\partial \hat y_t} = 2(\hat y_t-y_t),\quad
   \Rightarrow\quad
   \mathbb E\|\nabla_\theta \mathcal L_t\|^2 \;\propto\; \mathrm{Var}(y_t\mid x_t)\,\big\|\tfrac{\partial \hat y_t}{\partial \theta}\big\|^2.
   $$
   Absent special preconditioning, the **per-timestep gradient energy** is shaped by \(\mathrm{Var}(y_t\mid x_t)\): big where the target is noisy, small where it’s predictable.

These two quantities are easy to compute:

- **Covariance and correlation with \(x_t\):**
  $$
  \mathrm{Var}(x_t)=1,\quad
  \rho(y_t,x_t)=\frac{\mathrm{Cov}(y_t,x_t)}{\sqrt{\mathrm{Var}(y_t)}}.
  $$
- **Irreducible error:**
  $$
  \mathrm{Var}(y_t\mid x_t)=\mathrm{Var}(y_t)-\frac{\mathrm{Cov}(y_t,x_t)^2}{\mathrm{Var}(x_t)}.
  $$

Plugging \((x_0,\epsilon)\) algebra in, one obtains **closed forms** (all as functions of SNR):
- **Correlation** \(\rho(y_t,x_t)\):
  \[
  \rho_{\epsilon,x_t}=\frac{1}{\sqrt{1+\mathrm{SNR}}},\quad
  \rho_{x_0,x_t}=\sqrt{\frac{\mathrm{SNR}}{1+\mathrm{SNR}}},\quad
  \rho_{v,x_t}=0,\quad
  \rho_{s,x_t}=-\frac{1}{\sqrt{1+\mathrm{SNR}}}.
  \]
- **Irreducible error** \(\mathrm{Var}(y_t\mid x_t)\):
  \[
  \epsilon:\;\frac{\mathrm{SNR}}{1+\mathrm{SNR}}\;;\qquad
  x_0:\;\frac{1}{1+\mathrm{SNR}}\;;\qquad
  v:\;1\;;\qquad
  s:\;\mathrm{SNR}.
  \]

**Reading these curves:**
- \(\epsilon\) is **easy late** (low SNR; high correlation, low error) and **hard early** (high SNR).
- \(x_0\) is **easy early** and **hard late**—the mirror image.
- \(v\) is *uncorrelated* with \(x_t\) and has a **flat irreducible error** (constant 1): it spreads difficulty evenly.
- The score target’s *scale* grows like \(\sqrt{\mathrm{SNR}}\); without preconditioning it can be numerically unruly at high SNR.
