...
Diffusion training looks deceptively simple: draw a noisy pair $(x_0, x_t)$ from the forward process and regress a target $y_t$ with an MSE. Although the four targets — $\epsilon$-prediction, $x_0$-prediction, $v$-prediction, and score-prediction — are linearly transformable, in practice, **which target** you regress matters—a lot. Different targets place learning pressure on **different SNR bands**, change the **numerical conditioning** of the regression, and interact differently with **weighting/preconditioning** and the **sampler** you’ll use at test time. This section builds a unified, quantitative picture of *why* these differences arise and *how* to correct them.
...
It is well established that the four common training targets in diffusion models —  
$\epsilon$-prediction, $x_0$-prediction, $v$-prediction, and score-prediction —  
can all be expressed under a unified mean-squared error (MSE) loss formulation:
$$
\ell(\theta; x_t, y) = \|\, y - f_{\theta}(x_t, t) \,\|^2,
$$
where $y$ is the chosen target, and $f_{\theta}(x_t, t)$ is the model output given $x_t$ and timestep $t$.  
Although these targets are linearly interconvertible, empirical results show substantial differences in training dynamics and final performance depending on the target choice.
...
$$
\ell(\theta; x_t, y) = \frac12 \|\, y - f_{\theta}(x_t, t) \,\|^2, \quad L(\theta) = \mathbb{E}[\ell].
$$

#### Bayes-optimal predictor
For fixed $(x_t, t)$, the risk-minimizing predictor is:
$$
f^*(x_t, t) = \mu(x_t, t) := \mathbb{E}[\,y \mid x_t, t\,].
$$
...
#### Gradient structure
The gradient of the per-sample loss w.r.t. the model parameters is:
$$
...
For jointly Gaussian $(y_t, x_t)$ with $\mathrm{Var}(x_t)=1$:
$$
\mathbb{E}[y_t \mid x_t] \;=\; \mathrm{Cov}(y_t, x_t)\, x_t \;=\; k_y(t)\, x_t.
$$

- $\epsilon$-prediction: $k_\epsilon(t) = \dfrac{1}{\sqrt{1+\mathrm{SNR}}}$  
- $x_0$-prediction: $k_{x_0}(t) = \sqrt{\dfrac{\mathrm{SNR}}{1+\mathrm{SNR}}}$  
- $v$-prediction: $k_v(t) = 0$  
- score-prediction: with $\mathrm{Var}(s)=1+\mathrm{SNR}$ and $\rho_{s,x_t}=-\dfrac{1}{\sqrt{1+\mathrm{SNR}}}$,
$$
k_s(t) \;=\; \mathrm{Cov}(s,x_t) \;=\; \rho_{s,x_t}\,\sqrt{\mathrm{Var}(s)} \;=\; -1
$$
(a constant slope, independent of SNR).

![Conditional mean coefficient vs SNR](/images/posts/post_2/condmean.png)
