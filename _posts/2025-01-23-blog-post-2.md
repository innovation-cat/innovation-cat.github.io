...
<style>
  /* scope: only affect the block wrapped by #pretty-443 */
  #pretty-443 { position: relative; }

  /* base styling for Rouge blocks inside #pretty-443 */
  #pretty-443 .highlighter-rouge .highlight pre,
  #pretty-443 pre.highlight {
  ...
  /* code text color (let Rouge spans keep their colors) */
  #pretty-443 .highlighter-rouge .highlight code { 
    color: inherit;
  }
  ...
  #pretty-443 .copy-btn:hover { transform: translateY(-1px); }
  #pretty-443 .copy-btn:active { transform: translateY(0); }
  ...
  #pretty-443 .copy-btn svg { width: 14px; height: 14px; }

  /* position container so button sits over the code block */
  #pretty-443 .pretty-wrapper {
    position: relative;
  }

  /* ensure button stays inside when horizontally scrolling code */
  #pretty-443 .pretty-wrapper { padding-top: 36px; }
  #pretty-443 .copy-btn { top: 6px; right: 6px; }
</style>


<script>
(function () {
  const root = document.getElementById('pretty-443');
  if (!root) return;

  // 找到该容器内第一个 Rouge 代码块
  const pre = root.querySelector('.highlighter-rouge .highlight pre, pre.highlight');
  const code = pre ? pre.querySelector('code') : null;
  const btn  = root.querySelector('.copy-btn');

  if (!pre || !code || !btn) return;

  // 复制函数：优先 Clipboard API，回退到 execCommand
  async function copyCode() {
    const text = code.innerText; // 保留换行和缩进
    try {
  ...
  function btnSuccess() {
    ...
    btn.style.borderColor = 'rgba(16,185,129,0.6)'; // green-ish
    setTimeout(() => { label.textContent = prev; btn.style.borderColor=''; }, 1400);
  }
  function btnFail() {
    const label = btn.querySelector('span');
    const prev = label.textContent;
    label.textContent = 'Press Cmd/Ctrl+C';
...
## 🎯 Loss Scaling and Unscaling in Mixed-Precision Training
...
for x, y in dataloader:
	...
	scaler.scale(loss).backward()        # Scale loss → scaled gradients
	scaler.unscale_(optimizer)           # Unscale before clipping
	torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
	scaler.step(optimizer)               # Apply update in FP32
