...
| ----------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| Clean data ($x_0$)                  | $\hat x_0 = D_\theta(x;\sigma)$                               | $D_\theta$ is designed as an MMSE denoiser, i.e. the optimal estimate of $x_0$.                                     |
| Noise ($\epsilon$)                  | $\hat\epsilon = \tfrac{x - D_\theta(x;\sigma)}{\sigma}$       | Direct inversion of the corruption process $x = x_0 + \sigma\epsilon$.                                              |
| Score ($\nabla_x \log p(x;\sigma)$) | $\hat s(x;\sigma) = \tfrac{D_\theta(x;\sigma) - x}{\sigma^2}$ | EDM’s denoising–score matching identity.                                                                            |
| $v$-prediction                      | $\hat v = a(\sigma)\,\hat\epsilon - b(\sigma)\,\hat x_0$      | $v$ is a linear combination of $(x_0,\epsilon)$; coefficients $a(\sigma), b(\sigma)$ depend on the forward process. |
...
| $\epsilon$   | $\dfrac{\sigma}{\sigma_{\text{data}}^2+\sigma^2}$                 | $\dfrac{\sigma_{\text{data}}}{\sqrt{\sigma_{\text{data}}^2+\sigma^2}}$         | 同左                                                  | 同左                         | $1+\dfrac{\sigma^2}{\sigma_{\text{data}}^2}$                                |
| $v$（VP）      | $0$                                                               | $1$                                                                            | 同左                                                  | 同左                         |
| score（直接训练版） | $-\dfrac{1}{\sigma_{\text{data}}^2+\sigma^2}$                     | $\dfrac{\sigma_{\text{data}}}{\sigma\sqrt{\sigma_{\text{data}}^2+\sigma^2}}$   | 同左                                                  | 同左                         | 
