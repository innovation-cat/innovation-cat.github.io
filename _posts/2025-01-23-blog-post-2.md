...
Since $ x_t $ is observed, the above equality holds deterministically, Linking All Targets via $ \mathbb{E}[\epsilon \mid x_t] $, the four commonly used prediction targets:
...
2. **$ x_0 $-prediction:**
   Using $ \mathbb{E}[x_0 \mid x_t] = \frac{x_t - b_t \mathbb{E}[\epsilon \mid x_t]}{a_t} \),  
   we have:
   $$
   \mathbb{E}[x_0 \mid x_t] = \frac{x_t}{a_t} - \frac{b_t}{a_t} \mathbb{E}[\epsilon \mid x_t].
   $$

3. **$ v $-prediction:**
   With $ v = a_t \epsilon - b_t x_0 $, substituting the above conditional means yields:
   $$
   \mathbb{E}[v \mid x_t] = a_t \, \mathbb{E}[\epsilon \mid x_t] - b_t \, \mathbb{E}[x_0 \mid x_t],
   $$
