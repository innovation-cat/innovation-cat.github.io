...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section4">4. Stabilizing Diffusion Training</a></li>
		<ul>
			<li><a href="#section4.1">4.1 Root cause of DM instability and inefficiency</a>
				<ul>
					<li><a href="#section4.1.1">4.1.1 The Universal Language of Instability</a></li>
					<li><a href="#section4.1.2">4.1.2 Why Different Spaces</a></li>
				</ul>
			</li>	
...
# <a id="section4">4. SOTA Stabilizing Diffusion Training Solutions</a>
...
The instability of diffusion model training arises from a common root: **different noise levels correspond to learning signals and gradient magnitudes that vary drastically.** At low noise the task is almost trivial and gradients vanish, while at high noise the signal is almost absent and gradients explode. Stabilizing training therefore requires careful reparameterization, weighting, and sampling.

Over the past few years, researchers have approached this challenge from **different parameterization spaces**:

* **t-space (time-step parameterization):**
  The original DDPM/DDIM formulation defines the process through discrete timesteps and noise schedules.
  Stabilization here focuses on *reweighting timesteps* (e.g., p2, min-SNR) and *designing schedules* (linear, cosine, exponential).

* **SNR-space (signal-to-noise ratio):**
  Many weighting and scheduling methods in t-space can be reinterpreted in terms of SNR, which directly measures the visibility of data under noise. Stabilization here is motivated by *equalizing the information content* across training.

* **σ-space (noise-level parameterization, EDM):**
  More recent work abandons timesteps altogether and directly parameterizes the process by the continuous noise scale $\sigma$. Stabilization here comes from *analytic preconditioning*, *log-normal sigma sampling*, and *loss weights tied to preconditioning*.

These spaces reflect **different perspectives on the same instability problem**. While $t/{\text SNR}$ methods emphasize schedule design and signal rebalancing, $\sigma$-space methods focus on reparameterization and explicit normalization. Because the **optimization levers are fundamentally different**, we will discuss them in separate subsections rather than mixing them together.
...
### <a id="section4.2.2">4.2.2 EDM Preconditioning Formulation</a>
...
### 4.2.4 EDM as a Unified Interface for different targets</a>
