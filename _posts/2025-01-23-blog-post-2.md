...
In $x_0$-prediction, the neural network is trained to directly estimate the clean original data $x_0$ from the noisy input $x_t$ at timestep $t$. Denoted the network as $ x_{\theta}(x_t, t) = \hat{x}_0 $, this approach reparameterizes the predicted mean $\mu_{\theta}$ using the estimated $\hat{x}_0$. Substituting into Equation \ref{eq:8}, the mean becomes:
