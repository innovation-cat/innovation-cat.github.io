...
$$
\rho_t = \frac{\mathrm{Cov}(y_t, x_t)}{\sqrt{\mathrm{Var}(y_t)\,\mathrm{Var}(x_t)}}.
$$
...
**Per-target behavior (DDPM convention, \$\text{SNR}=a\_t^2/b\_t^2\$):**
...
For simplicity and convenience, we assume that the distribution of **$ x_0 $ follows a Gaussian distribution**. Under this assumption, the conditional mean $ E(y_t \mid x_t) $ results in a linear function of $ x_t $: 

$$
\mathbb{E}(y_t \mid x_t) = \mathbb{E}(y_t) + \frac{Cov(y_t, x_t)}{Var(x_t)}(x_t - \mathbb{E}(x_t)) 
$$
...
-  **$ \epsilon $-prediction:**
   ...
   Strong in low-SNR regime, weak in high-SNR regime. Learns heavily noised states well, but struggles when data is clean.
...
-  **$ x_0 $-prediction:**
   ...
   Strong in high-SNR regime, weak in low-SNR regime. Captures coarse structure early, but struggles with heavily noised states.
...
-  **$ v $-prediction:**
   ...
   More uniform but weaker signal (zero in Gaussian proxy). Stable but slower unless boosted.
...
**Takeaway.** Conditional mean magnitude determines **how much usable information exists**. \$\epsilon\$ and \$x\_0\$ are biased to opposite ends of SNR; \$v\$ is balanced but weak; score is uniformly strong in mean.
...
Near the optimum, the per-sample MSE gradient variance is proportional to:

$$
\mathrm{Var}(y_t \mid x_t).
$$
...
**Per-target behavior (qualitative trends):**

* **\$\epsilon\$-prediction.** High variance in **high-SNR** regime, low variance in low-SNR regime. → Stable when data is noisy, unstable when data is clean. Combined with §5.2 → strongly biased toward low-SNR.
* **\$x\_0\$-prediction.** High variance in **low-SNR** regime, low variance in high-SNR regime. → Stable when data is clean, unstable when data is noisy. Combined with §5.2 → strongly biased toward high-SNR.
* **\$v\$-prediction.** Constant variance across SNR. → Most stable overall, but slower progress due to weaker signal.
* **Score-prediction.** Variance grows with SNR. → Very noisy in high-SNR regime, clean in low-SNR regime. Even with uniform mean signal, instability arises in high-SNR regime.

**Takeaway.** Conditional variance indicates **gradient reliability**. Balanced training requires amplifying weak signals (§5.2) and damping variance spikes (§5.3).
