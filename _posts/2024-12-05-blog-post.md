...
<h1 id="section5.5" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.5 Memory Budget with Mixed Precision: Training vs. Inference</h1>
...
<h1 id="section5.5.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">5.5.1 Components and Precisions During Training</h1>

We will discuss the scenarios of without mixed-precision training and with mixed-precision training separately.
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">1️⃣ Pure FP32 Training</h1>
...
<h1 style="color: #212121; font-size: 21px; font-weight: bold; ">1️⃣ Mixed Precision Training (BF16/FP16)</h1>

During mixed precision training precedure, the following data needs to be stored in the memory.

- **Master weights (FP32)**: $W$  
- **Forward weights (low precision)**:  $W/2$ for FP16/BF16)  
- **Gradients (low precision storage; FP32 for accumulation/update)**: $W/2$ for FP16/BF16)  
- **Optimizer states (FP32)**: depends on optimizer  
  - Adam/AdamW: two moments $m,v \Rightarrow 2W$  
  - SGD+momentum: one moment $\Rightarrow W$ 
  - SGD (no momentum): $\Rightarrow 0$  
- **Activations (low precision)**: \(\tfrac{b_{\text{low}}}{4} \, A^{\text{train}}_{32}\)  (divide by \(c\) if using checkpointing)
