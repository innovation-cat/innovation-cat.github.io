...
<details style="background:#f6f8fa; border:1px solid #e5e7eb; border-radius:10px; padding:.6rem .9rem; margin:1rem 0;">
  ...
  <ul style="margin:0; padding-left:1.1rem;">
	...
	<li><a href="#section5">5. Mixed Precision Training</a>
		<ul>
		  <li><a href="#section5.1">5.1 Floating-Point Formats in Practice</a></li>
		  <li><a href="#section5.2">5.2 Reference Table: Precision Handling of Operators</a></li>
		  <li><a href="#section5.3">5.3 Loss Scaling and Unscaling in Mixed-Precision Training</a></li>
		  <li><a href="#section5.4">5.4 Operations That Should Use FP32</a></li>
		</ul>
	</li>
	<li><a href="#section6">6. Stabilizing Diffusion Training via Network Architecture</a></li>
...
<h1 id="section5" style="color: #1E3A8A; font-size: 27px; font-weight: bold; text-decoration: underline;">5 Mixed Precision Training</h1>
...
<h1 id="section5.1" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.1 Floating-Point Formats in Practice</h1>
...
<h1 id="section5.2" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.2 Reference Table: Precision Handling of Operators</h1>
...
<h1 id="section5.3" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.3 Loss Scaling and Unscaling in Mixed-Precision Training</h1>
...
<h1 id="section5.4" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.4 Operations That Should Use FP32</h1>
...
<h1 id="section5.5" style="color: #1E40AF; font-size: 25px; font-weight: bold; text-decoration: underline;">5.5 Memory Budget with Mixed Precision: Training</h1>
...
| $$P$$ | Number of model parameters | — | $$P = \tfrac{W}{4}$$ since FP32 uses 4 bytes per parameter. |
...
| $$A^{\text{infer}}_{32}$$ | Peak activation memory during **inference** under FP32 | Bytes / GB | Typically much smaller than $$A^{\text{train}}_{32}$$. |
| $$c$$ | Checkpointing reduction factor | — | $$c \ge 1$$; when using activation recomputation, activation memory scales by $$1/c$$. |
...
| $$M^{\text{infer}}_{\text{mix}}$$ | Total inference memory under mixed precision (FP16/BF16) | Bytes / GB | Includes low-precision weights and activations. |
| $$M^{\text{infer}}_{\text{FP32}}$$ | Total inference memory under full FP32 precision | Bytes / GB | Used as the upper bound for deployment memory. |


---

<h1 id="section5.5.1" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">5.5.1 Pure FP32 Training</h1>
...
- **Master weights (FP32)**: $W$  
- **Gradients (FP32)**: $W$
- **Optimizer states (FP32)**: depends on optimizer  
  - Adam/AdamW: two moments $m,v \Rightarrow 2W$  
  - SGD+momentum: one moment $\Rightarrow W$ 
  - SGD (no momentum): $\Rightarrow 0$  
- **Activations (FP32)**: $A^{\text{train}}_{32}$
...
<h1 id="section5.5.2" style="color: #1D4ED8; font-size: 23px; font-weight: bold; text-decoration: underline;">5.5.2 Mixed Precision Training (BF16/FP16)</h1>
...
| **Parameters** | Forward Weights | $$\text{FP16/BF16}$$ |   $$\tfrac{1}{2}W$$ | Low-precision copy used for forward/backward computation. |
| **Gradients** | Gradient Storage | $$\text{FP16/BF16}$$ | $$\tfrac{1}{2}W$$ | Computed in low precision, then cast to FP32 before updates. |
| **Optimizer States** | Adam/AdamW: $$m,v$$ | $$\text{FP32}$$ | $$2W$$ | Momentum and variance accumulators must remain FP32. |
| **Activations** | Intermediate Feature Maps | $$\text{FP16/BF16}$$ | $$\tfrac{1}{2}A^{\text{train}}_{32}$$ | Main source of memory saving. Further reduced to $$\tfrac{1}{2c}A^{\text{train}}_{32}$$ with checkpointing. |


For convenience, we will overlook other overheads such as input data, temporary buffers, and the framework's context overhead. Under pure FP32 precision, the total storage capacity is

$$
\boxed{
M^{\text{train}}_{\text{mix}} \;=\; 
\underbrace{W}_{\text{master}}
+ \underbrace{W\frac{b_{\text{low}}}{4}}_{\text{forward}}
+ \underbrace{W\frac{b_{\text{low}}}{4}}_{\text{grads}}
+ \underbrace{2W}_{\text{optimizer}}
+ \underbrace{\tfrac{b_{\text{low}}}{4}A^{\text{train}}_{32}}_{\text{activations}}
}
$$

For FP16/BF16 (\(b_{\text{low}}=2\)):
\[
\boxed{M^{\text{train}}_{\text{mix}} \;=\; 4W \;+\; \tfrac{1}{2}\,A^{\text{train}}_{32}} \quad (\text{Adam/AdamW})
\]

**Comparison — Pure FP32 (Adam/AdamW):**
\[
\boxed{M^{\text{train}}_{\text{FP32}} \;=\; 
\underbrace{W}_{\text{weights}}
+ \underbrace{W}_{\text{grads}}
+ \underbrace{2W}_{\text{optimizer}}
+ \underbrace{A^{\text{train}}_{32}}_{\text{activations}}
\;=\; 4W + A^{\text{train}}_{32}}
\]

> **Key takeaway.** With Adam/AdamW, the **(parameters + optimizer)** portion is \(4W\) in both pure FP32 and mixed precision.  
> Mixed precision mainly **halves activation memory** and boosts throughput.

---

### B) Components and Precisions During Inference

- **Weights**: FP32: \(W\)  →  FP16/BF16: \(W \cdot \tfrac{b_{\text{low}}}{4} = W/2\)  
- **Activations**: FP32: \(A^{\text{infer}}_{32}\)  →  FP16/BF16: \(\tfrac{b_{\text{low}}}{4} A^{\text{infer}}_{32} = \tfrac{1}{2} A^{\text{infer}}_{32}\)  
- No gradients, no optimizer states.

**Totals:**
\[
\boxed{M^{\text{infer}}_{\text{FP32}} \;=\; W + A^{\text{infer}}_{32}}, \qquad
\boxed{M^{\text{infer}}_{\text{mix}} \;=\; \tfrac{1}{2}W + \tfrac{1}{2}A^{\text{infer}}_{32}}
\]
...
| Component | Pure FP32 | Mixed Precision (FP16/BF16) |
|---|---:|---:|
| Weights (forward/master) | \(W\) | \(W \;(\text{master}) + \tfrac{1}{2}W \;(\text{forward})\) |
| Gradients (stored) | \(W\) | \(\tfrac{1}{2}W\) |
| Optimizer states | \(2W\) | \(2W\) |
| Activations | \(A^{\text{train}}_{32}\) | \(\tfrac{1}{2} A^{\text{train}}_{32}\) \(\big/\!c\) if checkpointing |
| **Total** | **\(4W + A^{\text{train}}_{32}\)** | **\(4W + \tfrac{1}{2} A^{\text{train}}_{32}\)** |

> For **SGD+momentum**, replace \(2W\) by \(W\); for **SGD (no momentum)**, replace by \(0\).  
> Mixed precision and pure FP32 remain identical on the parameter+optimizer portion; savings come from activations.

**Inference**

| Component | Pure FP32 | Mixed Precision (FP16/BF16) |
|---|---:|---:|
| Weights | \(W\) | \(\tfrac{1}{2}W\) |
| Activations | \(A^{\text{infer}}_{32}\) | \(\tfrac{1}{2} A^{\text{infer}}_{32}\) |
| **Total** | **\(W + A^{\text{infer}}_{32}\)** | **\(\tfrac{1}{2}W + \tfrac{1}{2} A^{\text{infer}}_{32}\)** |
...
Assume:
- **Weight FP32 footprint** \(W = 2\) GB (thus \(P = 0.5\) billion params),  
- **Training activations (FP32)** \(A^{\text{train}}_{32} = 6\) GB,  
- **Inference activations (FP32)** \(A^{\text{infer}}_{32} = 2\) GB.

**Training (Adam/AdamW)**  
- Pure FP32: \(M = 4W + A^{\text{train}}_{32} = 8 + 6 = \mathbf{14\ GB}\)  
- Mixed precision (FP16/BF16): \(M = 4W + \tfrac{1}{2}A^{\text{train}}_{32} = 8 + 3 = \mathbf{11\ GB}\)  
- With checkpointing \(c=2\): \(M \approx 8 + \tfrac{1}{2}\cdot 6 / 2 = 8 + 1.5 = \mathbf{9.5\ GB}\)

**Inference**  
- Pure FP32: \(M = W + A^{\text{infer}}_{32} = 2 + 2 = \mathbf{4\ GB}\)  
- Mixed precision (FP16/BF16): \(M = \tfrac{1}{2}W + \tfrac{1}{2}A^{\text{infer}}_{32} = 1 + 1 = \mathbf{2\ GB}\)

---

<h1 style="color: #212121; font-size: 20px; font-weight: bold;">Key Takeaways</h1>

- With Adam/AdamW, **parameters + optimizer states** cost **\(4W\)** regardless of precision;  
  mixed precision primarily **halves activation memory** and **accelerates compute**.  
- Inference sees near-**half total memory** when switching FP32 → FP16/BF16 for both weights and activations.  
- Additional savings come from **activation checkpointing** and **operator-level kernel choices** (e.g., FlashAttention).



Here’s a polished **English summary paragraph** that integrates both the previous analytical results and your concluding insight — suitable as the final paragraph of your *4.4.3 Mixed Precision Training* section:

---

### **Summary**

In summary, mixed precision training significantly improves computational efficiency in diffusion models but provides only limited reduction in parameter-related memory. Because **master weights** and **optimizer states** (e.g., Adam’s (m,v)) must remain in **FP32** for stable and precise updates, the overall parameter-related footprint (≈ 4 W) remains nearly unchanged compared to full-precision training. The true advantage of mixed precision lies in its handling of **activations**, which dominate the memory budget in deep diffusion architectures such as U-Nets and DiTs. Storing activations in FP16/BF16 reduces their footprint by roughly half, and further combining this with **activation checkpointing** can yield an additional 2–3× reduction. Consequently, mixed precision achieves **substantial overall memory savings** and **1.3–1.8× faster throughput**, while maintaining numerical stability through FP32 computation in critical operations (e.g., normalization, softmax, optimizer updates). In practice, it is best viewed not as a parameter-compression technique, but as a **precision-aware activation optimization strategy** that balances speed, memory, and training stability.


---

<h1 id="section6" style="color: #1E3A8A; font-size: 27px; font-weight: bold; text-decoration: underline;">6. Stabilizing Diffusion Training via Network Architecture</h1>
...
<h1 id="section7" style="color: #1E3A8A; font-size: 27px; font-weight: bold; text-decoration: underline;">7. References</h1>
